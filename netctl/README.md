...
`netctl` is a lightweight, cross-platform command-line tool designed to generate web server configurations abstractly. It acts as a universal abstraction layer over **Nginx**, **Caddy**, **Apache HTTPD**, and **IIS**.

It is implemented in pure POSIX `/bin/sh` for Unix-like systems and pure Windows Batch (`.cmd`) for Windows, relying only on `jq` for state management.
