...
## Overview
This document describes the `app` folder within the LibScript ecosystem. This directory serves as the root for deploying higher-level applications and services.

It works both as a local version manager (similar to rvm, nvm, pyenv, uv) and can be invoked from the global version manager `libscript`. Furthermore, the components within the `app` folder can be used by libscript to build bigger stacks (like WordPress, Open edX, nextcloud, etc.) by composing multiple dependencies together.
...
- **Install:** `libscript install app`
- **Uninstall:** `libscript uninstall app`
- **Start:** `libscript start app`
- **Stop:** `libscript stop app`
- **Package:** `libscript package_as docker app` (or `msi`, `docker_compose`, etc.)
