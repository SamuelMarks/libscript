...
if [ "${WORDPRESS_WEBSERVER}" = "nginx" ]; then
  ...
  export LOCATIONS=$(cat "${LOC_BLOCK_TMP}")
  ...
  if [ -d /etc/caddy/conf.d ] || [ -d /etc/caddy/caddy.d ]; then
    ...
    cat "${CADDY_BLOCK_TMP}" | priv tee -a /etc/caddy/Caddyfile >/dev/null
  fi
  ...
  if [ -d /etc/httpd/conf.d ]; then
    ...
    cat "${HTTPD_BLOCK_TMP}" | priv tee -a /etc/httpd/conf/httpd.conf >/dev/null
  fi
