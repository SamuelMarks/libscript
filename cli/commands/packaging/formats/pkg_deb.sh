  ...
  if [ $# -gt 0 ]; then
    ...
    deps_list=$(find_components | sort | awk '{printf "%s latest ", $1}')
        ...
        if [ "$pkg" = "openedx" ]; then
          ...
          printf '%s\n' "cat << 'EOF' > \"\$BUILD_DIR/etc/systemd/system/openedx-lms.service\""
          printf '%s\n' "[Unit]"
          printf '%s\n' "Description=Open edX LMS Application Server"
          printf '%s\n' "After=network.target mysql.service redis.service"
          printf '%s\n' "[Service]"
          printf '%s\n' "Type=simple"
          printf '%s\n' "ExecStart=/opt/libscript/stacks/cms/openedx/cli.sh start lms"
          printf '%s\n' "Restart=always"
          printf '%s\n' "[Install]"
          printf '%s\n' "WantedBy=multi-user.target"
          printf '%s\n' "EOF"
          printf '%s\n' "cat << 'EOF' > \"\$BUILD_DIR/etc/systemd/system/openedx-cms.service\""
          printf '%s\n' "[Unit]"
          printf '%s\n' "Description=Open edX Studio CMS Application Server"
          printf '%s\n' "After=network.target mysql.service redis.service"
          printf '%s\n' "[Service]"
          printf '%s\n' "Type=simple"
          printf '%s\n' "ExecStart=/opt/libscript/stacks/cms/openedx/cli.sh start cms"
          printf '%s\n' "Restart=always"
          printf '%s\n' "[Install]"
          printf '%s\n' "WantedBy=multi-user.target"
          printf '%s\n' "EOF"
          printf '%s\n' "cat << 'EOF' > \"\$BUILD_DIR/etc/systemd/system/openedx-workers.service\""
          printf '%s\n' "[Unit]"
          printf '%s\n' "Description=Open edX Celery Background Workers"
          printf '%s\n' "After=network.target mysql.service redis.service"
          printf '%s\n' "[Service]"
          printf '%s\n' "Type=simple"
          printf '%s\n' "ExecStart=/opt/libscript/stacks/cms/openedx/workers.sh start"
          printf '%s\n' "ExecStop=/opt/libscript/stacks/cms/openedx/workers.sh stop"
          printf '%s\n' "Restart=always"
          printf '%s\n' "[Install]"
          printf '%s\n' "WantedBy=multi-user.target"
          printf '%s\n' "EOF"
