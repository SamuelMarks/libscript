...
- [ ] **Dependency Resolution Script**
  ...
  - [ ] Add logic to install required C libraries (e.g., JSON parsers if not bundled with the
        framework).
- [x] **Framework Acquisition**
  - [x] Add a step to `git clone https://github.com/SamuelMarks/c-rest-framework` into a `vendor/`
        directory within `libscript-rest-api/`.
- [x] **Build System Configuration**
  - [x] Create the root `CMakeLists.txt` for the `libscript-rest-api` project in the new
        subdirectory.
  - [x] Adhere to best practices: place source files (`.c`, `.h`) in a `src/` subdirectory and add a
        secondary `CMakeLists.txt` there.
  - [x] Link the `c-rest-framework` and necessary system/OS-specific libraries within CMake.
  - [x] Configure the build output directory (e.g., `build/`).
- [x] **Initial Server Execution**
  - [x] Write a minimal `main.c` (in `src/`) that starts the server on a default port (e.g., 8080).
  - [x] Add a cross-platform command (like `cmake --build . --target run`) to compile and execute
