...
- `WORDPRESS_WWWROOT`: The directory to install to (default `/var/www/wordpress` or `C:\inetpub\wwwroot\wordpress`).
- `WORDPRESS_DB_NAME`, `WORDPRESS_DB_USER`, `WORDPRESS_DB_PASS`: Database credentials.
- `WORDPRESS_PHP_FPM_LISTEN`: Unix socket or host:port for PHP-FPM connection (auto-detected if unset).
...
- Uses `setup_windows.ps1` for clean `.msi` or `.exe` Windows Installer generation, leveraging `winget` and native IIS PowerShell configuration blocks (`WebAdministration`).
