#!/bin/sh
# ## Overview
# Provides a generic, cross-platform setup mechanism for the WordPress CMS stack.
# 
# ## Usage
# Execute this script to perform generic initialization steps for wordpress.
...
if [ "${WORDPRESS_WEBSERVER}" = "nginx" ]; then
  ...
  if [ -d /etc/caddy/conf.d ] || [ -d /etc/caddy/caddy.d ]; then
    ...
    grep -q "${WORDPRESS_SERVER_NAME}" /etc/caddy/Caddyfile 2>/dev/null || priv tee -a /etc/caddy/Caddyfile < "${CADDY_BLOCK_TMP}" >/dev/null
  fi
  ...
  if [ -d /etc/httpd/conf.d ]; then
    ...
    grep -q "${WORDPRESS_SERVER_NAME}" /etc/httpd/conf/httpd.conf 2>/dev/null || priv tee -a /etc/httpd/conf/httpd.conf < "${HTTPD_BLOCK_TMP}" >/dev/null
  fi
...
printf '%s\n' "WordPress setup complete on ${WORDPRESS_SERVER_NAME}"
