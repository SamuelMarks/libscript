...
- `MAGENTO_WEBSERVER`: The web server to configure (`nginx`, `caddy`, `httpd`, `iis` on Windows)
  (default: `nginx`).
- `MAGENTO_DB_DRIVER`: The database driver to use (`mariadb`, `postgres`, `sqlite`) (default:
  `mariadb`).
...
- `MAGENTO_WWWROOT`: The directory to install Magento to (default: `/var/www/magento` on Linux,
  `C:\inetpub\wwwroot\magento` on Windows).
