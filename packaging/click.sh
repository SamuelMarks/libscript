...
# Invokes click_button.ps1 via PowerShell to send click messages to controls.
#
# ## Usage
# ./packaging/click.sh [button_text]
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  powershell.exe -ExecutionPolicy Bypass -File "${WIN_PS_SCRIPT}" > "${LOG_FILE}" 2>&1
elif command -v pwsh >/dev/null 2>&1; then
  pwsh -ExecutionPolicy Bypass -File "${PS_SCRIPT}" > "${LOG_FILE}" 2>&1
else
