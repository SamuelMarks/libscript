<#
.SYNOPSIS
Windows PowerShell setup stub for cargo
...
$InstallMethod = $env:CARGO_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cargo; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cargo; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cargo; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cargo"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cargo; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cargo; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cargo; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CARGO_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CARGO_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cargo@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cargo "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cargo@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cargo") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cargo") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cargo $CompVersion to $DownloadDir\cargo..."
        $CompDownloadDir = Join-Path $DownloadDir "cargo"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CARGO_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CARGO_DOWNLOAD_URL -OutFile "$CompDownloadDir\cargo-$CompVersion.zip"
        } else {
            Write-Output "CARGO_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cargo@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cargo "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cargo@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cargo; vfox install "cargo@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cargo") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cargo $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cargo\cargo-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cargo\cargo-$CompVersion.tar.gz"
        ...
        } elseif ($env:CARGO_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cargo.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cargo" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cargo $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cargo") $CompVersion
