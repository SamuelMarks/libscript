<#
.SYNOPSIS
Windows PowerShell setup stub for rvm
...
$InstallMethod = $env:RVM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls rvm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list rvm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls rvm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "rvm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote rvm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all rvm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all rvm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:RVM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:RVM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "rvm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global rvm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "rvm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "rvm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "rvm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rvm $CompVersion to $DownloadDir\rvm..."
        $CompDownloadDir = Join-Path $DownloadDir "rvm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:RVM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:RVM_DOWNLOAD_URL -OutFile "$CompDownloadDir\rvm-$CompVersion.zip"
        } else {
            Write-Output "RVM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "rvm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install rvm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "rvm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add rvm; vfox install "rvm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "rvm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing rvm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\rvm\rvm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\rvm\rvm-$CompVersion.tar.gz"
        ...
        } elseif ($env:RVM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "rvm.zip"
            Invoke-WebRequest -Uri $env:RVM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for rvm."
        }
    } else {
        Write-Output "rvm $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_rvm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling rvm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "rvm") $CompVersion
