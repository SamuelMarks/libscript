<#
.SYNOPSIS
Windows PowerShell setup stub for pdm
...
$InstallMethod = $env:PDM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pdm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pdm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pdm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pdm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pdm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pdm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pdm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PDM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PDM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pdm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pdm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pdm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pdm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pdm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pdm $CompVersion to $DownloadDir\pdm..."
        $CompDownloadDir = Join-Path $DownloadDir "pdm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PDM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PDM_DOWNLOAD_URL -OutFile "$CompDownloadDir\pdm-$CompVersion.zip"
        } else {
            Write-Output "PDM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pdm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pdm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pdm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pdm; vfox install "pdm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pdm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pdm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pdm\pdm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pdm\pdm-$CompVersion.tar.gz"
        ...
        } elseif ($env:PDM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pdm.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pdm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pdm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pdm") $CompVersion
