<#
.SYNOPSIS
Windows PowerShell setup stub for macports
...
$CompVersion = $env:MACPORTS_VERSION
...
$InstallMethod = $env:MACPORTS_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls macports; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list macports; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls macports; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "macports"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote macports; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all macports; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all macports; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MACPORTS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MACPORTS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "macports@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global macports "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "macports@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "macports") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "macports") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading macports $CompVersion to $DownloadDir\macports..."
        $CompDownloadDir = Join-Path $DownloadDir "macports"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MACPORTS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MACPORTS_DOWNLOAD_URL -OutFile "$CompDownloadDir\macports-$CompVersion.zip"
        } else {
            Write-Output "MACPORTS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "macports@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install macports "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "macports@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add macports; vfox install "macports@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "macports") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing macports $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\macports\macports-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\macports\macports-$CompVersion.tar.gz"
        ...
        } elseif ($env:MACPORTS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "macports.zip"
            Invoke-WebRequest -Uri $env:MACPORTS_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for macports."
        }
    } else {
        Write-Output "macports $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_macports" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling macports $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "macports") $CompVersion
