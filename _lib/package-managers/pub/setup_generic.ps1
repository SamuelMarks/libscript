<#
.SYNOPSIS
Windows PowerShell setup stub for pub
...
$InstallMethod = $env:PUB_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pub; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pub; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pub; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pub"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pub; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pub; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pub; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PUB_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PUB_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pub@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pub "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pub@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pub") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pub") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pub $CompVersion to $DownloadDir\pub..."
        $CompDownloadDir = Join-Path $DownloadDir "pub"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PUB_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PUB_DOWNLOAD_URL -OutFile "$CompDownloadDir\pub-$CompVersion.zip"
        } else {
            Write-Output "PUB_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pub@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pub "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pub@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pub; vfox install "pub@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pub") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pub $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pub\pub-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pub\pub-$CompVersion.tar.gz"
        ...
        } elseif ($env:PUB_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pub.zip"
            Invoke-WebRequest -Uri $env:PUB_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for pub."
        }
    } else {
        Write-Output "pub $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pub" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pub $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pub") $CompVersion
