<#
.SYNOPSIS
Windows PowerShell setup stub for stack
...
$InstallMethod = $env:STACK_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls stack; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list stack; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls stack; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "stack"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote stack; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all stack; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all stack; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:STACK_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:STACK_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "stack@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global stack "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "stack@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "stack") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "stack") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading stack $CompVersion to $DownloadDir\stack..."
        $CompDownloadDir = Join-Path $DownloadDir "stack"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:STACK_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:STACK_DOWNLOAD_URL -OutFile "$CompDownloadDir\stack-$CompVersion.zip"
        } else {
            Write-Output "STACK_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "stack@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install stack "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "stack@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add stack; vfox install "stack@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "stack") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing stack $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\stack\stack-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\stack\stack-$CompVersion.tar.gz"
        ...
        } elseif ($env:STACK_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "stack.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_stack" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling stack $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "stack") $CompVersion
