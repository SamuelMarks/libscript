...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$STACK_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${STACK_RELEASES_URL:-}" ]; then
        curl -sSL "${STACK_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/commercialhaskell/stack" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$STACK_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "stack" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/stack/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "stack ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "stack" "default" "${EXACT_VERSION}"
      log_info "Set default stack version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env stack \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
  ...
  install)
    if [ "$STACK_INSTALL_METHOD" = "system" ]; then
      ...
      if [ -f /etc/alpine-release ] && [ "$(uname -m)" != "x86_64" ]; then
        log_info "stack binary on Alpine is only available for x86_64. Skipping."
        exit 0
      fi
