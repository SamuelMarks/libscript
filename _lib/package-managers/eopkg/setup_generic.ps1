<#
.SYNOPSIS
Windows PowerShell setup stub for eopkg
...
$InstallMethod = $env:EOPKG_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls eopkg; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list eopkg; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls eopkg; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "eopkg"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote eopkg; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all eopkg; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all eopkg; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:EOPKG_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:EOPKG_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "eopkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global eopkg "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "eopkg@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "eopkg") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "eopkg") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading eopkg $CompVersion to $DownloadDir\eopkg..."
        $CompDownloadDir = Join-Path $DownloadDir "eopkg"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:EOPKG_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:EOPKG_DOWNLOAD_URL -OutFile "$CompDownloadDir\eopkg-$CompVersion.zip"
        } else {
            Write-Output "EOPKG_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "eopkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install eopkg "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "eopkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add eopkg; vfox install "eopkg@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "eopkg") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing eopkg $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\eopkg\eopkg-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\eopkg\eopkg-$CompVersion.tar.gz"
        ...
        } elseif ($env:EOPKG_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "eopkg.zip"
            Invoke-WebRequest -Uri $env:EOPKG_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for eopkg."
        }
    } else {
        Write-Output "eopkg $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_eopkg" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling eopkg $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "eopkg") $CompVersion
