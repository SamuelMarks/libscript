<#
.SYNOPSIS
Windows PowerShell setup stub for winget
...
$CompVersion = $env:WINGET_VERSION
...
$InstallMethod = $env:WINGET_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls winget; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list winget; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls winget; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "winget"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote winget; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all winget; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all winget; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:WINGET_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:WINGET_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "winget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global winget "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "winget@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "winget") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "winget") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading winget $CompVersion to $DownloadDir\winget..."
        $CompDownloadDir = Join-Path $DownloadDir "winget"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:WINGET_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:WINGET_DOWNLOAD_URL -OutFile "$CompDownloadDir\winget-$CompVersion.zip"
        } else {
            Write-Output "WINGET_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "winget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install winget "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "winget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add winget; vfox install "winget@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "winget") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing winget $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\winget\winget-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\winget\winget-$CompVersion.tar.gz"
        ...
        } elseif ($env:WINGET_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "winget.zip"
            Invoke-WebRequest -Uri $env:WINGET_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for winget."
            exit 1
        }
    } else {
        Write-Output "winget $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_winget" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling winget $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "winget") $CompVersion
