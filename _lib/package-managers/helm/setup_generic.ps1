<#
.SYNOPSIS
Windows PowerShell setup stub for helm
...
$InstallMethod = $env:HELM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls helm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list helm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls helm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "helm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote helm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all helm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all helm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:HELM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:HELM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "helm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global helm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "helm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "helm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "helm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading helm $CompVersion to $DownloadDir\helm..."
        $CompDownloadDir = Join-Path $DownloadDir "helm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:HELM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:HELM_DOWNLOAD_URL -OutFile "$CompDownloadDir\helm-$CompVersion.zip"
        } else {
            Write-Output "HELM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "helm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install helm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "helm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add helm; vfox install "helm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "helm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing helm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\helm\helm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\helm\helm-$CompVersion.tar.gz"
        ...
        } elseif ($env:HELM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "helm.zip"
            Invoke-WebRequest -Uri $env:HELM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for helm."
        }
    } else {
        Write-Output "helm $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_helm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling helm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "helm") $CompVersion
