<#
.SYNOPSIS
Orchestrates the setup and installation process for the component 'vfox' stack.

.DESCRIPTION
Execute this script to install and configure vfox on the local system.
#>
...
if (-not (Get-Command vfox -ErrorAction SilentlyContinue)) {
    ...
    winget install -e --id version-fox.vfox --accept-package-agreements --accept-source-agreements
} else {
    Write-Host "vfox is already installed."
}
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading vfox to $DownloadDir\vfox..."
    }
    exit 0
}
