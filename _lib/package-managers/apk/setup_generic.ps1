<#
.SYNOPSIS
Windows PowerShell setup stub for apk
...
$InstallMethod = $env:APK_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls apk; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list apk; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls apk; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "apk"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote apk; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all apk; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all apk; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:APK_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:APK_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "apk@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global apk "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "apk@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "apk") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "apk") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading apk $CompVersion to $DownloadDir\apk..."
        $CompDownloadDir = Join-Path $DownloadDir "apk"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:APK_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:APK_DOWNLOAD_URL -OutFile "$CompDownloadDir\apk-$CompVersion.zip"
        } else {
            Write-Output "APK_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "apk@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install apk "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "apk@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add apk; vfox install "apk@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "apk") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing apk $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\apk\apk-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\apk\apk-$CompVersion.tar.gz"
        ...
        } elseif ($env:APK_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "apk.zip"
            Invoke-WebRequest -Uri $env:APK_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for apk."
        }
    } else {
        Write-Output "apk $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_apk" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling apk $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "apk") $CompVersion
