<#
.SYNOPSIS
Windows PowerShell setup stub for dnf
...
$InstallMethod = $env:DNF_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls dnf; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list dnf; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls dnf; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "dnf"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote dnf; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all dnf; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all dnf; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:DNF_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:DNF_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "dnf@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global dnf "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "dnf@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "dnf") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "dnf") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading dnf $CompVersion to $DownloadDir\dnf..."
        $CompDownloadDir = Join-Path $DownloadDir "dnf"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:DNF_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:DNF_DOWNLOAD_URL -OutFile "$CompDownloadDir\dnf-$CompVersion.zip"
        } else {
            Write-Output "DNF_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "dnf@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install dnf "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "dnf@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add dnf; vfox install "dnf@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "dnf") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing dnf $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\dnf\dnf-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\dnf\dnf-$CompVersion.tar.gz"
        ...
        } elseif ($env:DNF_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "dnf.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_dnf" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling dnf $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "dnf") $CompVersion
