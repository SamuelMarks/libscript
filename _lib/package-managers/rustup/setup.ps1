...
<#
.SYNOPSIS
Orchestrates the setup and installation process for the component 'rustup' stack.

.DESCRIPTION
Execute this script to install and configure rustup on the local system.
#>
...
if (-Not (Get-Command rustup -ErrorAction SilentlyContinue)) {
    ...
    if (Test-Path -Path (Join-Path $cargoBin "rustup.exe")) {
        ...
        $rustSetup = [System.IO.Path]::Combine($scriptDir, "..", "..", "_toolchain", "rust", "setup.ps1")
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rustup to $DownloadDir\rustup..."
    }
    exit 0
}
