  ...
  ls-remote)
    if [ "$RUSTUP_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${RUSTUP_RELEASES_URL:-}" ]; then
        curl -sSL "${RUSTUP_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/rustup" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
    fi
    fi
    exit 0
    ;;
  ...
  install|*)
    if [ "$RUSTUP_INSTALL_METHOD" = "system" ]; then
      ...
      if [ ! -d "${TARGET_DIR}" ]; then
        ...
        if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/rustup/"*"${VERSION}"* >/dev/null 2>&1; then
          log_info "Extracting from cache..."
          cache_file=$(find "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/rustup/" -maxdepth 1 -type f -name "*${VERSION}*" 2>/dev/null | head -n 1 || true)
          if [ -n "$cache_file" ]; then
            if case "$cache_file" in *.tar.gz|*.tgz) true;; *) false;; esac; then
              ...
              cp "$cache_file" "${TARGET_DIR}/bin/rustup" || true
              chmod +x "${TARGET_DIR}/bin/rustup" || true
            fi
          fi
        else
          if [ -n "${RUSTUP_DOWNLOAD_URL:-}" ]; then
            TEMP_FILE=$(mktemp)
            libscript_download "${RUSTUP_DOWNLOAD_URL:-}" "${TEMP_FILE}"
            if case "${RUSTUP_DOWNLOAD_URL:-}" in *.tar.gz|*.tgz) true;; *) false;; esac; then
              tar -xzf "${TEMP_FILE}" -C "${TARGET_DIR}" --strip-components=1 || true
            elif case "${RUSTUP_DOWNLOAD_URL:-}" in *.zip) true;; *) false;; esac; then
              unzip -q "${TEMP_FILE}" -d "${TARGET_DIR}" || true
            else
              cp "${TEMP_FILE}" "${TARGET_DIR}/bin/rustup" || true
              chmod +x "${TARGET_DIR}/bin/rustup" || true
            fi
            rm -f "${TEMP_FILE}"
          else
            log_warn "No download URL provided for rustup ${VERSION}."
          fi
        fi
      else
