<#
.SYNOPSIS
Windows PowerShell setup stub for rustup
...
$CompVersion = $env:RUSTUP_VERSION
...
$InstallMethod = $env:RUSTUP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls rustup; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list rustup; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls rustup; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "rustup"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote rustup; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all rustup; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all rustup; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:RUSTUP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:RUSTUP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "rustup@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global rustup "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "rustup@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "rustup") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "rustup") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rustup $CompVersion to $DownloadDir\rustup..."
        $CompDownloadDir = Join-Path $DownloadDir "rustup"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:RUSTUP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:RUSTUP_DOWNLOAD_URL -OutFile "$CompDownloadDir\rustup-$CompVersion.zip"
        } else {
            Write-Output "RUSTUP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "rustup@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install rustup "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "rustup@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add rustup; vfox install "rustup@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "rustup") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing rustup $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\rustup\rustup-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\rustup\rustup-$CompVersion.tar.gz"
        ...
        } elseif ($env:RUSTUP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "rustup.zip"
            Invoke-WebRequest -Uri $env:RUSTUP_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for rustup."
        }
    } else {
        Write-Output "rustup $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_rustup" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling rustup $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "rustup") $CompVersion
