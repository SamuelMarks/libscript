...
Provides bootstrap definitions and wrappers for installing the Google Cloud SDK (`gcloud`).
...
| `GOOGLE_CLOUD_SDK_INSTALL_METHOD` | How to install GOOGLE_CLOUD_SDK. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager, 'mise', 'asdf', 'pkgx', or 'vfox' defers to third-party tools. | `libscript_native` |  |
| `GOOGLE_CLOUD_SDK_VERSION` | Version of Google Cloud SDK to install | `latest` |  |
<!-- END_VARS -->
...
`libscript` manages `google-cloud-sdk` versions natively by default
(`GOOGLE_CLOUD_SDK_INSTALL_METHOD=libscript_native`), ensuring isolated installations without
polluting global system paths. You can override this to use `system`, `mise`, `asdf`, `pkgx`, or
`vfox` if preferred.

Libscript manages google-cloud-sdk versions natively by installing them into isolated directories
under `LIBSCRIPT_HOME/google-cloud-sdk/<version>`.
