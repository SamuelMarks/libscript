<#
.SYNOPSIS
Windows PowerShell setup stub for google-cloud-sdk
...
$CompVersion = $env:GOOGLE_CLOUD_SDK_VERSION
...
$InstallMethod = $env:GOOGLE_CLOUD_SDK_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls google-cloud-sdk; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list google-cloud-sdk; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls google-cloud-sdk; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "google-cloud-sdk"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote google-cloud-sdk; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all google-cloud-sdk; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all google-cloud-sdk; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GOOGLE_CLOUD_SDK_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GOOGLE_CLOUD_SDK_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "google-cloud-sdk@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global google-cloud-sdk "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "google-cloud-sdk@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "google-cloud-sdk") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "google-cloud-sdk") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading google-cloud-sdk $CompVersion to $DownloadDir\google-cloud-sdk..."
        $CompDownloadDir = Join-Path $DownloadDir "google-cloud-sdk"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GOOGLE_CLOUD_SDK_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GOOGLE_CLOUD_SDK_DOWNLOAD_URL -OutFile "$CompDownloadDir\google-cloud-sdk-$CompVersion.zip"
        } else {
            Write-Output "GOOGLE_CLOUD_SDK_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "google-cloud-sdk@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install google-cloud-sdk "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "google-cloud-sdk@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add google-cloud-sdk; vfox install "google-cloud-sdk@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "google-cloud-sdk") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing google-cloud-sdk $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\google-cloud-sdk\google-cloud-sdk-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\google-cloud-sdk\google-cloud-sdk-$CompVersion.tar.gz"
        ...
        } elseif ($env:GOOGLE_CLOUD_SDK_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "google-cloud-sdk.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_google-cloud-sdk" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling google-cloud-sdk $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "google-cloud-sdk") $CompVersion
