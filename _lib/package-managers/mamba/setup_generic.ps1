<#
.SYNOPSIS
Windows PowerShell setup stub for mamba
...
$InstallMethod = $env:MAMBA_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls mamba; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list mamba; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls mamba; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "mamba"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote mamba; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all mamba; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all mamba; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MAMBA_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MAMBA_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "mamba@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global mamba "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "mamba@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "mamba") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "mamba") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading mamba $CompVersion to $DownloadDir\mamba..."
        $CompDownloadDir = Join-Path $DownloadDir "mamba"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MAMBA_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MAMBA_DOWNLOAD_URL -OutFile "$CompDownloadDir\mamba-$CompVersion.zip"
        } else {
            Write-Output "MAMBA_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "mamba@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install mamba "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "mamba@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add mamba; vfox install "mamba@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "mamba") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing mamba $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\mamba\mamba-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\mamba\mamba-$CompVersion.tar.gz"
        ...
        } elseif ($env:MAMBA_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "mamba.zip"
            Invoke-WebRequest -Uri $env:MAMBA_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for mamba."
        }
    } else {
        Write-Output "mamba $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_mamba" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling mamba $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "mamba") $CompVersion
