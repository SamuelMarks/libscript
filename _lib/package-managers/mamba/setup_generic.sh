#!/bin/sh
# ## Overview
# Generic setup script for the mamba component.
...
if ! command -v micromamba >/dev/null 2>&1 && ! command -v mamba >/dev/null 2>&1; then
  ...
  if [ "$OS" = "Linux" ]; then
    if [ "$ARCH" = "x86_64" ]; then
      ...
      printf '%s\n' "Error: Unsupported architecture $ARCH for micromamba." >&2
      exit 1
    fi
  elif [ "$OS" = "Darwin" ]; then
    if [ "$ARCH" = "x86_64" ]; then
    ...
    printf '%s\n' "Error: Unsupported OS $OS for micromamba automated install." >&2
    exit 1
  fi
