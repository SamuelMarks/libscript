<#
.SYNOPSIS
Windows PowerShell setup stub for rye
...
$InstallMethod = $env:RYE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls rye; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list rye; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls rye; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "rye"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote rye; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all rye; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all rye; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:RYE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:RYE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "rye@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global rye "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "rye@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "rye") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "rye") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rye $CompVersion to $DownloadDir\rye..."
        $CompDownloadDir = Join-Path $DownloadDir "rye"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:RYE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:RYE_DOWNLOAD_URL -OutFile "$CompDownloadDir\rye-$CompVersion.zip"
        } else {
            Write-Output "RYE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "rye@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install rye "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "rye@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add rye; vfox install "rye@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "rye") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing rye $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\rye\rye-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\rye\rye-$CompVersion.tar.gz"
        ...
        } elseif ($env:RYE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "rye.zip"
            Invoke-WebRequest -Uri $env:RYE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for rye."
        }
    } else {
        Write-Output "rye $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_rye" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling rye $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "rye") $CompVersion
