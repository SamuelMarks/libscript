<#
.SYNOPSIS
Windows PowerShell setup stub for mix
...
$InstallMethod = $env:MIX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls mix; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list mix; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls mix; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "mix"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote mix; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all mix; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all mix; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MIX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MIX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "mix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global mix "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "mix@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "mix") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "mix") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading mix $CompVersion to $DownloadDir\mix..."
        $CompDownloadDir = Join-Path $DownloadDir "mix"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MIX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MIX_DOWNLOAD_URL -OutFile "$CompDownloadDir\mix-$CompVersion.zip"
        } else {
            Write-Output "MIX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "mix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install mix "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "mix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add mix; vfox install "mix@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "mix") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing mix $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\mix\mix-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\mix\mix-$CompVersion.tar.gz"
        ...
        } elseif ($env:MIX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "mix.zip"
            Invoke-WebRequest -Uri $env:MIX_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for mix."
        }
    } else {
        Write-Output "mix $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_mix" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling mix $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "mix") $CompVersion
