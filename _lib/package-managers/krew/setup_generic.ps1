<#
.SYNOPSIS
Windows PowerShell setup stub for krew
...
$InstallMethod = $env:KREW_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list krew; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all krew; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:KREW_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:KREW_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global krew "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "krew") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "krew") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading krew $CompVersion to $DownloadDir\krew..."
        $CompDownloadDir = Join-Path $DownloadDir "krew"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:KREW_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:KREW_DOWNLOAD_URL -OutFile "$CompDownloadDir\krew-$CompVersion.zip"
        } else {
            Write-Output "KREW_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "krew@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install krew "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "krew@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add krew; vfox install "krew@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "krew") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing krew $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\krew\krew-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\krew\krew-$CompVersion.tar.gz"
        ...
        } elseif ($env:KREW_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "krew.zip"
