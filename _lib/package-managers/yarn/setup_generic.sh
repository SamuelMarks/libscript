...
YARN_INSTALL_METHOD="$(libscript_resolve_install_method "YARN")"
  ...
  ls-remote)
    if [ "$YARN_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${YARN_RELEASES_URL:-}" ]; then
        curl -sSL "${YARN_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/yarn" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
    fi
    fi
    exit 0
    ;;
    ...
    if [ "$YARN_INSTALL_METHOD" = "system" ]; then
      ...
      if [ ! -d "${TARGET_DIR}" ]; then
        ...
        if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/yarn/"*"${VERSION}"* >/dev/null 2>&1; then
          ...
          if [ -n "${YARN_DOWNLOAD_URL:-}" ]; then
            TEMP_FILE=$(mktemp)
            libscript_download "${YARN_DOWNLOAD_URL:-}" "${TEMP_FILE}"
            if case "${YARN_DOWNLOAD_URL:-}" in *.tar.gz|*.tgz) true;; *) false;; esac; then
              tar -xzf "${TEMP_FILE}" -C "${TARGET_DIR}" --strip-components=1 || true
            elif case "${YARN_DOWNLOAD_URL:-}" in *.zip) true;; *) false;; esac; then
              unzip -q "${TEMP_FILE}" -d "${TARGET_DIR}" || true
            else
              cp "${TEMP_FILE}" "${TARGET_DIR}/bin/yarn" || true
              chmod +x "${TARGET_DIR}/bin/yarn" || true
            fi
            rm -f "${TEMP_FILE}"
          else
            log_warn "No download URL provided for yarn ${VERSION}."
          fi
        fi
      else
