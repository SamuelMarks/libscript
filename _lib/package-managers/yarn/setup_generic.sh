#!/bin/sh
# ## Overview
# Generic setup script for the yarn component.
...
if ! command -v yarn >/dev/null 2>&1; then
  if [ -f "${LIBSCRIPT_ROOT_DIR}/_lib/package-managers/npm/setup.sh" ]; then
    "${LIBSCRIPT_ROOT_DIR}/_lib/package-managers/npm/setup.sh"
  fi
  npm install -g yarn
fi
