<#
.SYNOPSIS
Windows PowerShell setup stub for yarn
...
$InstallMethod = $env:YARN_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls yarn; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list yarn; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls yarn; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "yarn"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote yarn; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all yarn; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all yarn; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:YARN_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:YARN_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "yarn@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global yarn "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "yarn@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "yarn") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "yarn") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading yarn $CompVersion to $DownloadDir\yarn..."
        $CompDownloadDir = Join-Path $DownloadDir "yarn"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:YARN_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:YARN_DOWNLOAD_URL -OutFile "$CompDownloadDir\yarn-$CompVersion.zip"
        } else {
            Write-Output "YARN_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "yarn@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install yarn "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "yarn@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add yarn; vfox install "yarn@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "yarn") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing yarn $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\yarn\yarn-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\yarn\yarn-$CompVersion.tar.gz"
        ...
        } elseif ($env:YARN_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "yarn.zip"
            Invoke-WebRequest -Uri $env:YARN_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for yarn."
        }
    } else {
        Write-Output "yarn $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_yarn" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling yarn $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "yarn") $CompVersion
