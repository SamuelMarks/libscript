<#
.SYNOPSIS
Windows PowerShell setup stub for pyenv
...
$InstallMethod = $env:PYENV_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pyenv; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pyenv; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pyenv; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pyenv"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pyenv; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pyenv; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pyenv; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PYENV_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PYENV_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pyenv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pyenv "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pyenv@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pyenv") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pyenv") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pyenv $CompVersion to $DownloadDir\pyenv..."
        $CompDownloadDir = Join-Path $DownloadDir "pyenv"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PYENV_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PYENV_DOWNLOAD_URL -OutFile "$CompDownloadDir\pyenv-$CompVersion.zip"
        } else {
            Write-Output "PYENV_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pyenv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pyenv "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pyenv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pyenv; vfox install "pyenv@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pyenv") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pyenv $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pyenv\pyenv-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pyenv\pyenv-$CompVersion.tar.gz"
        ...
        } elseif ($env:PYENV_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pyenv.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pyenv" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pyenv $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pyenv") $CompVersion
