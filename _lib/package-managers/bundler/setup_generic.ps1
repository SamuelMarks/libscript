<#
.SYNOPSIS
Windows PowerShell setup stub for bundler
...
$InstallMethod = $env:BUNDLER_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls bundler; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list bundler; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls bundler; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "bundler"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote bundler; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all bundler; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all bundler; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:BUNDLER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:BUNDLER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "bundler@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global bundler "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "bundler@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "bundler") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "bundler") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading bundler $CompVersion to $DownloadDir\bundler..."
        $CompDownloadDir = Join-Path $DownloadDir "bundler"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:BUNDLER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:BUNDLER_DOWNLOAD_URL -OutFile "$CompDownloadDir\bundler-$CompVersion.zip"
        } else {
            Write-Output "BUNDLER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "bundler@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install bundler "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "bundler@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add bundler; vfox install "bundler@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "bundler") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing bundler $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\bundler\bundler-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\bundler\bundler-$CompVersion.tar.gz"
        ...
        } elseif ($env:BUNDLER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "bundler.zip"
            Invoke-WebRequest -Uri $env:BUNDLER_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for bundler."
        }
    } else {
        Write-Output "bundler $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_bundler" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling bundler $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "bundler") $CompVersion
