<#
.SYNOPSIS
Windows PowerShell setup stub for poetry
...
$CompVersion = $env:POETRY_VERSION
...
$InstallMethod = $env:POETRY_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls poetry; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list poetry; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls poetry; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "poetry"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote poetry; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all poetry; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all poetry; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:POETRY_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:POETRY_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "poetry@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global poetry "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "poetry@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "poetry") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "poetry") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading poetry $CompVersion to $DownloadDir\poetry..."
        $CompDownloadDir = Join-Path $DownloadDir "poetry"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:POETRY_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:POETRY_DOWNLOAD_URL -OutFile "$CompDownloadDir\poetry-$CompVersion.zip"
        } else {
            Write-Output "POETRY_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "poetry@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install poetry "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "poetry@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add poetry; vfox install "poetry@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "poetry") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing poetry $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\poetry\poetry-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\poetry\poetry-$CompVersion.tar.gz"
        ...
        } elseif ($env:POETRY_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "poetry.zip"
            Invoke-WebRequest -Uri $env:POETRY_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for poetry."
        }
    } else {
        Write-Output "poetry $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_poetry" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling poetry $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "poetry") $CompVersion
