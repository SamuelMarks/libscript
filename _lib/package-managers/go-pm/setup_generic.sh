#!/bin/sh
# ## Overview
# Generic setup module for go-pm.
# 
# ## Usage
# Execute this script to perform generic initialization steps for go-pm.
...
resolve_exact_version() {
  if [ "${VERSION:-}" = "latest" ] || [ "${VERSION:-}" = "lts" ] || [ "${VERSION:-}" = "stable" ]; then
    _latest=$("${LIBSCRIPT_ROOT_DIR}/libscript.sh" ls-remote go-pm 2>/dev/null | tail -n 1)
...
case "$ACTION" in
  ls)
    if [ "$GO_PM_INSTALL_METHOD" = "mise" ]; then
      mise ls go-pm || true
  ...
  use)
    if [ "$GO_PM_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "go-pm" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/go-pm/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "go-pm ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "go-pm" "default" "${EXACT_VERSION}"
      log_info "Set default go-pm version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env go-pm \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
    if [ "$GO_PM_INSTALL_METHOD" = "libscript_native" ]; then
      log_info "Downloading go-pm ${VERSION} to ${DOWNLOAD_DIR:-/tmp/libscript_downloads}/go-pm..."
      mkdir -p "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/go-pm"
      if [ -n "${GO_PM_DOWNLOAD_URL:-}" ]; then
        libscript_download "${GO_PM_DOWNLOAD_URL:-}" "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/go-pm/go-pm-${VERSION}.tar.gz"
      else
        log_warn "GO_PM_DOWNLOAD_URL is not defined for go-pm ${VERSION}."
      fi
    fi
    exit 0
    ;;
  install)
    if [ "$GO_PM_INSTALL_METHOD" = "system" ]; then
      ...
      asdf install go-pm "${VERSION}"
    elif [ "$GO_PM_INSTALL_METHOD" = "pkgx" ]; then
      pkgx install "go-pm@${VERSION}"
    elif [ "$GO_PM_INSTALL_METHOD" = "vfox" ]; then
      vfox add go-pm || true
      vfox install "go-pm@${VERSION}"
    else
      ...
      if [ ! -d "${TARGET_DIR}" ]; then
        ...
        log_info "go-pm ${VERSION} is already installed."
      fi
      libscript_symlink_alias "go-pm" "$VERSION" "${EXACT_VERSION}"
    fi
    ;;
  start|stop|restart|status|health|logs|up|down)
    if [ "$GO_PM_INSTALL_METHOD" = "libscript_native" ] || [ "$GO_PM_INSTALL_METHOD" = "system" ]; then
      SCRIPT_NAME="${LIBSCRIPT_ROOT_DIR}/_lib/_common/service.sh"
      export SCRIPT_NAME
      . "${SCRIPT_NAME}"
      service_name="${LIBSCRIPT_SERVICE_NAME:-libscript_${PACKAGE_NAME:-go-pm}}"
      libscript_service "$ACTION" "$service_name" "$@"
    else
      log_info "$ACTION not natively implemented for $GO_PM_INSTALL_METHOD."
    fi
    exit 0
    ;;
  install-service)
    if [ "$GO_PM_INSTALL_METHOD" = "libscript_native" ] || [ "$GO_PM_INSTALL_METHOD" = "system" ]; then
      ...
      log_info "install-service not implemented for $GO_PM_INSTALL_METHOD."
    fi
    exit 0
    ;;
  uninstall-service)
    if [ "$GO_PM_INSTALL_METHOD" = "libscript_native" ] || [ "$GO_PM_INSTALL_METHOD" = "system" ]; then
      ...
      log_info "uninstall-service not implemented for $GO_PM_INSTALL_METHOD."
    fi
    exit 0
    ;;
  uninstall)
    if [ "$GO_PM_INSTALL_METHOD" = "libscript_native" ]; then
      ...
      log_info "Uninstalling go-pm $VERSION..."
      rm -rf "${LIBSCRIPT_HOME:-$HOME/.libscript}/go-pm/${EXACT_VERSION}"
      rm -f "${LIBSCRIPT_HOME:-$HOME/.libscript}/go-pm/$VERSION"
    else
      log_info "Uninstall not implemented or supported for $GO_PM_INSTALL_METHOD."
    fi
    exit 0
    ;;
