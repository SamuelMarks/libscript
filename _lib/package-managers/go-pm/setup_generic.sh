#!/bin/sh
# ## Overview
# Generic setup script for the go-pm component.
...
if ! command -v go >/dev/null 2>&1; then
  if [ -f "${LIBSCRIPT_ROOT_DIR}/_lib/languages/go/setup.sh" ]; then
    "${LIBSCRIPT_ROOT_DIR}/_lib/languages/go/setup.sh"
  else
    if ! libscript_depends golang || libscript_depends go ; then
      true
    fi
  fi
fi
