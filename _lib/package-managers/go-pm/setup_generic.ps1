<#
.SYNOPSIS
Windows PowerShell setup stub for go-pm
...
$InstallMethod = $env:GO_PM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls go-pm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list go-pm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls go-pm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "go-pm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote go-pm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all go-pm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all go-pm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GO_PM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GO_PM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "go-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global go-pm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "go-pm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "go-pm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "go-pm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading go-pm $CompVersion to $DownloadDir\go-pm..."
        $CompDownloadDir = Join-Path $DownloadDir "go-pm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GO_PM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GO_PM_DOWNLOAD_URL -OutFile "$CompDownloadDir\go-pm-$CompVersion.zip"
        } else {
            Write-Output "GO_PM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "go-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install go-pm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "go-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add go-pm; vfox install "go-pm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "go-pm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing go-pm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\go-pm\go-pm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\go-pm\go-pm-$CompVersion.tar.gz"
        ...
        } elseif ($env:GO_PM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "go-pm.zip"
            Invoke-WebRequest -Uri $env:GO_PM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for go-pm."
        }
    } else {
        Write-Output "go-pm $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_go-pm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling go-pm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "go-pm") $CompVersion
