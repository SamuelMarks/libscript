<#
.SYNOPSIS
Windows PowerShell setup stub for sbt
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls sbt; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list sbt; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls sbt; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "sbt"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote sbt; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all sbt; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all sbt; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SBT_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SBT_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "sbt@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global sbt "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "sbt@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "sbt") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "sbt") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading sbt $CompVersion to $DownloadDir\sbt..."
        $CompDownloadDir = Join-Path $DownloadDir "sbt"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SBT_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SBT_DOWNLOAD_URL -OutFile "$CompDownloadDir\sbt-$CompVersion.zip"
        } else {
            Write-Output "SBT_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "sbt@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install sbt "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "sbt@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add sbt; vfox install "sbt@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "sbt") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing sbt $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\sbt\sbt-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\sbt\sbt-$CompVersion.tar.gz"
        ...
        } elseif ($env:SBT_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "sbt.zip"
            Invoke-WebRequest -Uri $env:SBT_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for sbt."
        }
    } else {
        Write-Output "sbt $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_sbt" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling sbt $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "sbt") $CompVersion
