<#
.SYNOPSIS
Windows PowerShell setup stub for bun-pm
...
$InstallMethod = $env:BUN_PM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls bun-pm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list bun-pm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls bun-pm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "bun-pm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote bun-pm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all bun-pm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all bun-pm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:BUN_PM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:BUN_PM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "bun-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global bun-pm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "bun-pm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "bun-pm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "bun-pm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading bun-pm $CompVersion to $DownloadDir\bun-pm..."
        $CompDownloadDir = Join-Path $DownloadDir "bun-pm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:BUN_PM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:BUN_PM_DOWNLOAD_URL -OutFile "$CompDownloadDir\bun-pm-$CompVersion.zip"
        } else {
            Write-Output "BUN_PM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "bun-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install bun-pm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "bun-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add bun-pm; vfox install "bun-pm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "bun-pm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing bun-pm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\bun-pm\bun-pm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\bun-pm\bun-pm-$CompVersion.tar.gz"
        ...
        } elseif ($env:BUN_PM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "bun-pm.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_bun-pm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling bun-pm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "bun-pm") $CompVersion
