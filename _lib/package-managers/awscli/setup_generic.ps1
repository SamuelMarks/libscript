...
<#
.SYNOPSIS
Windows PowerShell setup stub for awscli
...
$InstallMethod = $env:AWSCLI_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls awscli; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list awscli; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls awscli; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "awscli"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote awscli; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all awscli; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all awscli; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:AWSCLI_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:AWSCLI_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "awscli@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global awscli "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "awscli@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "awscli") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "awscli") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading awscli $CompVersion to $DownloadDir\awscli..."
        $CompDownloadDir = Join-Path $DownloadDir "awscli"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:AWSCLI_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:AWSCLI_DOWNLOAD_URL -OutFile "$CompDownloadDir\awscli-$CompVersion.zip"
        } else {
            Write-Output "AWSCLI_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "awscli@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install awscli "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "awscli@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add awscli; vfox install "awscli@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "awscli") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing awscli $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\awscli\awscli-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\awscli\awscli-$CompVersion.tar.gz"
        ...
        } elseif ($env:AWSCLI_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "awscli.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_awscli" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling awscli $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "awscli") $CompVersion
