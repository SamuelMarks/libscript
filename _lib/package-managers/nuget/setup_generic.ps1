<#
.SYNOPSIS
Windows PowerShell setup stub for nuget
...
$InstallMethod = $env:NUGET_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls nuget; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list nuget; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls nuget; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "nuget"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote nuget; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all nuget; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all nuget; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:NUGET_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NUGET_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "nuget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global nuget "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "nuget@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "nuget") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "nuget") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading nuget $CompVersion to $DownloadDir\nuget..."
        $CompDownloadDir = Join-Path $DownloadDir "nuget"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NUGET_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NUGET_DOWNLOAD_URL -OutFile "$CompDownloadDir\nuget-$CompVersion.zip"
        } else {
            Write-Output "NUGET_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "nuget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install nuget "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "nuget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add nuget; vfox install "nuget@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "nuget") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing nuget $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\nuget\nuget-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\nuget\nuget-$CompVersion.tar.gz"
        ...
        } elseif ($env:NUGET_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "nuget.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_nuget" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling nuget $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "nuget") $CompVersion
