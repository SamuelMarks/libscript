<#
.SYNOPSIS
Windows PowerShell setup stub for zypper
...
$CompVersion = $env:ZYPPER_VERSION
...
$InstallMethod = $env:ZYPPER_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls zypper; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list zypper; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls zypper; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "zypper"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote zypper; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all zypper; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all zypper; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:ZYPPER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:ZYPPER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "zypper@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global zypper "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "zypper@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "zypper") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "zypper") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading zypper $CompVersion to $DownloadDir\zypper..."
        $CompDownloadDir = Join-Path $DownloadDir "zypper"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:ZYPPER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:ZYPPER_DOWNLOAD_URL -OutFile "$CompDownloadDir\zypper-$CompVersion.zip"
        } else {
            Write-Output "ZYPPER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "zypper@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install zypper "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "zypper@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add zypper; vfox install "zypper@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "zypper") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing zypper $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\zypper\zypper-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\zypper\zypper-$CompVersion.tar.gz"
        ...
        } elseif ($env:ZYPPER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "zypper.zip"
            Invoke-WebRequest -Uri $env:ZYPPER_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for zypper."
        }
    } else {
        Write-Output "zypper $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_zypper" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling zypper $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "zypper") $CompVersion
