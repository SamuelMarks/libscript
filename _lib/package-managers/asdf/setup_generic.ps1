<#
.SYNOPSIS
Windows PowerShell setup stub for asdf
...
$InstallMethod = $env:ASDF_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls asdf; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list asdf; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls asdf; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "asdf"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote asdf; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all asdf; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all asdf; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:ASDF_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:ASDF_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "asdf@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global asdf "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "asdf@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "asdf") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "asdf") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading asdf $CompVersion to $DownloadDir\asdf..."
        $CompDownloadDir = Join-Path $DownloadDir "asdf"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:ASDF_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:ASDF_DOWNLOAD_URL -OutFile "$CompDownloadDir\asdf-$CompVersion.zip"
        } else {
            Write-Output "ASDF_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "asdf@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install asdf "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "asdf@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add asdf; vfox install "asdf@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "asdf") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing asdf $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\asdf\asdf-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\asdf\asdf-$CompVersion.tar.gz"
        ...
        } elseif ($env:ASDF_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "asdf.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_asdf" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling asdf $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "asdf") $CompVersion
