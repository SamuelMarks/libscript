<#
.SYNOPSIS
Windows PowerShell setup stub for emerge
...
$CompVersion = $env:EMERGE_VERSION
...
$InstallMethod = $env:EMERGE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls emerge; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list emerge; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls emerge; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "emerge"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote emerge; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all emerge; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all emerge; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:EMERGE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:EMERGE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "emerge@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global emerge "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "emerge@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "emerge") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "emerge") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading emerge $CompVersion to $DownloadDir\emerge..."
        $CompDownloadDir = Join-Path $DownloadDir "emerge"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:EMERGE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:EMERGE_DOWNLOAD_URL -OutFile "$CompDownloadDir\emerge-$CompVersion.zip"
        } else {
            Write-Output "EMERGE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "emerge@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install emerge "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "emerge@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add emerge; vfox install "emerge@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "emerge") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing emerge $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\emerge\emerge-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\emerge\emerge-$CompVersion.tar.gz"
        ...
        } elseif ($env:EMERGE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "emerge.zip"
            Invoke-WebRequest -Uri $env:EMERGE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for emerge."
        }
    } else {
        Write-Output "emerge $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_emerge" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling emerge $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "emerge") $CompVersion
