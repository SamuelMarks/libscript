...
EMERGE_INSTALL_METHOD="${EMERGE_INSTALL_METHOD:-system}"
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$EMERGE_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${EMERGE_RELEASES_URL:-}" ]; then
        curl -sSL "${EMERGE_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/glato/emerge" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$EMERGE_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "emerge" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/emerge/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "emerge ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "emerge" "default" "${EXACT_VERSION}"
      log_info "Set default emerge version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env emerge \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
  ...
  install)
    if [ "$EMERGE_INSTALL_METHOD" = "system" ]; then
      libscript_depends "emerge" || log_info "emerge is not supported or failed to install natively on this OS."
