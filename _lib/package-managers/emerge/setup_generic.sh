#!/bin/sh
# ## Overview
# Generic setup script for the emerge component.
...
if ! command -v emerge >/dev/null 2>&1; then
  if [ -f /etc/os-release ] && grep -qi "gentoo" /etc/os-release; then
    printf '%s\n' "Warning: emerge not found on a Gentoo Linux system. This is highly unusual." >&2
  else
    printf '%s\n' "Warning: The 'emerge' package manager is only applicable on Gentoo Linux. Skipping." >&2
  fi
fi
