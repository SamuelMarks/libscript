<#
.SYNOPSIS
Windows PowerShell setup stub for opam
...
$InstallMethod = $env:OPAM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls opam; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list opam; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls opam; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "opam"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote opam; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all opam; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all opam; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:OPAM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:OPAM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "opam@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global opam "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "opam@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "opam") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "opam") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading opam $CompVersion to $DownloadDir\opam..."
        $CompDownloadDir = Join-Path $DownloadDir "opam"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:OPAM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:OPAM_DOWNLOAD_URL -OutFile "$CompDownloadDir\opam-$CompVersion.zip"
        } else {
            Write-Output "OPAM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "opam@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install opam "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "opam@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add opam; vfox install "opam@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "opam") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing opam $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\opam\opam-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\opam\opam-$CompVersion.tar.gz"
        ...
        } elseif ($env:OPAM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "opam.zip"
            Invoke-WebRequest -Uri $env:OPAM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for opam."
        }
    } else {
        Write-Output "opam $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_opam" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling opam $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "opam") $CompVersion
