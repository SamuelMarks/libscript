<#
.SYNOPSIS
Windows PowerShell setup stub for composer
...
$InstallMethod = $env:COMPOSER_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls composer; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list composer; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls composer; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "composer"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote composer; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all composer; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all composer; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:COMPOSER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:COMPOSER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "composer@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global composer "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "composer@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "composer") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "composer") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading composer $CompVersion to $DownloadDir\composer..."
        $CompDownloadDir = Join-Path $DownloadDir "composer"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:COMPOSER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:COMPOSER_DOWNLOAD_URL -OutFile "$CompDownloadDir\composer-$CompVersion.zip"
        } else {
            Write-Output "COMPOSER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "composer@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install composer "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "composer@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add composer; vfox install "composer@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "composer") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing composer $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\composer\composer-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\composer\composer-$CompVersion.tar.gz"
        ...
        } elseif ($env:COMPOSER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "composer.zip"
            Invoke-WebRequest -Uri $env:COMPOSER_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for composer."
        }
    } else {
        Write-Output "composer $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_composer" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling composer $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "composer") $CompVersion
