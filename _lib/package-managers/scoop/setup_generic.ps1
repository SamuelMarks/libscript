...
<#
.SYNOPSIS
Windows PowerShell setup stub for scoop
...
$InstallMethod = $env:SCOOP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls scoop; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list scoop; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls scoop; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "scoop"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote scoop; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all scoop; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all scoop; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SCOOP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SCOOP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "scoop@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global scoop "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "scoop@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "scoop") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "scoop") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading scoop $CompVersion to $DownloadDir\scoop..."
        $CompDownloadDir = Join-Path $DownloadDir "scoop"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SCOOP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SCOOP_DOWNLOAD_URL -OutFile "$CompDownloadDir\scoop-$CompVersion.zip"
        } else {
            Write-Output "SCOOP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "scoop@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install scoop "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "scoop@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add scoop; vfox install "scoop@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "scoop") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing scoop $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\scoop\scoop-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\scoop\scoop-$CompVersion.tar.gz"
        ...
        } elseif ($env:SCOOP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "scoop.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_scoop" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling scoop $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "scoop") $CompVersion
