<#
.SYNOPSIS
Windows PowerShell setup stub for uv
...
$InstallMethod = $env:UV_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list uv; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all uv; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:UV_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:UV_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global uv "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "uv") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "uv") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading uv $CompVersion to $DownloadDir\uv..."
        $CompDownloadDir = Join-Path $DownloadDir "uv"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:UV_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:UV_DOWNLOAD_URL -OutFile "$CompDownloadDir\uv-$CompVersion.zip"
        } else {
            Write-Output "UV_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "uv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install uv "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "uv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add uv; vfox install "uv@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "uv") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing uv $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\uv\uv-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\uv\uv-$CompVersion.tar.gz"
        ...
        } elseif ($env:UV_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "uv.zip"
            Invoke-WebRequest -Uri $env:UV_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for uv."
        }
    } else {
        Write-Output "uv $CompVersion is already installed."
    }
