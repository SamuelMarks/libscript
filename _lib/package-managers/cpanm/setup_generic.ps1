<#
.SYNOPSIS
Windows PowerShell setup stub for cpanm
...
$InstallMethod = $env:CPANM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cpanm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cpanm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cpanm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cpanm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cpanm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cpanm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cpanm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CPANM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CPANM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cpanm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cpanm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cpanm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cpanm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cpanm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cpanm $CompVersion to $DownloadDir\cpanm..."
        $CompDownloadDir = Join-Path $DownloadDir "cpanm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CPANM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CPANM_DOWNLOAD_URL -OutFile "$CompDownloadDir\cpanm-$CompVersion.zip"
        } else {
            Write-Output "CPANM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cpanm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cpanm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cpanm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cpanm; vfox install "cpanm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cpanm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cpanm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cpanm\cpanm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cpanm\cpanm-$CompVersion.tar.gz"
        ...
        } elseif ($env:CPANM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cpanm.zip"
            Invoke-WebRequest -Uri $env:CPANM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for cpanm."
        }
    } else {
        Write-Output "cpanm $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cpanm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cpanm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cpanm") $CompVersion
