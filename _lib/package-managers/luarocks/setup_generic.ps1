<#
.SYNOPSIS
Windows PowerShell setup stub for luarocks
...
$CompVersion = $env:LUAROCKS_VERSION
...
$InstallMethod = $env:LUAROCKS_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls luarocks; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list luarocks; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls luarocks; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "luarocks"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote luarocks; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all luarocks; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all luarocks; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:LUAROCKS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:LUAROCKS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "luarocks@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global luarocks "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "luarocks@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "luarocks") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "luarocks") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading luarocks $CompVersion to $DownloadDir\luarocks..."
        $CompDownloadDir = Join-Path $DownloadDir "luarocks"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:LUAROCKS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:LUAROCKS_DOWNLOAD_URL -OutFile "$CompDownloadDir\luarocks-$CompVersion.zip"
        } else {
            Write-Output "LUAROCKS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "luarocks@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install luarocks "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "luarocks@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add luarocks; vfox install "luarocks@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "luarocks") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing luarocks $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\luarocks\luarocks-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\luarocks\luarocks-$CompVersion.tar.gz"
        ...
        } elseif ($env:LUAROCKS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "luarocks.zip"
            Invoke-WebRequest -Uri $env:LUAROCKS_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for luarocks."
        }
    } else {
        Write-Output "luarocks $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_luarocks" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling luarocks $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "luarocks") $CompVersion
