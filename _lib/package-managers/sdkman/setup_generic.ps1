<#
.SYNOPSIS
Windows PowerShell setup stub for sdkman
...
$CompVersion = $env:SDKMAN_VERSION
...
$InstallMethod = $env:SDKMAN_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls sdkman; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list sdkman; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls sdkman; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "sdkman"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote sdkman; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all sdkman; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all sdkman; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SDKMAN_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SDKMAN_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "sdkman@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global sdkman "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "sdkman@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "sdkman") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "sdkman") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading sdkman $CompVersion to $DownloadDir\sdkman..."
        $CompDownloadDir = Join-Path $DownloadDir "sdkman"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SDKMAN_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SDKMAN_DOWNLOAD_URL -OutFile "$CompDownloadDir\sdkman-$CompVersion.zip"
        } else {
            Write-Output "SDKMAN_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "sdkman@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install sdkman "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "sdkman@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add sdkman; vfox install "sdkman@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "sdkman") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing sdkman $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\sdkman\sdkman-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\sdkman\sdkman-$CompVersion.tar.gz"
        ...
        } elseif ($env:SDKMAN_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "sdkman.zip"
            Invoke-WebRequest -Uri $env:SDKMAN_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for sdkman."
        }
    } else {
        Write-Output "sdkman $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_sdkman" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling sdkman $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "sdkman") $CompVersion
