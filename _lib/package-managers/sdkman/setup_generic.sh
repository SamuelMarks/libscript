...
# 
# ## Usage
# Execute this script to perform generic initialization steps for sdkman.
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$SDKMAN_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${SDKMAN_RELEASES_URL:-}" ]; then
        curl -sSL "${SDKMAN_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/sdkman/sdkman-cli" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$SDKMAN_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "sdkman" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/sdkman/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "sdkman ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "sdkman" "default" "${EXACT_VERSION}"
      log_info "Set default sdkman version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env sdkman \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
    ...
    if [ "$SDKMAN_INSTALL_METHOD" = "system" ]; then
      ...
      if [ ! -d "${TARGET_DIR}" ]; then
        ...
        if command -v bash >/dev/null 2>&1; then
          curl -s "https://get.sdkman.io?rcupdate=false" | bash || true
        else
          curl -s "https://get.sdkman.io?rcupdate=false" | sh || true
        fi
      else
