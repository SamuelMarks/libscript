<#
.SYNOPSIS
Windows PowerShell setup stub for ansible-galaxy
...
$InstallMethod = $env:ANSIBLE_GALAXY_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls ansible-galaxy; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list ansible-galaxy; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls ansible-galaxy; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "ansible-galaxy"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote ansible-galaxy; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all ansible-galaxy; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all ansible-galaxy; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:ANSIBLE_GALAXY_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:ANSIBLE_GALAXY_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "ansible-galaxy@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global ansible-galaxy "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "ansible-galaxy@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "ansible-galaxy") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "ansible-galaxy") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading ansible-galaxy $CompVersion to $DownloadDir\ansible-galaxy..."
        $CompDownloadDir = Join-Path $DownloadDir "ansible-galaxy"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:ANSIBLE_GALAXY_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:ANSIBLE_GALAXY_DOWNLOAD_URL -OutFile "$CompDownloadDir\ansible-galaxy-$CompVersion.zip"
        } else {
            Write-Output "ANSIBLE_GALAXY_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "ansible-galaxy@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install ansible-galaxy "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "ansible-galaxy@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add ansible-galaxy; vfox install "ansible-galaxy@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "ansible-galaxy") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing ansible-galaxy $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\ansible-galaxy\ansible-galaxy-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\ansible-galaxy\ansible-galaxy-$CompVersion.tar.gz"
        ...
        } elseif ($env:ANSIBLE_GALAXY_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "ansible-galaxy.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_ansible-galaxy" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling ansible-galaxy $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "ansible-galaxy") $CompVersion
