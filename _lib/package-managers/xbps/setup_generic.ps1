<#
.SYNOPSIS
Windows PowerShell setup stub for xbps
...
$InstallMethod = $env:XBPS_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls xbps; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list xbps; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls xbps; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "xbps"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote xbps; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all xbps; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all xbps; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:XBPS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:XBPS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "xbps@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global xbps "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "xbps@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "xbps") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "xbps") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading xbps $CompVersion to $DownloadDir\xbps..."
        $CompDownloadDir = Join-Path $DownloadDir "xbps"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:XBPS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:XBPS_DOWNLOAD_URL -OutFile "$CompDownloadDir\xbps-$CompVersion.zip"
        } else {
            Write-Output "XBPS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "xbps@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install xbps "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "xbps@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add xbps; vfox install "xbps@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "xbps") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing xbps $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\xbps\xbps-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\xbps\xbps-$CompVersion.tar.gz"
        ...
        } elseif ($env:XBPS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "xbps.zip"
            Invoke-WebRequest -Uri $env:XBPS_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for xbps."
        }
    } else {
        Write-Output "xbps $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_xbps" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling xbps $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "xbps") $CompVersion
