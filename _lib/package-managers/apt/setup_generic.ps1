<#
.SYNOPSIS
Windows PowerShell setup stub for apt
...
$InstallMethod = $env:APT_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls apt; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list apt; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls apt; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "apt"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote apt; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all apt; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all apt; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:APT_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:APT_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "apt@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global apt "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "apt@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "apt") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "apt") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading apt $CompVersion to $DownloadDir\apt..."
        $CompDownloadDir = Join-Path $DownloadDir "apt"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:APT_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:APT_DOWNLOAD_URL -OutFile "$CompDownloadDir\apt-$CompVersion.zip"
        } else {
            Write-Output "APT_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "apt@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install apt "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "apt@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add apt; vfox install "apt@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "apt") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing apt $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\apt\apt-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\apt\apt-$CompVersion.tar.gz"
        ...
        } elseif ($env:APT_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "apt.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_apt" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling apt $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "apt") $CompVersion
