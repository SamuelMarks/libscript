<#
.SYNOPSIS
Windows PowerShell setup stub for julia
...
$InstallMethod = $env:JULIA_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls julia; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list julia; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls julia; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "julia"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote julia; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all julia; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all julia; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:JULIA_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:JULIA_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "julia@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global julia "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "julia@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "julia") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "julia") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading julia $CompVersion to $DownloadDir\julia..."
        $CompDownloadDir = Join-Path $DownloadDir "julia"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:JULIA_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:JULIA_DOWNLOAD_URL -OutFile "$CompDownloadDir\julia-$CompVersion.zip"
        } else {
            Write-Output "JULIA_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "julia@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install julia "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "julia@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add julia; vfox install "julia@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "julia") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing julia $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\julia\julia-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\julia\julia-$CompVersion.tar.gz"
        ...
        } elseif ($env:JULIA_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "julia.zip"
            Invoke-WebRequest -Uri $env:JULIA_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for julia."
        }
    } else {
        Write-Output "julia $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_julia" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling julia $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "julia") $CompVersion
