<#
.SYNOPSIS
Windows PowerShell setup stub for vcpkg
...
$InstallMethod = $env:VCPKG_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls vcpkg; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list vcpkg; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls vcpkg; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "vcpkg"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote vcpkg; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all vcpkg; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all vcpkg; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:VCPKG_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:VCPKG_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "vcpkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global vcpkg "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "vcpkg@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "vcpkg") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "vcpkg") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading vcpkg $CompVersion to $DownloadDir\vcpkg..."
        $CompDownloadDir = Join-Path $DownloadDir "vcpkg"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:VCPKG_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:VCPKG_DOWNLOAD_URL -OutFile "$CompDownloadDir\vcpkg-$CompVersion.zip"
        } else {
            Write-Output "VCPKG_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "vcpkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install vcpkg "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "vcpkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add vcpkg; vfox install "vcpkg@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "vcpkg") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing vcpkg $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\vcpkg\vcpkg-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\vcpkg\vcpkg-$CompVersion.tar.gz"
        ...
        } elseif ($env:VCPKG_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "vcpkg.zip"
            Invoke-WebRequest -Uri $env:VCPKG_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for vcpkg."
        }
    } else {
        Write-Output "vcpkg $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_vcpkg" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling vcpkg $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "vcpkg") $CompVersion
