<#
.SYNOPSIS
Windows PowerShell setup stub for r
...
$InstallMethod = $env:R_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls r; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list r; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls r; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "r"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote r; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all r; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all r; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:R_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:R_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "r@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global r "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "r@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "r") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "r") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading r $CompVersion to $DownloadDir\r..."
        $CompDownloadDir = Join-Path $DownloadDir "r"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:R_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:R_DOWNLOAD_URL -OutFile "$CompDownloadDir\r-$CompVersion.zip"
        } else {
            Write-Output "R_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "r@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install r "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "r@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add r; vfox install "r@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "r") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing r $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\r\r-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\r\r-$CompVersion.tar.gz"
        ...
        } elseif ($env:R_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "r.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_r" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling r $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "r") $CompVersion
