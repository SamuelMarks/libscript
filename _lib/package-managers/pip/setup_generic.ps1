<#
.SYNOPSIS
Windows PowerShell setup stub for pip
...
$InstallMethod = $env:PIP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pip; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pip; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pip; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pip"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pip; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pip; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pip; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PIP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PIP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pip@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pip "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pip@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pip") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pip") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pip $CompVersion to $DownloadDir\pip..."
        $CompDownloadDir = Join-Path $DownloadDir "pip"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PIP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PIP_DOWNLOAD_URL -OutFile "$CompDownloadDir\pip-$CompVersion.zip"
        } else {
            Write-Output "PIP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pip@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pip "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pip@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pip; vfox install "pip@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pip") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pip $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pip\pip-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pip\pip-$CompVersion.tar.gz"
        ...
        } elseif ($env:PIP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pip.zip"
            Invoke-WebRequest -Uri $env:PIP_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for pip."
        }
    } else {
        Write-Output "pip $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pip" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pip $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pip") $CompVersion
