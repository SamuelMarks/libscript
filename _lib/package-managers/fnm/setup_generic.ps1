<#
.SYNOPSIS
Windows PowerShell setup stub for fnm
...
$InstallMethod = $env:FNM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls fnm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list fnm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls fnm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "fnm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote fnm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all fnm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all fnm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:FNM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:FNM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "fnm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global fnm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "fnm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "fnm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "fnm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading fnm $CompVersion to $DownloadDir\fnm..."
        $CompDownloadDir = Join-Path $DownloadDir "fnm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:FNM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:FNM_DOWNLOAD_URL -OutFile "$CompDownloadDir\fnm-$CompVersion.zip"
        } else {
            Write-Output "FNM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "fnm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install fnm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "fnm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add fnm; vfox install "fnm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "fnm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing fnm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\fnm\fnm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\fnm\fnm-$CompVersion.tar.gz"
        ...
        } elseif ($env:FNM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "fnm.zip"
            Invoke-WebRequest -Uri $env:FNM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for fnm."
        }
    } else {
        Write-Output "fnm $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_fnm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling fnm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "fnm") $CompVersion
