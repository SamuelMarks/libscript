<#
.SYNOPSIS
Windows PowerShell setup stub for spack
...
$InstallMethod = $env:SPACK_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls spack; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list spack; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls spack; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "spack"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote spack; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all spack; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all spack; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SPACK_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SPACK_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "spack@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global spack "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "spack@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "spack") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "spack") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading spack $CompVersion to $DownloadDir\spack..."
        $CompDownloadDir = Join-Path $DownloadDir "spack"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SPACK_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SPACK_DOWNLOAD_URL -OutFile "$CompDownloadDir\spack-$CompVersion.zip"
        } else {
            Write-Output "SPACK_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "spack@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install spack "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "spack@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add spack; vfox install "spack@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "spack") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing spack $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\spack\spack-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\spack\spack-$CompVersion.tar.gz"
        ...
        } elseif ($env:SPACK_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "spack.zip"
            Invoke-WebRequest -Uri $env:SPACK_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for spack."
        }
    } else {
        Write-Output "spack $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_spack" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling spack $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "spack") $CompVersion
