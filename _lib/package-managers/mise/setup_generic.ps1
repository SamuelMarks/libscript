<#
.SYNOPSIS
Windows PowerShell setup stub for mise
...
$InstallMethod = $env:MISE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls mise; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list mise; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls mise; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "mise"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote mise; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all mise; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all mise; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MISE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MISE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "mise@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global mise "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "mise@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "mise") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "mise") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading mise $CompVersion to $DownloadDir\mise..."
        $CompDownloadDir = Join-Path $DownloadDir "mise"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MISE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MISE_DOWNLOAD_URL -OutFile "$CompDownloadDir\mise-$CompVersion.zip"
        } else {
            Write-Output "MISE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "mise@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install mise "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "mise@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add mise; vfox install "mise@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "mise") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing mise $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\mise\mise-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\mise\mise-$CompVersion.tar.gz"
        ...
        } elseif ($env:MISE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "mise.zip"
            Invoke-WebRequest -Uri $env:MISE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for mise."
        }
    } else {
        Write-Output "mise $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_mise" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling mise $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "mise") $CompVersion
