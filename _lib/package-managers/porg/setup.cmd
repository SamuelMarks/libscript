...
::
:: ## Overview
:: Recipe for Porg: Package organizer with LD_PRELOAD logging for LFS.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.porg"
if exist "%STAMP_FILE%" (
    echo [SKIP]  porg already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Porg into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    porg staged successfully: %STAMP_FILE%
