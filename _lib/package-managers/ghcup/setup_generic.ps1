<#
.SYNOPSIS
Windows PowerShell setup stub for ghcup
...
$InstallMethod = $env:GHCUP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls ghcup; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list ghcup; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls ghcup; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "ghcup"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote ghcup; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all ghcup; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all ghcup; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GHCUP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GHCUP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "ghcup@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global ghcup "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "ghcup@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "ghcup") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "ghcup") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading ghcup $CompVersion to $DownloadDir\ghcup..."
        $CompDownloadDir = Join-Path $DownloadDir "ghcup"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GHCUP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GHCUP_DOWNLOAD_URL -OutFile "$CompDownloadDir\ghcup-$CompVersion.zip"
        } else {
            Write-Output "GHCUP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "ghcup@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install ghcup "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "ghcup@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add ghcup; vfox install "ghcup@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "ghcup") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing ghcup $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\ghcup\ghcup-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\ghcup\ghcup-$CompVersion.tar.gz"
        ...
        } elseif ($env:GHCUP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "ghcup.zip"
            Invoke-WebRequest -Uri $env:GHCUP_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for ghcup."
        }
    } else {
        Write-Output "ghcup $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_ghcup" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling ghcup $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "ghcup") $CompVersion
