<#
.SYNOPSIS
Windows PowerShell setup stub for npm
...
$InstallMethod = $env:NPM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls npm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list npm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls npm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "npm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote npm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all npm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all npm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:NPM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NPM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "npm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global npm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "npm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "npm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "npm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading npm $CompVersion to $DownloadDir\npm..."
        $CompDownloadDir = Join-Path $DownloadDir "npm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NPM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NPM_DOWNLOAD_URL -OutFile "$CompDownloadDir\npm-$CompVersion.zip"
        } else {
            Write-Output "NPM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "npm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install npm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "npm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add npm; vfox install "npm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "npm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing npm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\npm\npm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\npm\npm-$CompVersion.tar.gz"
        ...
        } elseif ($env:NPM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "npm.zip"
            Invoke-WebRequest -Uri $env:NPM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for npm."
        }
    } else {
        Write-Output "npm $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_npm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling npm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "npm") $CompVersion
