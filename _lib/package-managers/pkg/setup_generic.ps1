<#
.SYNOPSIS
Windows PowerShell setup stub for pkg
...
$InstallMethod = $env:PKG_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pkg; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pkg; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pkg; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pkg"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pkg; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pkg; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pkg; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PKG_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PKG_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pkg "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pkg@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pkg") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pkg") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pkg $CompVersion to $DownloadDir\pkg..."
        $CompDownloadDir = Join-Path $DownloadDir "pkg"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PKG_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PKG_DOWNLOAD_URL -OutFile "$CompDownloadDir\pkg-$CompVersion.zip"
        } else {
            Write-Output "PKG_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pkg "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pkg@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pkg; vfox install "pkg@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pkg") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pkg $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pkg\pkg-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pkg\pkg-$CompVersion.tar.gz"
        ...
        } elseif ($env:PKG_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pkg.zip"
            Invoke-WebRequest -Uri $env:PKG_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for pkg."
        }
    } else {
        Write-Output "pkg $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pkg" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pkg $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pkg") $CompVersion
