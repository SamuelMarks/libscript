#!/bin/sh
# ## Overview
# Generic setup script for the pkg component.
  ...
  if [ "$(uname)" = "FreeBSD" ]; then
    if [ "$(id -u)" = "0" ]; then
      ...
      if command -v sudo >/dev/null 2>&1; then
        ...
        printf '%s\n' "Error: Must be root or have sudo to bootstrap pkg on FreeBSD." >&2
        exit 1
      fi
    fi
  else
    printf '%s\n' "Warning: The 'pkg' package manager is only applicable on FreeBSD. Skipping." >&2
  fi
fi
