<#
.SYNOPSIS
Windows PowerShell setup stub for pnpm
...
$InstallMethod = $env:PNPM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pnpm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pnpm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pnpm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pnpm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pnpm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pnpm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pnpm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PNPM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PNPM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pnpm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pnpm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pnpm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pnpm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pnpm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pnpm $CompVersion to $DownloadDir\pnpm..."
        $CompDownloadDir = Join-Path $DownloadDir "pnpm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PNPM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PNPM_DOWNLOAD_URL -OutFile "$CompDownloadDir\pnpm-$CompVersion.zip"
        } else {
            Write-Output "PNPM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pnpm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pnpm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pnpm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pnpm; vfox install "pnpm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pnpm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pnpm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pnpm\pnpm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pnpm\pnpm-$CompVersion.tar.gz"
        ...
        } elseif ($env:PNPM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pnpm.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pnpm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pnpm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pnpm") $CompVersion
