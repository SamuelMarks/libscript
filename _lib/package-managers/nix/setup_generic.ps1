<#
.SYNOPSIS
Windows PowerShell setup stub for nix
...
$InstallMethod = $env:NIX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls nix; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list nix; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls nix; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "nix"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote nix; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all nix; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all nix; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:NIX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NIX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "nix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global nix "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "nix@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "nix") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "nix") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading nix $CompVersion to $DownloadDir\nix..."
        $CompDownloadDir = Join-Path $DownloadDir "nix"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NIX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NIX_DOWNLOAD_URL -OutFile "$CompDownloadDir\nix-$CompVersion.zip"
        } else {
            Write-Output "NIX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "nix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install nix "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "nix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add nix; vfox install "nix@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "nix") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing nix $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\nix\nix-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\nix\nix-$CompVersion.tar.gz"
        ...
        } elseif ($env:NIX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "nix.zip"
            Invoke-WebRequest -Uri $env:NIX_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for nix."
        }
    } else {
        Write-Output "nix $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_nix" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling nix $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "nix") $CompVersion
