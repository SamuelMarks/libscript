#!/bin/sh
# ## Overview
# Generic setup script for the conda component.
...
if ! command -v conda >/dev/null 2>&1; then
  ...
  if [ "$OS" = "Linux" ]; then
    if [ "$ARCH" = "x86_64" ]; then
      ...
      printf '%s\n' "Error: Unsupported architecture $ARCH for Miniconda." >&2
      exit 1
    fi
  elif [ "$OS" = "Darwin" ]; then
    if [ "$ARCH" = "x86_64" ]; then
    ...
    printf '%s\n' "Error: Unsupported OS $OS for miniconda automated install." >&2
    exit 1
  fi
