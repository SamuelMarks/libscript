<#
.SYNOPSIS
Windows PowerShell setup stub for gem
...
$InstallMethod = $env:GEM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls gem; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list gem; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls gem; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "gem"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote gem; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all gem; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all gem; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GEM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GEM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "gem@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global gem "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "gem@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "gem") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "gem") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading gem $CompVersion to $DownloadDir\gem..."
        $CompDownloadDir = Join-Path $DownloadDir "gem"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GEM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GEM_DOWNLOAD_URL -OutFile "$CompDownloadDir\gem-$CompVersion.zip"
        } else {
            Write-Output "GEM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "gem@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install gem "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "gem@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add gem; vfox install "gem@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "gem") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing gem $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\gem\gem-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\gem\gem-$CompVersion.tar.gz"
        ...
        } elseif ($env:GEM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "gem.zip"
            Invoke-WebRequest -Uri $env:GEM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for gem."
        }
    } else {
        Write-Output "gem $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_gem" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling gem $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "gem") $CompVersion
