<#
.SYNOPSIS
Windows PowerShell setup stub for guix
...
$InstallMethod = $env:GUIX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls guix; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list guix; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls guix; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "guix"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote guix; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all guix; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all guix; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GUIX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GUIX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "guix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global guix "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "guix@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "guix") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "guix") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading guix $CompVersion to $DownloadDir\guix..."
        $CompDownloadDir = Join-Path $DownloadDir "guix"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GUIX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GUIX_DOWNLOAD_URL -OutFile "$CompDownloadDir\guix-$CompVersion.zip"
        } else {
            Write-Output "GUIX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "guix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install guix "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "guix@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add guix; vfox install "guix@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "guix") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing guix $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\guix\guix-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\guix\guix-$CompVersion.tar.gz"
        ...
        } elseif ($env:GUIX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "guix.zip"
            Invoke-WebRequest -Uri $env:GUIX_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for guix."
        }
    } else {
        Write-Output "guix $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_guix" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling guix $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "guix") $CompVersion
