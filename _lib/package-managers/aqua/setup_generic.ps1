<#
.SYNOPSIS
Windows PowerShell setup stub for aqua
...
$InstallMethod = $env:AQUA_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls aqua; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list aqua; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls aqua; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "aqua"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote aqua; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all aqua; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all aqua; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:AQUA_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:AQUA_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "aqua@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global aqua "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "aqua@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "aqua") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "aqua") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading aqua $CompVersion to $DownloadDir\aqua..."
        $CompDownloadDir = Join-Path $DownloadDir "aqua"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:AQUA_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:AQUA_DOWNLOAD_URL -OutFile "$CompDownloadDir\aqua-$CompVersion.zip"
        } else {
            Write-Output "AQUA_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "aqua@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install aqua "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "aqua@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add aqua; vfox install "aqua@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "aqua") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing aqua $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\aqua\aqua-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\aqua\aqua-$CompVersion.tar.gz"
        ...
        } elseif ($env:AQUA_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "aqua.zip"
            Invoke-WebRequest -Uri $env:AQUA_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for aqua."
        }
    } else {
        Write-Output "aqua $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_aqua" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling aqua $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "aqua") $CompVersion
