<#
.SYNOPSIS
Windows PowerShell setup stub for flatpak
...
$CompVersion = $env:FLATPAK_VERSION
...
$InstallMethod = $env:FLATPAK_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls flatpak; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list flatpak; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls flatpak; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "flatpak"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote flatpak; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all flatpak; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all flatpak; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:FLATPAK_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:FLATPAK_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "flatpak@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global flatpak "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "flatpak@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "flatpak") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "flatpak") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading flatpak $CompVersion to $DownloadDir\flatpak..."
        $CompDownloadDir = Join-Path $DownloadDir "flatpak"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:FLATPAK_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:FLATPAK_DOWNLOAD_URL -OutFile "$CompDownloadDir\flatpak-$CompVersion.zip"
        } else {
            Write-Output "FLATPAK_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "flatpak@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install flatpak "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "flatpak@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add flatpak; vfox install "flatpak@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "flatpak") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing flatpak $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\flatpak\flatpak-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\flatpak\flatpak-$CompVersion.tar.gz"
        ...
        } elseif ($env:FLATPAK_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "flatpak.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_flatpak" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling flatpak $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "flatpak") $CompVersion
