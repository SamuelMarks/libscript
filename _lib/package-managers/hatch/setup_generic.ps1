...
<#
.SYNOPSIS
Windows PowerShell setup stub for hatch
...
$InstallMethod = $env:HATCH_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls hatch; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list hatch; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls hatch; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "hatch"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote hatch; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all hatch; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all hatch; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:HATCH_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:HATCH_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "hatch@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global hatch "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "hatch@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "hatch") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "hatch") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading hatch $CompVersion to $DownloadDir\hatch..."
        $CompDownloadDir = Join-Path $DownloadDir "hatch"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:HATCH_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:HATCH_DOWNLOAD_URL -OutFile "$CompDownloadDir\hatch-$CompVersion.zip"
        } else {
            Write-Output "HATCH_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "hatch@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install hatch "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "hatch@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add hatch; vfox install "hatch@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "hatch") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing hatch $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\hatch\hatch-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\hatch\hatch-$CompVersion.tar.gz"
        ...
        } elseif ($env:HATCH_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "hatch.zip"
            Invoke-WebRequest -Uri $env:HATCH_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for hatch."
            exit 1
        }
    } else {
        Write-Output "hatch $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_hatch" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling hatch $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "hatch") $CompVersion
