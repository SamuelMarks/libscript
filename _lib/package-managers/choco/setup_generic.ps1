<#
.SYNOPSIS
Windows PowerShell setup stub for choco
...
$InstallMethod = $env:CHOCO_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls choco; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list choco; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls choco; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "choco"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote choco; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all choco; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all choco; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CHOCO_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CHOCO_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "choco@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global choco "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "choco@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "choco") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "choco") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading choco $CompVersion to $DownloadDir\choco..."
        $CompDownloadDir = Join-Path $DownloadDir "choco"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CHOCO_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CHOCO_DOWNLOAD_URL -OutFile "$CompDownloadDir\choco-$CompVersion.zip"
        } else {
            Write-Output "CHOCO_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "choco@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install choco "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "choco@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add choco; vfox install "choco@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "choco") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing choco $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\choco\choco-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\choco\choco-$CompVersion.tar.gz"
        ...
        } elseif ($env:CHOCO_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "choco.zip"
            Invoke-WebRequest -Uri $env:CHOCO_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for choco."
        }
    } else {
        Write-Output "choco $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_choco" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling choco $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "choco") $CompVersion
