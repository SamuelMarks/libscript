<#
.SYNOPSIS
Windows PowerShell setup stub for paru
...
$InstallMethod = $env:PARU_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls paru; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list paru; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls paru; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "paru"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote paru; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all paru; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all paru; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PARU_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PARU_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "paru@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global paru "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "paru@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "paru") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "paru") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading paru $CompVersion to $DownloadDir\paru..."
        $CompDownloadDir = Join-Path $DownloadDir "paru"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PARU_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PARU_DOWNLOAD_URL -OutFile "$CompDownloadDir\paru-$CompVersion.zip"
        } else {
            Write-Output "PARU_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "paru@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install paru "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "paru@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add paru; vfox install "paru@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "paru") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing paru $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\paru\paru-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\paru\paru-$CompVersion.tar.gz"
        ...
        } elseif ($env:PARU_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "paru.zip"
            Invoke-WebRequest -Uri $env:PARU_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for paru."
        }
    } else {
        Write-Output "paru $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_paru" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling paru $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "paru") $CompVersion
