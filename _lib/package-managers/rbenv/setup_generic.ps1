<#
.SYNOPSIS
Windows PowerShell setup stub for rbenv
...
$InstallMethod = $env:RBENV_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls rbenv; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list rbenv; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls rbenv; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "rbenv"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote rbenv; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all rbenv; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all rbenv; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:RBENV_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:RBENV_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "rbenv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global rbenv "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "rbenv@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "rbenv") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "rbenv") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rbenv $CompVersion to $DownloadDir\rbenv..."
        $CompDownloadDir = Join-Path $DownloadDir "rbenv"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:RBENV_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:RBENV_DOWNLOAD_URL -OutFile "$CompDownloadDir\rbenv-$CompVersion.zip"
        } else {
            Write-Output "RBENV_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "rbenv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install rbenv "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "rbenv@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add rbenv; vfox install "rbenv@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "rbenv") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing rbenv $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\rbenv\rbenv-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\rbenv\rbenv-$CompVersion.tar.gz"
        ...
        } elseif ($env:RBENV_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "rbenv.zip"
            Invoke-WebRequest -Uri $env:RBENV_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for rbenv."
        }
    } else {
        Write-Output "rbenv $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_rbenv" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling rbenv $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "rbenv") $CompVersion
