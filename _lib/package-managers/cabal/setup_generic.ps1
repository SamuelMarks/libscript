<#
.SYNOPSIS
Windows PowerShell setup stub for cabal
...
$InstallMethod = $env:CABAL_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cabal; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cabal; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cabal; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cabal"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cabal; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cabal; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cabal; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CABAL_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CABAL_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cabal@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cabal "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cabal@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cabal") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cabal") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cabal $CompVersion to $DownloadDir\cabal..."
        $CompDownloadDir = Join-Path $DownloadDir "cabal"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CABAL_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CABAL_DOWNLOAD_URL -OutFile "$CompDownloadDir\cabal-$CompVersion.zip"
        } else {
            Write-Output "CABAL_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cabal@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cabal "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cabal@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cabal; vfox install "cabal@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cabal") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cabal $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cabal\cabal-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cabal\cabal-$CompVersion.tar.gz"
        ...
        } elseif ($env:CABAL_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cabal.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cabal" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cabal $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cabal") $CompVersion
