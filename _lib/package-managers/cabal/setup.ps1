...
<#
.SYNOPSIS
Orchestrates the setup and installation process for the component 'cabal' stack.

.DESCRIPTION
Execute this script to install and configure cabal on the local system.
#>
...
if (-Not (Get-Command cabal -ErrorAction SilentlyContinue)) {
    ...
    $ghcupSetup = [System.IO.Path]::Combine($scriptDir, "..", "ghcup", "setup.ps1")
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cabal to $DownloadDir\cabal..."
    }
    exit 0
}
