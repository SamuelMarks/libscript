<#
.SYNOPSIS
Windows PowerShell setup stub for cygwin
...
$CompVersion = $env:CYGWIN_VERSION
...
$InstallMethod = $env:CYGWIN_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cygwin; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cygwin; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cygwin; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cygwin"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cygwin; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cygwin; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cygwin; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CYGWIN_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CYGWIN_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cygwin@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cygwin "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cygwin@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cygwin") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cygwin") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cygwin $CompVersion to $DownloadDir\cygwin..."
        $CompDownloadDir = Join-Path $DownloadDir "cygwin"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CYGWIN_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CYGWIN_DOWNLOAD_URL -OutFile "$CompDownloadDir\cygwin-$CompVersion.zip"
        } else {
            Write-Output "CYGWIN_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cygwin@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cygwin "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cygwin@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cygwin; vfox install "cygwin@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cygwin") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cygwin $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cygwin\cygwin-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cygwin\cygwin-$CompVersion.tar.gz"
        ...
        } elseif ($env:CYGWIN_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cygwin.zip"
            Invoke-WebRequest -Uri $env:CYGWIN_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for cygwin."
        }
    } else {
        Write-Output "cygwin $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cygwin" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cygwin $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cygwin") $CompVersion
