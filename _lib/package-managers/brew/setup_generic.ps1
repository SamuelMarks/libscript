<#
.SYNOPSIS
Windows PowerShell setup stub for brew
...
$InstallMethod = $env:BREW_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls brew; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list brew; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls brew; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "brew"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote brew; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all brew; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all brew; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:BREW_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:BREW_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "brew@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global brew "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "brew@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "brew") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "brew") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading brew $CompVersion to $DownloadDir\brew..."
        $CompDownloadDir = Join-Path $DownloadDir "brew"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:BREW_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:BREW_DOWNLOAD_URL -OutFile "$CompDownloadDir\brew-$CompVersion.zip"
        } else {
            Write-Output "BREW_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "brew@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install brew "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "brew@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add brew; vfox install "brew@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "brew") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing brew $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\brew\brew-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\brew\brew-$CompVersion.tar.gz"
        ...
        } elseif ($env:BREW_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "brew.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_brew" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling brew $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "brew") $CompVersion
