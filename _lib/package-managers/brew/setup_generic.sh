#!/bin/sh
# ## Overview
# Generic setup script for the brew component.
...
if ! command -v brew >/dev/null 2>&1; then
  _tmp_script="/tmp/brew-install.sh"
  libscript_download "https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh" "$_tmp_script"
  NONINTERACTIVE=1 /bin/bash "$_tmp_script"
  rm -f "$_tmp_script"
fi
