<#
.SYNOPSIS
Windows PowerShell setup stub for msys2
...
$InstallMethod = $env:MSYS2_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls msys2; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list msys2; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls msys2; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "msys2"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote msys2; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all msys2; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all msys2; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MSYS2_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MSYS2_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "msys2@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global msys2 "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "msys2@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "msys2") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "msys2") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading msys2 $CompVersion to $DownloadDir\msys2..."
        $CompDownloadDir = Join-Path $DownloadDir "msys2"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MSYS2_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MSYS2_DOWNLOAD_URL -OutFile "$CompDownloadDir\msys2-$CompVersion.zip"
        } else {
            Write-Output "MSYS2_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "msys2@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install msys2 "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "msys2@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add msys2; vfox install "msys2@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "msys2") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing msys2 $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\msys2\msys2-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\msys2\msys2-$CompVersion.tar.gz"
        ...
        } elseif ($env:MSYS2_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "msys2.zip"
            Invoke-WebRequest -Uri $env:MSYS2_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for msys2."
        }
    } else {
        Write-Output "msys2 $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_msys2" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling msys2 $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "msys2") $CompVersion
