<#
.SYNOPSIS
Windows PowerShell setup stub for snap
...
$InstallMethod = $env:SNAP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls snap; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list snap; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls snap; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "snap"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote snap; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all snap; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all snap; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SNAP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SNAP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "snap@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global snap "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "snap@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "snap") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "snap") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading snap $CompVersion to $DownloadDir\snap..."
        $CompDownloadDir = Join-Path $DownloadDir "snap"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SNAP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SNAP_DOWNLOAD_URL -OutFile "$CompDownloadDir\snap-$CompVersion.zip"
        } else {
            Write-Output "SNAP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "snap@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install snap "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "snap@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add snap; vfox install "snap@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "snap") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing snap $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\snap\snap-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\snap\snap-$CompVersion.tar.gz"
        ...
        } elseif ($env:SNAP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "snap.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_snap" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling snap $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "snap") $CompVersion
