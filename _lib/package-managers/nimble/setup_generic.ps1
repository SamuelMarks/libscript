<#
.SYNOPSIS
Windows PowerShell setup stub for nimble
...
$CompVersion = $env:NIMBLE_VERSION
...
$InstallMethod = $env:NIMBLE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls nimble; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list nimble; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls nimble; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "nimble"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote nimble; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all nimble; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all nimble; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:NIMBLE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NIMBLE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "nimble@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global nimble "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "nimble@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "nimble") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "nimble") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading nimble $CompVersion to $DownloadDir\nimble..."
        $CompDownloadDir = Join-Path $DownloadDir "nimble"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NIMBLE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NIMBLE_DOWNLOAD_URL -OutFile "$CompDownloadDir\nimble-$CompVersion.zip"
        } else {
            Write-Output "NIMBLE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "nimble@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install nimble "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "nimble@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add nimble; vfox install "nimble@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "nimble") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing nimble $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\nimble\nimble-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\nimble\nimble-$CompVersion.tar.gz"
        ...
        } elseif ($env:NIMBLE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "nimble.zip"
            Invoke-WebRequest -Uri $env:NIMBLE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for nimble."
        }
    } else {
        Write-Output "nimble $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_nimble" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling nimble $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "nimble") $CompVersion
