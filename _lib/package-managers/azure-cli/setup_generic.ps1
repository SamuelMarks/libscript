<#
.SYNOPSIS
Windows PowerShell setup stub for azure-cli
...
$InstallMethod = $env:AZURE_CLI_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls azure-cli; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list azure-cli; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls azure-cli; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "azure-cli"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote azure-cli; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all azure-cli; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all azure-cli; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:AZURE_CLI_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:AZURE_CLI_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "azure-cli@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global azure-cli "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "azure-cli@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "azure-cli") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "azure-cli") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading azure-cli $CompVersion to $DownloadDir\azure-cli..."
        $CompDownloadDir = Join-Path $DownloadDir "azure-cli"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:AZURE_CLI_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:AZURE_CLI_DOWNLOAD_URL -OutFile "$CompDownloadDir\azure-cli-$CompVersion.zip"
        } else {
            Write-Output "AZURE_CLI_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "azure-cli@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install azure-cli "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "azure-cli@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add azure-cli; vfox install "azure-cli@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "azure-cli") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing azure-cli $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\azure-cli\azure-cli-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\azure-cli\azure-cli-$CompVersion.tar.gz"
        ...
        } elseif ($env:AZURE_CLI_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "azure-cli.zip"
            Invoke-WebRequest -Uri $env:AZURE_CLI_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for azure-cli."
        }
    } else {
        Write-Output "azure-cli $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_azure-cli" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling azure-cli $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "azure-cli") $CompVersion
