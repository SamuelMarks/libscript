...
| `AZURE_CLI_INSTALL_METHOD` | How to install AZURE_CLI. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager, 'mise', 'asdf', 'pkgx', or 'vfox' defers to third-party tools. | `libscript_native` |  |
| `AZURE_CLI_VERSION` | Version of Azure CLI to install | `latest` |  |
<!-- END_VARS -->
...
`libscript` manages `azure-cli` versions natively by default
(`AZURE_CLI_INSTALL_METHOD=libscript_native`), ensuring isolated installations without polluting
global system paths. You can override this to use `system`, `mise`, `asdf`, `pkgx`, or `vfox` if
preferred.

Libscript manages azure-cli versions natively by installing them into isolated directories under
`LIBSCRIPT_HOME/azure-cli/<version>`.
