...
<#
.SYNOPSIS
Windows PowerShell setup stub for rebar3
...
$CompVersion = $env:REBAR3_VERSION
...
$InstallMethod = $env:REBAR3_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls rebar3; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list rebar3; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls rebar3; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "rebar3"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote rebar3; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all rebar3; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all rebar3; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:REBAR3_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:REBAR3_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "rebar3@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global rebar3 "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "rebar3@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "rebar3") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "rebar3") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rebar3 $CompVersion to $DownloadDir\rebar3..."
        $CompDownloadDir = Join-Path $DownloadDir "rebar3"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:REBAR3_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:REBAR3_DOWNLOAD_URL -OutFile "$CompDownloadDir\rebar3-$CompVersion.zip"
        } else {
            Write-Output "REBAR3_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "rebar3@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install rebar3 "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "rebar3@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add rebar3; vfox install "rebar3@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "rebar3") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing rebar3 $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\rebar3\rebar3-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\rebar3\rebar3-$CompVersion.tar.gz"
        ...
        } elseif ($env:REBAR3_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "rebar3.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_rebar3" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling rebar3 $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "rebar3") $CompVersion
