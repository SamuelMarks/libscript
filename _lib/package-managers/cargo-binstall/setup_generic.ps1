<#
.SYNOPSIS
Windows PowerShell setup stub for cargo-binstall
...
$InstallMethod = $env:CARGO_BINSTALL_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list cargo-binstall; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all cargo-binstall; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:CARGO_BINSTALL_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CARGO_BINSTALL_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global cargo-binstall "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cargo-binstall") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cargo-binstall") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cargo-binstall $CompVersion to $DownloadDir\cargo-binstall..."
        $CompDownloadDir = Join-Path $DownloadDir "cargo-binstall"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CARGO_BINSTALL_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CARGO_BINSTALL_DOWNLOAD_URL -OutFile "$CompDownloadDir\cargo-binstall-$CompVersion.zip"
        } else {
            Write-Output "CARGO_BINSTALL_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cargo-binstall@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cargo-binstall "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cargo-binstall@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cargo-binstall; vfox install "cargo-binstall@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cargo-binstall $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cargo-binstall\cargo-binstall-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cargo-binstall\cargo-binstall-$CompVersion.tar.gz"
        ...
        } elseif ($env:CARGO_BINSTALL_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cargo-binstall.zip"
            Invoke-WebRequest -Uri $env:CARGO_BINSTALL_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for cargo-binstall."
        }
    } else {
        Write-Output "cargo-binstall $CompVersion is already installed."
    }
