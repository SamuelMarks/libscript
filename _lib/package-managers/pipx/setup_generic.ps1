...
<#
.SYNOPSIS
Windows PowerShell setup stub for pipx
...
$InstallMethod = $env:PIPX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls pipx; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list pipx; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls pipx; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "pipx"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote pipx; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all pipx; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all pipx; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PIPX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PIPX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "pipx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global pipx "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "pipx@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pipx") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pipx") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pipx $CompVersion to $DownloadDir\pipx..."
        $CompDownloadDir = Join-Path $DownloadDir "pipx"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PIPX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PIPX_DOWNLOAD_URL -OutFile "$CompDownloadDir\pipx-$CompVersion.zip"
        } else {
            Write-Output "PIPX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pipx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pipx "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pipx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pipx; vfox install "pipx@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pipx") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pipx $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pipx\pipx-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pipx\pipx-$CompVersion.tar.gz"
        ...
        } elseif ($env:PIPX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pipx.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_pipx" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling pipx $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "pipx") $CompVersion
