...
<#
.SYNOPSIS
Windows PowerShell setup stub for pkgx
...
$InstallMethod = $env:PKGX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list pkgx; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all pkgx; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:PKGX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PKGX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global pkgx "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "pkgx") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "pkgx") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading pkgx $CompVersion to $DownloadDir\pkgx..."
        $CompDownloadDir = Join-Path $DownloadDir "pkgx"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PKGX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PKGX_DOWNLOAD_URL -OutFile "$CompDownloadDir\pkgx-$CompVersion.zip"
        } else {
            Write-Output "PKGX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "pkgx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install pkgx "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "pkgx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add pkgx; vfox install "pkgx@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "pkgx") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing pkgx $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\pkgx\pkgx-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\pkgx\pkgx-$CompVersion.tar.gz"
        ...
        } elseif ($env:PKGX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "pkgx.zip"
