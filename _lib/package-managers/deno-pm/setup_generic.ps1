<#
.SYNOPSIS
Windows PowerShell setup stub for deno-pm
...
$InstallMethod = $env:DENO_PM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls deno-pm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list deno-pm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls deno-pm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "deno-pm"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote deno-pm; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all deno-pm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all deno-pm; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:DENO_PM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:DENO_PM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "deno-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global deno-pm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "deno-pm@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "deno-pm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "deno-pm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading deno-pm $CompVersion to $DownloadDir\deno-pm..."
        $CompDownloadDir = Join-Path $DownloadDir "deno-pm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:DENO_PM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:DENO_PM_DOWNLOAD_URL -OutFile "$CompDownloadDir\deno-pm-$CompVersion.zip"
        } else {
            Write-Output "DENO_PM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "deno-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install deno-pm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "deno-pm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add deno-pm; vfox install "deno-pm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "deno-pm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing deno-pm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\deno-pm\deno-pm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\deno-pm\deno-pm-$CompVersion.tar.gz"
        ...
        } elseif ($env:DENO_PM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "deno-pm.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_deno-pm" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling deno-pm $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "deno-pm") $CompVersion
