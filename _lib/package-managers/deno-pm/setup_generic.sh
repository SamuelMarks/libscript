...
# 
# ## Usage
# Execute this script to perform generic initialization steps for deno-pm.
...
DENO_PM_INSTALL_METHOD="${DENO_PM_INSTALL_METHOD:-system}"
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$DENO_PM_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${DENO_PM_RELEASES_URL:-}" ]; then
        curl -sSL "${DENO_PM_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/rycont/pm-b310-w2-reversing-deno" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$DENO_PM_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "deno-pm" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/deno-pm/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "deno-pm ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "deno-pm" "default" "${EXACT_VERSION}"
      log_info "Set default deno-pm version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env deno-pm \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
