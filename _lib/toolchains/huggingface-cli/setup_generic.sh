...
HUGGINGFACE_CLI_INSTALL_METHOD="${HUGGINGFACE_CLI_INSTALL_METHOD:-libscript_native}"
HUGGINGFACE_CLI_INSTALL_METHOD="$(LIBSCRIPT_DEFAULT_INSTALL_METHOD="$HUGGINGFACE_CLI_INSTALL_METHOD" libscript_resolve_install_method "HUGGINGFACE_CLI")"
...
case "$ACTION" in
  ...
  use)
    if [ "${HUGGINGFACE_CLI_INSTALL_METHOD}" = "mise" ]; then
      ...
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env huggingface-cli \"${HUGGINGFACE_CLI_VERSION}\")"
    fi
    exit 0
    ;;
  download)
