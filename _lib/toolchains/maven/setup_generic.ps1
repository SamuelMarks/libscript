<#
.SYNOPSIS
Windows PowerShell setup stub for maven
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls maven; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list maven; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls maven; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "maven"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote maven; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all maven; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all maven; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MAVEN_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MAVEN_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "maven@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global maven "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "maven@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/apache/maven/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "maven") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "maven") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading maven $CompVersion to $DownloadDir\maven..."
        $CompDownloadDir = Join-Path $DownloadDir "maven"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MAVEN_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MAVEN_DOWNLOAD_URL -OutFile "$CompDownloadDir\maven-$CompVersion.zip"
        } else {
            Write-Output "MAVEN_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "maven@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install maven "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "maven@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add maven; vfox install "maven@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "maven") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing maven $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\maven\maven-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\maven\maven-$CompVersion.tar.gz"
        ...
        } elseif ($env:MAVEN_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "maven.zip"
            Invoke-WebRequest -Uri $env:MAVEN_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for maven."
        }
    } else {
        Write-Output "maven $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_maven" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling maven $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "maven") $CompVersion
