...
MAVEN_INSTALL_METHOD="$(libscript_resolve_install_method "MAVEN")"
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "${MAVEN_INSTALL_METHOD}" = "mise" ]; then
      ...
      echo "Fetching remote versions not implemented generically for maven"
    fi
    exit 0
    ;;
  use)
    ...
    if [ "${MAVEN_INSTALL_METHOD}" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/maven/"*"${VERSION}"* >/dev/null 2>&1; then
        ...
        if [ -n "${MAVEN_DOWNLOAD_URL:-}" ]; then
          ...
          # Fallback to mock
          echo "#!/bin/sh" > "${TARGET_DIR}/bin/maven"
          echo "echo 'Mock maven executable for version ${EXACT_VERSION}'" >> "${TARGET_DIR}/bin/maven"
          chmod +x "${TARGET_DIR}/bin/maven"
        fi
      fi
