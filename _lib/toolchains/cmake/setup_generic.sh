...
CMAKE_INSTALL_METHOD="${CMAKE_INSTALL_METHOD:-system}"
CMAKE_INSTALL_METHOD="$(LIBSCRIPT_DEFAULT_INSTALL_METHOD="$CMAKE_INSTALL_METHOD" libscript_resolve_install_method "CMAKE")"
...
case "$ACTION" in
  ...
  use)
    if [ "$CMAKE_INSTALL_METHOD" = "mise" ]; then
      ...
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env cmake \"${CMAKE_VERSION}\")"
    fi
    exit 0
    ;;
  download)
    ...
    if [ "$CMAKE_INSTALL_METHOD" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/cmake/"*"${VERSION:-}"* >/dev/null 2>&1; then
        ...
        if [ -n "${CMAKE_DOWNLOAD_URL:-}" ]; then
          ...
          log_error "No download URL provided for cmake ${VERSION:-}."
          exit 1
        fi
      fi
