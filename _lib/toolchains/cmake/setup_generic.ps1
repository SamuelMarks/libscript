<#
.SYNOPSIS
Windows PowerShell setup stub for cmake
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cmake; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cmake; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cmake; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cmake"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cmake; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cmake; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cmake; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CMAKE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CMAKE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cmake@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cmake "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cmake@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cmake") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cmake") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cmake $CompVersion to $DownloadDir\cmake..."
        $CompDownloadDir = Join-Path $DownloadDir "cmake"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CMAKE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CMAKE_DOWNLOAD_URL -OutFile "$CompDownloadDir\cmake-$CompVersion.zip"
        } else {
            Write-Output "CMAKE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cmake@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cmake "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cmake@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cmake; vfox install "cmake@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cmake") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cmake $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cmake\cmake-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cmake\cmake-$CompVersion.tar.gz"
        ...
        } elseif ($env:CMAKE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cmake.zip"
            Invoke-WebRequest -Uri $env:CMAKE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for cmake."
            exit 1
        }
    } else {
        Write-Output "cmake $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cmake" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cmake $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cmake") $CompVersion
