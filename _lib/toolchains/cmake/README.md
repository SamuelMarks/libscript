...
| `CMAKE_VERSION` | Specific version of cmake to install. Can be a numeric version or an alias. | `latest` | latest, stable |
| `CMAKE_INSTALL_METHOD` | How to install CMAKE. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager, 'mise', 'asdf', 'pkgx', or 'vfox' defers to third-party tools. | `libscript_native` |  |
...
| `CMAKE_LISTEN_PORT` | Port for CMAKE to listen on | `none` |  |
| `CMAKE_LISTEN_ADDRESS` | Address for CMAKE to listen on | `none` |  |
| `CMAKE_LISTEN_SOCKET` | Unix socket for CMAKE to listen on | `none` |  |
<!-- END_VARS -->
...
Libscript manages cmake versions natively by installing them into isolated directories under
`LIBSCRIPT_HOME/cmake/<version>`.
