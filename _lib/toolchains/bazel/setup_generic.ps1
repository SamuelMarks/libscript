<#
.SYNOPSIS
Windows PowerShell setup stub for bazel
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls bazel; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list bazel; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls bazel; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "bazel"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote bazel; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all bazel; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all bazel; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:BAZEL_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:BAZEL_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "bazel@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global bazel "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "bazel@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/bazelbuild/bazel/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "bazel") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "bazel") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading bazel $CompVersion to $DownloadDir\bazel..."
        $CompDownloadDir = Join-Path $DownloadDir "bazel"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:BAZEL_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:BAZEL_DOWNLOAD_URL -OutFile "$CompDownloadDir\bazel-$CompVersion.zip"
        } else {
            Write-Output "BAZEL_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "bazel@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install bazel "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "bazel@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add bazel; vfox install "bazel@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "bazel") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing bazel $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\bazel\bazel-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\bazel\bazel-$CompVersion.tar.gz"
        ...
        } elseif ($env:BAZEL_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "bazel.zip"
            Invoke-WebRequest -Uri $env:BAZEL_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for bazel."
        }
    } else {
        Write-Output "bazel $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_bazel" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling bazel $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "bazel") $CompVersion
