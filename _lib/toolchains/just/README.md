...
| `JUST_INSTALL_METHOD` | How to install JUST. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager, 'mise', 'asdf', 'pkgx', or 'vfox' defers to third-party tools. | `libscript_native` |  |
...
`libscript` manages `just` versions natively by default (`JUST_INSTALL_METHOD=libscript_native`),
ensuring isolated installations without polluting global system paths. You can override this to use
`system`, `mise`, `asdf`, `pkgx`, or `vfox` if preferred.

Libscript manages just versions natively by installing them into isolated directories under
`LIBSCRIPT_HOME/just/<version>`.
