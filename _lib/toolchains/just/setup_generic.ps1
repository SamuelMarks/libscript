<#
.SYNOPSIS
Windows PowerShell setup stub for just
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls just; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list just; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls just; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "just"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote just; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all just; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all just; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:JUST_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:JUST_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "just@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global just "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "just@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/casey/just/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "just") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "just") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading just $CompVersion to $DownloadDir\just..."
        $CompDownloadDir = Join-Path $DownloadDir "just"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:JUST_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:JUST_DOWNLOAD_URL -OutFile "$CompDownloadDir\just-$CompVersion.zip"
        } else {
            Write-Output "JUST_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "just@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install just "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "just@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add just; vfox install "just@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "just") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing just $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\just\just-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\just\just-$CompVersion.tar.gz"
        ...
        } elseif ($env:JUST_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "just.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_just" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling just $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "just") $CompVersion
