...
<#
.SYNOPSIS
Windows PowerShell setup stub for gradle
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls gradle; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list gradle; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls gradle; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "gradle"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote gradle; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all gradle; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all gradle; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GRADLE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GRADLE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "gradle@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global gradle "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "gradle@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/gradle/gradle/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "gradle") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "gradle") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading gradle $CompVersion to $DownloadDir\gradle..."
        $CompDownloadDir = Join-Path $DownloadDir "gradle"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GRADLE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GRADLE_DOWNLOAD_URL -OutFile "$CompDownloadDir\gradle-$CompVersion.zip"
        } else {
            Write-Output "GRADLE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "gradle@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install gradle "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "gradle@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add gradle; vfox install "gradle@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "gradle") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing gradle $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\gradle\gradle-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\gradle\gradle-$CompVersion.tar.gz"
        ...
        } elseif ($env:GRADLE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "gradle.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_gradle" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling gradle $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "gradle") $CompVersion
