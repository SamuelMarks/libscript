...
<#
.SYNOPSIS
Windows PowerShell setup stub for coursier
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls coursier; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list coursier; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls coursier; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "coursier"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote coursier; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all coursier; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all coursier; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:COURSIER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:COURSIER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "coursier@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global coursier "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "coursier@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/coursier/coursier/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "coursier") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "coursier") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading coursier $CompVersion to $DownloadDir\coursier..."
        $CompDownloadDir = Join-Path $DownloadDir "coursier"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:COURSIER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:COURSIER_DOWNLOAD_URL -OutFile "$CompDownloadDir\coursier-$CompVersion.zip"
        } else {
            Write-Output "COURSIER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "coursier@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install coursier "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "coursier@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add coursier; vfox install "coursier@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "coursier") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing coursier $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\coursier\coursier-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\coursier\coursier-$CompVersion.tar.gz"
        ...
        } elseif ($env:COURSIER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "coursier.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_coursier" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling coursier $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "coursier") $CompVersion
