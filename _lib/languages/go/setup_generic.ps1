<#
.SYNOPSIS
Windows PowerShell setup stub for go
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls go; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list go; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls go; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "go"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote go; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all go; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all go; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GO_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GO_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "go@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global go "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "go@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "go") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "go") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading go $CompVersion to $DownloadDir\go..."
        $CompDownloadDir = Join-Path $DownloadDir "go"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GO_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GO_DOWNLOAD_URL -OutFile "$CompDownloadDir\go-$CompVersion.zip"
        } else {
            Write-Output "GO_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "go@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install go "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "go@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add go; vfox install "go@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "go") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing go $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\go\go-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\go\go-$CompVersion.tar.gz"
        ...
        } elseif ($env:GO_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "go.zip"
            Invoke-WebRequest -Uri $env:GO_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for go."
        }
    } else {
        Write-Output "go $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_go" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling go $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "go") $CompVersion
