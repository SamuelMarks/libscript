...
<#
.SYNOPSIS
Orchestrates the setup and installation process for the component 'cpp' stack.

.DESCRIPTION
Execute this script to install and configure cpp on the local system.
#>
...
if ($InstallMethod -eq "system" -and $WinPkgMgr -eq "winget") {
    ...
    Write-Host "From-source or alternative Windows package manager requested for cpp."
    winget install --silent --force --id=LLVM.LLVM -e --accept-package-agreements --accept-source-agreements
}
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cpp to $DownloadDir\cpp..."
    }
    exit 0
}
