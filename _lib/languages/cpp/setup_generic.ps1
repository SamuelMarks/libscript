<#
.SYNOPSIS
Windows PowerShell setup stub for cpp
...
$InstallMethod = $env:CPP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cpp; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cpp; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cpp; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cpp"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cpp; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cpp; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cpp; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CPP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CPP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cpp@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cpp "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cpp@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cpp") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cpp") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cpp $CompVersion to $DownloadDir\cpp..."
        $CompDownloadDir = Join-Path $DownloadDir "cpp"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CPP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CPP_DOWNLOAD_URL -OutFile "$CompDownloadDir\cpp-$CompVersion.zip"
        } else {
            Write-Output "CPP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cpp@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cpp "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cpp@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cpp; vfox install "cpp@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cpp") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cpp $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cpp\cpp-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cpp\cpp-$CompVersion.tar.gz"
        ...
        } elseif ($env:CPP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cpp.zip"
            Invoke-WebRequest -Uri $env:CPP_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for cpp."
        }
    } else {
        Write-Output "cpp $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cpp" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cpp $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cpp") $CompVersion
