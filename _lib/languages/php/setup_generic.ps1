...
<#
.SYNOPSIS
Windows PowerShell setup stub for php
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls php; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list php; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls php; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "php"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote php; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all php; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all php; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PHP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PHP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "php@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global php "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "php@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "php") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "php") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading php $CompVersion to $DownloadDir\php..."
        $CompDownloadDir = Join-Path $DownloadDir "php"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PHP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PHP_DOWNLOAD_URL -OutFile "$CompDownloadDir\php-$CompVersion.zip"
        } else {
            Write-Output "PHP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "php@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install php "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "php@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add php; vfox install "php@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "php") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing php $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\php\php-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\php\php-$CompVersion.tar.gz"
        ...
        } elseif ($env:PHP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "php.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_php" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling php $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "php") $CompVersion
