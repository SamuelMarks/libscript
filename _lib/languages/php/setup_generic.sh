  ...
  ls-remote)
    if [ "$PHP_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${PHP_RELEASES_URL:-}" ]; then
        curl -sSL "${PHP_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/php" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
    fi
    fi
    exit 0
    ;;
