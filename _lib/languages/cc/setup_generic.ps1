<#
.SYNOPSIS
Windows PowerShell setup stub for cc
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls cc; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list cc; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls cc; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "cc"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote cc; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all cc; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all cc; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CC_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CC_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "cc@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global cc "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "cc@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "cc") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "cc") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading cc $CompVersion to $DownloadDir\cc..."
        $CompDownloadDir = Join-Path $DownloadDir "cc"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CC_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CC_DOWNLOAD_URL -OutFile "$CompDownloadDir\cc-$CompVersion.zip"
        } else {
            Write-Output "CC_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "cc@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install cc "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "cc@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add cc; vfox install "cc@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "cc") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing cc $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\cc\cc-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\cc\cc-$CompVersion.tar.gz"
        ...
        } elseif ($env:CC_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "cc.zip"
            Invoke-WebRequest -Uri $env:CC_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for cc."
        }
    } else {
        Write-Output "cc $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_cc" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling cc $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "cc") $CompVersion
