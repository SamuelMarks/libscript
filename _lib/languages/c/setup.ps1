...
<#
.SYNOPSIS
Orchestrates the setup and installation process for the component 'c' stack.

.DESCRIPTION
Execute this script to install and configure c on the local system.
#>
...
if ($InstallMethod -eq "system" -and $WinPkgMgr -eq "winget") {
    ...
    Write-Host "From-source or alternative Windows package manager requested for c."
    winget install --silent --force --id=LLVM.LLVM -e --accept-package-agreements --accept-source-agreements
}
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading c to $DownloadDir\c..."
    }
    exit 0
}
