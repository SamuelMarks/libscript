<#
.SYNOPSIS
Windows PowerShell setup stub for kotlin
...
$CompVersion = $env:KOTLIN_VERSION
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls kotlin; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list kotlin; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls kotlin; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "kotlin"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote kotlin; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all kotlin; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all kotlin; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:KOTLIN_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:KOTLIN_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "kotlin@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global kotlin "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "kotlin@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "kotlin") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "kotlin") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading kotlin $CompVersion to $DownloadDir\kotlin..."
        $CompDownloadDir = Join-Path $DownloadDir "kotlin"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:KOTLIN_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:KOTLIN_DOWNLOAD_URL -OutFile "$CompDownloadDir\kotlin-$CompVersion.zip"
        } else {
            Write-Output "KOTLIN_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "kotlin@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install kotlin "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "kotlin@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add kotlin; vfox install "kotlin@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "kotlin") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing kotlin $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\kotlin\kotlin-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\kotlin\kotlin-$CompVersion.tar.gz"
        ...
        } elseif ($env:KOTLIN_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "kotlin.zip"
            Invoke-WebRequest -Uri $env:KOTLIN_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for kotlin."
            exit 1
        }
    } else {
        Write-Output "kotlin $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_kotlin" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling kotlin $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "kotlin") $CompVersion
