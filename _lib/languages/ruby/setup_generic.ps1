<#
.SYNOPSIS
Windows PowerShell setup stub for ruby
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls ruby; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list ruby; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls ruby; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "ruby"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote ruby; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all ruby; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all ruby; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:RUBY_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:RUBY_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "ruby@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global ruby "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "ruby@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "ruby") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "ruby") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading ruby $CompVersion to $DownloadDir\ruby..."
        $CompDownloadDir = Join-Path $DownloadDir "ruby"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:RUBY_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:RUBY_DOWNLOAD_URL -OutFile "$CompDownloadDir\ruby-$CompVersion.zip"
        } else {
            Write-Output "RUBY_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "ruby@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install ruby "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "ruby@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add ruby; vfox install "ruby@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "ruby") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing ruby $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\ruby\ruby-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\ruby\ruby-$CompVersion.tar.gz"
        ...
        } elseif ($env:RUBY_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "ruby.zip"
            Invoke-WebRequest -Uri $env:RUBY_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for ruby."
        }
    } else {
        Write-Output "ruby $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_ruby" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling ruby $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "ruby") $CompVersion
