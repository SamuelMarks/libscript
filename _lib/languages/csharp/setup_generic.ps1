...
<#
.SYNOPSIS
Windows PowerShell setup stub for csharp
...
$CompVersion = $env:CSHARP_VERSION
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls csharp; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list csharp; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls csharp; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "csharp"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote csharp; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all csharp; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all csharp; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CSHARP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CSHARP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "csharp@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global csharp "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "csharp@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "csharp") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "csharp") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading csharp $CompVersion to $DownloadDir\csharp..."
        $CompDownloadDir = Join-Path $DownloadDir "csharp"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CSHARP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CSHARP_DOWNLOAD_URL -OutFile "$CompDownloadDir\csharp-$CompVersion.zip"
        } else {
            Write-Output "CSHARP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "csharp@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install csharp "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "csharp@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add csharp; vfox install "csharp@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "csharp") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing csharp $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\csharp\csharp-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\csharp\csharp-$CompVersion.tar.gz"
        ...
        } elseif ($env:CSHARP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "csharp.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_csharp" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling csharp $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "csharp") $CompVersion
