<#
.SYNOPSIS
Windows PowerShell setup stub for deno
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls deno; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list deno; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls deno; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "deno"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote deno; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all deno; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all deno; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:DENO_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:DENO_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "deno@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global deno "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "deno@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "deno") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "deno") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading deno $CompVersion to $DownloadDir\deno..."
        $CompDownloadDir = Join-Path $DownloadDir "deno"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:DENO_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:DENO_DOWNLOAD_URL -OutFile "$CompDownloadDir\deno-$CompVersion.zip"
        } else {
            Write-Output "DENO_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "deno@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install deno "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "deno@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add deno; vfox install "deno@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "deno") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing deno $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\deno\deno-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\deno\deno-$CompVersion.tar.gz"
        ...
        } elseif ($env:DENO_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "deno.zip"
            Invoke-WebRequest -Uri $env:DENO_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for deno."
        }
    } else {
        Write-Output "deno $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_deno" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling deno $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "deno") $CompVersion
