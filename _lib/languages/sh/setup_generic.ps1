<#
.SYNOPSIS
Windows PowerShell setup stub for sh
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls sh; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list sh; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls sh; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "sh"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote sh; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all sh; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all sh; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SH_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SH_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "sh@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global sh "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "sh@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "sh") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "sh") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading sh $CompVersion to $DownloadDir\sh..."
        $CompDownloadDir = Join-Path $DownloadDir "sh"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SH_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SH_DOWNLOAD_URL -OutFile "$CompDownloadDir\sh-$CompVersion.zip"
        } else {
            Write-Output "SH_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "sh@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install sh "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "sh@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add sh; vfox install "sh@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "sh") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing sh $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\sh\sh-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\sh\sh-$CompVersion.tar.gz"
        ...
        } elseif ($env:SH_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "sh.zip"
            Invoke-WebRequest -Uri $env:SH_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for sh."
        }
    } else {
        Write-Output "sh $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_sh" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling sh $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "sh") $CompVersion
