...
<#
.SYNOPSIS
Windows PowerShell setup stub for swift
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls swift; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list swift; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls swift; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "swift"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote swift; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all swift; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all swift; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SWIFT_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SWIFT_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "swift@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global swift "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "swift@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "swift") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "swift") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading swift $CompVersion to $DownloadDir\swift..."
        $CompDownloadDir = Join-Path $DownloadDir "swift"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SWIFT_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SWIFT_DOWNLOAD_URL -OutFile "$CompDownloadDir\swift-$CompVersion.zip"
        } else {
            Write-Output "SWIFT_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "swift@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install swift "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "swift@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add swift; vfox install "swift@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "swift") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing swift $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\swift\swift-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\swift\swift-$CompVersion.tar.gz"
        ...
        } elseif ($env:SWIFT_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "swift.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_swift" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling swift $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "swift") $CompVersion
