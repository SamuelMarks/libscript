...
# 
# ## Usage
# Execute this script to perform generic initialization steps for java.
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$JAVA_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${JAVA_RELEASES_URL:-}" ]; then
        curl -sSL "${JAVA_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/java" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$JAVA_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "java" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/java/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "java ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "java" "default" "${EXACT_VERSION}"
      log_info "Set default java version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env java \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
      ...
      if [ ! -d "${TARGET_DIR}" ]; then
        ...
        if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/java/"*"${VERSION}"* >/dev/null 2>&1; then
          ...
          if [ -n "${JAVA_DOWNLOAD_URL:-}" ]; then
            ...
            log_error "No download URL provided for java ${VERSION}."
            exit 1
          fi
        fi
      else
