<#
.SYNOPSIS
Windows PowerShell setup stub for python
...
$CompVersion = $env:PYTHON_VERSION
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls python; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list python; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls python; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "python"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote python; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all python; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all python; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:PYTHON_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PYTHON_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "python@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global python "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "python@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "python") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "python") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading python $CompVersion to $DownloadDir\python..."
        $CompDownloadDir = Join-Path $DownloadDir "python"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PYTHON_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PYTHON_DOWNLOAD_URL -OutFile "$CompDownloadDir\python-$CompVersion.zip"
        } else {
            Write-Output "PYTHON_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "python@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install python "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "python@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add python; vfox install "python@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "python") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing python $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\python\python-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\python\python-$CompVersion.tar.gz"
        ...
        } elseif ($env:PYTHON_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "python.zip"
            Invoke-WebRequest -Uri $env:PYTHON_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for python."
        }
    } else {
        Write-Output "python $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_python" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling python $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "python") $CompVersion
