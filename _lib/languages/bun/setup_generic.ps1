<#
.SYNOPSIS
Windows PowerShell setup stub for bun
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls bun; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list bun; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls bun; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "bun"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote bun; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all bun; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all bun; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:BUN_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:BUN_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "bun@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global bun "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "bun@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "bun") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "bun") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading bun $CompVersion to $DownloadDir\bun..."
        $CompDownloadDir = Join-Path $DownloadDir "bun"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:BUN_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:BUN_DOWNLOAD_URL -OutFile "$CompDownloadDir\bun-$CompVersion.zip"
        } else {
            Write-Output "BUN_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "bun@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install bun "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "bun@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add bun; vfox install "bun@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "bun") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing bun $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\bun\bun-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\bun\bun-$CompVersion.tar.gz"
        ...
        } elseif ($env:BUN_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "bun.zip"
            Invoke-WebRequest -Uri $env:BUN_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for bun."
        }
    } else {
        Write-Output "bun $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_bun" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling bun $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "bun") $CompVersion
