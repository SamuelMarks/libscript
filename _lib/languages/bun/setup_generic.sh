...
# 
# ## Usage
# Execute this script to perform generic initialization steps for bun.
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$BUN_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${BUN_RELEASES_URL:-}" ]; then
        curl -sSL "${BUN_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/bun" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$BUN_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "bun" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/bun/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "bun ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "bun" "default" "${EXACT_VERSION}"
      log_info "Set default bun version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env bun \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
        ...
        if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/bun/"*"${VERSION}"* >/dev/null 2>&1; then
          ...
          if [ -n "${BUN_DOWNLOAD_URL:-}" ]; then
            ...
            if [ "${TARGET_OS:-}" = "alpine" ]; then
              libscript_depends "bash" "curl" "unzip" "libstdc++"
            else
            ...
            if [ "${VERSION}" = "latest" ]; then
              curl -fsSL https://bun.sh/install | "$_installer_sh" || true
            else
              curl -fsSL https://bun.sh/install | "$_installer_sh" -s "bun-v${VERSION}" || true
            fi
