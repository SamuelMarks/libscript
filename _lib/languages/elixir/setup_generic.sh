...
# 
# ## Usage
# Execute this script to perform generic initialization steps for elixir.
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$ELIXIR_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${ELIXIR_RELEASES_URL:-}" ]; then
        curl -sSL "${ELIXIR_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/elixir" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$ELIXIR_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "elixir" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/elixir/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "elixir ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "elixir" "default" "${EXACT_VERSION}"
      log_info "Set default elixir version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env elixir \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
        ...
        if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/elixir/"*"${VERSION}"* >/dev/null 2>&1; then
          ...
          if [ -n "${ELIXIR_DOWNLOAD_URL:-}" ]; then
            ...
            if [ "$UNAME_LOWER" = "linux" ] || [ "$UNAME_LOWER" = "freebsd" ] && [ -n "${PKG_MGR:-}" ]; then
              ...
              log_error "No download URL provided for elixir ${VERSION}."
              exit 1
            fi
          fi
        fi
