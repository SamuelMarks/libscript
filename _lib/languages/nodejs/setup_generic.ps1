<#
.SYNOPSIS
Windows PowerShell setup stub for nodejs
...
$CompVersion = $env:NODEJS_VERSION
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls nodejs; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list nodejs; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls nodejs; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "nodejs"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote nodejs; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all nodejs; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all nodejs; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:NODEJS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NODEJS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "nodejs@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global nodejs "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "nodejs@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "nodejs") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "nodejs") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading nodejs $CompVersion to $DownloadDir\nodejs..."
        $CompDownloadDir = Join-Path $DownloadDir "nodejs"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NODEJS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NODEJS_DOWNLOAD_URL -OutFile "$CompDownloadDir\nodejs-$CompVersion.zip"
        } else {
            Write-Output "NODEJS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "nodejs@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install nodejs "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "nodejs@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add nodejs; vfox install "nodejs@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "nodejs") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing nodejs $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\nodejs\nodejs-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\nodejs\nodejs-$CompVersion.tar.gz"
        ...
        } elseif ($env:NODEJS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "nodejs.zip"
            Invoke-WebRequest -Uri $env:NODEJS_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for nodejs."
        }
    } else {
        Write-Output "nodejs $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_nodejs" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling nodejs $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "nodejs") $CompVersion
