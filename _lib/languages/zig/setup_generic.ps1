<#
.SYNOPSIS
Windows PowerShell setup stub for zig
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls zig; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list zig; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls zig; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "zig"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote zig; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all zig; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all zig; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:ZIG_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:ZIG_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "zig@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global zig "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "zig@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "zig") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "zig") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading zig $CompVersion to $DownloadDir\zig..."
        $CompDownloadDir = Join-Path $DownloadDir "zig"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:ZIG_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:ZIG_DOWNLOAD_URL -OutFile "$CompDownloadDir\zig-$CompVersion.zip"
        } else {
            Write-Output "ZIG_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "zig@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install zig "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "zig@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add zig; vfox install "zig@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "zig") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing zig $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\zig\zig-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\zig\zig-$CompVersion.tar.gz"
        ...
        } elseif ($env:ZIG_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "zig.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_zig" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling zig $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "zig") $CompVersion
