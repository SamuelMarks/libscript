...
STAMP_FILE="${STAMPS_DIR}/.stamp.chrony"
...
if ! command -v chronyd >/dev/null 2>&1 && ! command -v chronyc >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache chrony || true
    ...
    priv apt-get install -y chrony || true
    ...
    priv dnf install -y chrony || true
  fi
fi
...
' "Chrony NTP" "$TARGET_SYSROOT" "$ACTION"
