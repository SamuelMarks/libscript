...
STAMP_FILE="${STAMPS_DIR}/.stamp.wpa_supplicant"
...
if ! command -v wpa_supplicant >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache wpa_supplicant || true
    ...
    priv apt-get install -y wpasupplicant || priv apt-get install -y wpa_supplicant || true
    ...
    priv dnf install -y wpa_supplicant || true
  fi
fi

if [ -f "$STAMP_FILE" ]; then
  printf '[SKIP]  %s already installed (%s)
' "wpa_supplicant" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "WPA Supplicant" "$TARGET_SYSROOT" "$ACTION"
