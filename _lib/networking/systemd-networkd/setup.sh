#!/bin/sh
# ## Overview
# Recipe for systemd-networkd: Network device manager integrated into systemd.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.systemd-networkd"
...
if ! command -v networkctl >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache systemd-networkd || true
    ...
    priv apt-get install --no-install-recommends -y systemd || true
    ...
    priv dnf install -y systemd-networkd || (priv dnf install -y epel-release && priv dnf install -y systemd-networkd) || true
  fi
fi

if [ -f "$STAMP_FILE" ]; then
  printf '[SKIP]  %s already installed (%s)
' "systemd-networkd" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "systemd-networkd" "$TARGET_SYSROOT" "$ACTION"
