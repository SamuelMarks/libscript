...
Network device manager integrated into systemd

## Usage

```sh
./libscript.sh install systemd-networkd
```
...
| `SYSTEMD_NETWORKD_VERSION`         | Specific version of systemd-networkd to install.                                                                  | `latest`                   |                  |
| `SYSTEMD_NETWORKD_INSTALL_METHOD`  | How to install SYSTEMD_NETWORKD. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager. | `system`                   |                  |
