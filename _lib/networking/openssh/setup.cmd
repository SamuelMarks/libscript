...
::
:: ## Overview
:: Recipe for OpenSSH Server: Hardened secure remote login and shell daemon.
...
if "%~1"=="--help" (
    ...
    echo Recipe for OpenSSH Server.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.openssh"
if exist "%STAMP_FILE%" (
    echo [SKIP]  openssh already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging OpenSSH Server into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    openssh staged successfully: %STAMP_FILE%
