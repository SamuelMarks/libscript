...
::
:: ## Overview
:: Recipe for Packet Filter (PF): FreeBSD stateful packet inspection firewall.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Packet Filter (PF).
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.pf"
if exist "%STAMP_FILE%" (
    echo [SKIP]  pf already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Packet Filter (PF) into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    pf staged successfully: %STAMP_FILE%
