...
::
:: ## Overview
:: Recipe for nftables: Netfilter project packet classification framework.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.nftables"
if exist "%STAMP_FILE%" (
    echo [SKIP]  nftables already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging nftables into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    nftables staged successfully: %STAMP_FILE%
