...
STAMP_FILE="${STAMPS_DIR}/.stamp.nftables"
...
if ! command -v nft >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache nftables || true
    ...
    priv apt-get install -y nftables || true
    ...
    priv dnf install -y nftables || true
  fi
fi
...
' "nftables" "$TARGET_SYSROOT" "$ACTION"
