...
STAMP_FILE="${STAMPS_DIR}/.stamp.iptables"
...
if ! command -v iptables >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache iptables || true
    ...
    priv apt-get install -y iptables || true
    ...
    priv dnf install -y iptables iptables-nft || true
  fi
fi
...
' "iptables" "$TARGET_SYSROOT" "$ACTION"
