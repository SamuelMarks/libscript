...
::
:: ## Overview
:: Recipe for iptables: Traditional Linux packet filter userland utility.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.iptables"
if exist "%STAMP_FILE%" (
    echo [SKIP]  iptables already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging iptables into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    iptables staged successfully: %STAMP_FILE%
