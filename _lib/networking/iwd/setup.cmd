...
::
:: ## Overview
:: Recipe for iwd: Intel Wireless Daemon for Linux.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.iwd"
if exist "%STAMP_FILE%" (
    echo [SKIP]  iwd already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging iwd into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    iwd staged successfully: %STAMP_FILE%
