...
STAMP_FILE="${STAMPS_DIR}/.stamp.iwd"
...
if ! command -v iwctl >/dev/null 2>&1 && ! command -v iwd >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache iwd || true
    ...
    priv apt-get install --no-install-recommends -y iwd || true
