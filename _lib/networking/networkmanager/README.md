...
Universal network configuration and management daemon

## Usage

```sh
./libscript.sh install networkmanager
```
...
| `NETWORKMANAGER_VERSION`           | Specific version of networkmanager to install.                                                                  | `latest`                   |                  |
| `NETWORKMANAGER_INSTALL_METHOD`    | How to install NETWORKMANAGER. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager. | `system`                   |                  |
