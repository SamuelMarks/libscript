...
STAMP_FILE="${STAMPS_DIR}/.stamp.networkmanager"
...
if ! command -v nmcli >/dev/null 2>&1 && ! command -v NetworkManager >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache networkmanager || true
    ...
    priv apt-get install --no-install-recommends -y network-manager || true
    ...
    priv dnf install -y NetworkManager || true
  fi
fi

if [ -f "$STAMP_FILE" ]; then
  printf '[SKIP]  %s already installed (%s)
' "networkmanager" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "NetworkManager" "$TARGET_SYSROOT" "$ACTION"
