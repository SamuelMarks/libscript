...
::
:: ## Overview
:: Test suite for the GCP cloud provider on Windows.
::
:: ## Usage
:: Automatically invoked by the test framework to validate GCP provisioning commands.
...
call "%~dp0cli.cmd" network delete test-network > "%temp%\gcp_guard_out.txt" 2>&1
if not errorlevel 1 (
    echo FAIL: Network delete should have failed due to missing tag
    exit /b 1
)
findstr /i "Refusing to modify" "%temp%\gcp_guard_out.txt" >nul
if errorlevel 1 ( echo FAIL: Guard output missing & exit /b 1 )

set "LIBSCRIPT_ALLOW_ANY_TAG_MANIPULATION=1"
call "%~dp0cli.cmd" network delete test-network > "%temp%\gcp_guard_out.txt" 2>&1
findstr /i "Proceeding due to override flag" "%temp%\gcp_guard_out.txt" >nul
