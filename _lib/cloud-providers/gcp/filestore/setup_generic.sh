#!/bin/sh
# ## Overview
# Generic setup and configuration script for GCP Filestore instances.
#
# ## Usage
# Execute this script to configure Cloud Filestore NFS shares on Google Cloud.
...
case "$ACTION" in
  ...
  use)
    if [ "${FILESTORE_INSTALL_METHOD:-}" = "system" ]; then
      ...
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env filestore \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
