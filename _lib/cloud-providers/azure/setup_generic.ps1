...
<#
.SYNOPSIS
Windows PowerShell setup stub for azure
...
$InstallMethod = $env:AZURE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls azure; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list azure; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls azure; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "azure"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote azure; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all azure; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all azure; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:AZURE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:AZURE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "azure@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global azure "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "azure@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "azure") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "azure") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading azure $CompVersion to $DownloadDir\azure..."
        $CompDownloadDir = Join-Path $DownloadDir "azure"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:AZURE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:AZURE_DOWNLOAD_URL -OutFile "$CompDownloadDir\azure-$CompVersion.zip"
        } else {
            Write-Output "AZURE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "azure@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install azure "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "azure@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add azure; vfox install "azure@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "azure") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing azure $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\azure\azure-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\azure\azure-$CompVersion.tar.gz"
        ...
        } elseif ($env:AZURE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "azure.zip"
            Invoke-WebRequest -Uri $env:AZURE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for azure."
            exit 1
        }
    } else {
        Write-Output "azure $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_azure" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling azure $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "azure") $CompVersion
