<#
.SYNOPSIS
Windows PowerShell setup stub for aws
...
$InstallMethod = $env:AWS_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls aws; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list aws; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls aws; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "aws"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote aws; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all aws; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all aws; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:AWS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:AWS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "aws@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global aws "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "aws@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "aws") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "aws") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading aws $CompVersion to $DownloadDir\aws..."
        $CompDownloadDir = Join-Path $DownloadDir "aws"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:AWS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:AWS_DOWNLOAD_URL -OutFile "$CompDownloadDir\aws-$CompVersion.zip"
        } else {
            Write-Output "AWS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "aws@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install aws "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "aws@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add aws; vfox install "aws@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "aws") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing aws $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\aws\aws-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\aws\aws-$CompVersion.tar.gz"
        ...
        } elseif ($env:AWS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "aws.zip"
            Invoke-WebRequest -Uri $env:AWS_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for aws."
        }
    } else {
        Write-Output "aws $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_aws" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling aws $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "aws") $CompVersion
