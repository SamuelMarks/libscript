  ...
  ls-remote)
    if [ "$KAFKA_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${KAFKA_RELEASES_URL:-}" ]; then
        curl -sSL "${KAFKA_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/kafka" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || echo "No versions found"
    fi
    fi
    exit 0
    ;;
