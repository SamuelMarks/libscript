<#
.SYNOPSIS
Windows PowerShell setup stub for kafka
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls kafka; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list kafka; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls kafka; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "kafka"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote kafka; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all kafka; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all kafka; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:KAFKA_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:KAFKA_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "kafka@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global kafka "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "kafka@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "kafka") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "kafka") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading kafka $CompVersion to $DownloadDir\kafka..."
        $CompDownloadDir = Join-Path $DownloadDir "kafka"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:KAFKA_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:KAFKA_DOWNLOAD_URL -OutFile "$CompDownloadDir\kafka-$CompVersion.zip"
        } else {
            Write-Output "KAFKA_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "kafka@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install kafka "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "kafka@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add kafka; vfox install "kafka@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "kafka") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing kafka $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\kafka\kafka-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\kafka\kafka-$CompVersion.tar.gz"
        ...
        } elseif ($env:KAFKA_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "kafka.zip"
            Invoke-WebRequest -Uri $env:KAFKA_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for kafka."
        }
    } else {
        Write-Output "kafka $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_kafka" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling kafka $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "kafka") $CompVersion
