<#
.SYNOPSIS
Windows PowerShell setup stub for rabbitmq
...
$InstallMethod = $env:RABBITMQ_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls rabbitmq; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list rabbitmq; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls rabbitmq; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "rabbitmq"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote rabbitmq; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all rabbitmq; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all rabbitmq; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:RABBITMQ_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:RABBITMQ_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "rabbitmq@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global rabbitmq "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "rabbitmq@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "rabbitmq") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "rabbitmq") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading rabbitmq $CompVersion to $DownloadDir\rabbitmq..."
        $CompDownloadDir = Join-Path $DownloadDir "rabbitmq"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:RABBITMQ_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:RABBITMQ_DOWNLOAD_URL -OutFile "$CompDownloadDir\rabbitmq-$CompVersion.zip"
        } else {
            Write-Output "RABBITMQ_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "rabbitmq@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install rabbitmq "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "rabbitmq@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add rabbitmq; vfox install "rabbitmq@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "rabbitmq") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing rabbitmq $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\rabbitmq\rabbitmq-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\rabbitmq\rabbitmq-$CompVersion.tar.gz"
        ...
        } elseif ($env:RABBITMQ_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "rabbitmq.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_rabbitmq" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling rabbitmq $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "rabbitmq") $CompVersion
