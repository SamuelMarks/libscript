...
# 
# ## Usage
# Execute this script to perform generic initialization steps for fluentbit.
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$FLUENTBIT_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${FLUENTBIT_RELEASES_URL:-}" ]; then
        curl -sSL "${FLUENTBIT_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/fluent/fluent-bit" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$FLUENTBIT_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "fluentbit" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/fluentbit/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "fluentbit ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "fluentbit" "default" "${EXACT_VERSION}"
      log_info "Set default fluentbit version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env fluentbit \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
      ...
      if [ ! -d "${TARGET_DIR}" ]; then
        ...
        if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/fluentbit/"*"${VERSION}"* >/dev/null 2>&1; then
          ...
          if [ -n "${FLUENTBIT_DOWNLOAD_URL:-}" ]; then
            ...
            log_info "No download URL provided for fluentbit ${VERSION}. Attempting to use install script fallback..."
            libscript_depends "curl" "sh"
            curl -fsSL https://raw.githubusercontent.com/fluent/fluent-bit/master/install.sh | sh || true
          fi
        fi
      else
