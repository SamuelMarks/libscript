<#
.SYNOPSIS
Windows PowerShell setup stub for tensorboard
...
$CompVersion = $env:TENSORBOARD_VERSION
...
$InstallMethod = $env:TENSORBOARD_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list tensorboard; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all tensorboard; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:TENSORBOARD_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:TENSORBOARD_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global tensorboard "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "tensorboard") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "tensorboard") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading tensorboard $CompVersion to $DownloadDir\tensorboard..."
        $CompDownloadDir = Join-Path $DownloadDir "tensorboard"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:TENSORBOARD_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:TENSORBOARD_DOWNLOAD_URL -OutFile "$CompDownloadDir\tensorboard-$CompVersion.zip"
        } else {
            Write-Output "TENSORBOARD_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "tensorboard@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install tensorboard "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "tensorboard@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add tensorboard; vfox install "tensorboard@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing tensorboard $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\tensorboard\tensorboard-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\tensorboard\tensorboard-$CompVersion.tar.gz"
        ...
        } elseif ($env:TENSORBOARD_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "tensorboard.zip"
            Invoke-WebRequest -Uri $env:TENSORBOARD_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for tensorboard."
        }
    } else {
        Write-Output "tensorboard $CompVersion is already installed."
    }
