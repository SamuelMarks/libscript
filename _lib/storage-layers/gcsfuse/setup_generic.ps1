...
<#
.SYNOPSIS
Windows PowerShell setup stub for gcsfuse
...
$CompVersion = $env:GCSFUSE_VERSION
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list gcsfuse; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all gcsfuse; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:GCSFUSE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GCSFUSE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global gcsfuse "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "gcsfuse") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "gcsfuse") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading gcsfuse $CompVersion to $DownloadDir\gcsfuse..."
        $CompDownloadDir = Join-Path $DownloadDir "gcsfuse"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GCSFUSE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GCSFUSE_DOWNLOAD_URL -OutFile "$CompDownloadDir\gcsfuse-$CompVersion.zip"
        } else {
            Write-Output "GCSFUSE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "gcsfuse@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install gcsfuse "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "gcsfuse@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add gcsfuse; vfox install "gcsfuse@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing gcsfuse $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\gcsfuse\gcsfuse-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\gcsfuse\gcsfuse-$CompVersion.tar.gz"
        ...
        } elseif ($env:GCSFUSE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "gcsfuse.zip"
            Invoke-WebRequest -Uri $env:GCSFUSE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for gcsfuse."
            exit 1
        }
    } else {
        Write-Output "gcsfuse $CompVersion is already installed."
    }
