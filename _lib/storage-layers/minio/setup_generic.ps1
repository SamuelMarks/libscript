...
<#
.SYNOPSIS
Windows PowerShell setup stub for minio
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls minio; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list minio; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls minio; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "minio"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote minio; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all minio; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all minio; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MINIO_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MINIO_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "minio@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global minio "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "minio@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "minio") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "minio") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading minio $CompVersion to $DownloadDir\minio..."
        $CompDownloadDir = Join-Path $DownloadDir "minio"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MINIO_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MINIO_DOWNLOAD_URL -OutFile "$CompDownloadDir\minio-$CompVersion.zip"
        } else {
            Write-Output "MINIO_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "minio@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install minio "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "minio@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add minio; vfox install "minio@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "minio") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing minio $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\minio\minio-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\minio\minio-$CompVersion.tar.gz"
        ...
        } elseif ($env:MINIO_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "minio.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_minio" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling minio $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "minio") $CompVersion
