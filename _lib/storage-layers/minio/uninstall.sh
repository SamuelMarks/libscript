...
if [ -n "${INSTALLED_DIR:-}" ]; then
  if [ -d "${INSTALLED_DIR}" ]; then
    ...
    echo "No local installation directory found for minio at ${INSTALLED_DIR}."
  fi
else
  echo "Uninstalling minio is not supported via this script (or INSTALLED_DIR not set)."
fi
