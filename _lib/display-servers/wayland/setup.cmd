...
::
:: ## Overview
:: Recipe for Wayland Protocol: Core Wayland display server library and protocols.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Wayland Protocol.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.wayland"
if exist "%STAMP_FILE%" (
    echo [SKIP]  wayland already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Wayland Protocol into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    wayland staged successfully: %STAMP_FILE%
