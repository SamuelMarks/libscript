...
::
:: ## Overview
:: Recipe for libseat: Universal seat management client library.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.libseat"
if exist "%STAMP_FILE%" (
    echo [SKIP]  libseat already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging libseat into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    libseat staged successfully: %STAMP_FILE%
