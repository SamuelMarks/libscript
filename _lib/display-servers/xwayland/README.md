...
X11 server running as a Wayland client

## Usage

```sh
./libscript.sh install xwayland
```
...
| `XWAYLAND_VERSION`                 | Specific version of xwayland to install.                                                                  | `latest`                   |                  |
| `XWAYLAND_INSTALL_METHOD`          | How to install XWAYLAND. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager. | `system`                   |                  |
