...
::
:: ## Overview
:: Recipe for Xwayland: X11 server running as a Wayland client.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.xwayland"
if exist "%STAMP_FILE%" (
    echo [SKIP]  xwayland already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Xwayland into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    xwayland staged successfully: %STAMP_FILE%
