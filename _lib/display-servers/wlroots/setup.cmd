...
::
:: ## Overview
:: Recipe for wlroots: Modular Wayland compositor library.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.wlroots"
if exist "%STAMP_FILE%" (
    echo [SKIP]  wlroots already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging wlroots into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    wlroots staged successfully: %STAMP_FILE%
