#!/bin/sh
# ## Overview
# Recipe for libxkbcommon: Keyboard description parser and keymap library.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.libxkbcommon"

if [ ! -d "$STAMPS_DIR" ]; then
  mkdir -p "$STAMPS_DIR"
fi

if [ ! -f /usr/lib/libxkbcommon.so.0 ] && [ ! -f /usr/lib64/libxkbcommon.so.0 ] && [ ! -f /lib/libxkbcommon.so.0 ]; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache libxkbcommon || true
    ...
    priv apt-get install -y libxkbcommon0 || priv apt-get install -y libxkbcommon || true
...
' "libxkbcommon" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "libxkbcommon" "$TARGET_SYSROOT" "$ACTION"
