...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.libxkbcommon"
if exist "%STAMP_FILE%" (
    echo [SKIP]  libxkbcommon already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging libxkbcommon into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    libxkbcommon staged successfully: %STAMP_FILE%
