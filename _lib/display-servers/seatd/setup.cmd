...
::
:: ## Overview
:: Recipe for seatd: Minimal seat management daemon and library.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.seatd"
if exist "%STAMP_FILE%" (
    echo [SKIP]  seatd already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging seatd into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    seatd staged successfully: %STAMP_FILE%
