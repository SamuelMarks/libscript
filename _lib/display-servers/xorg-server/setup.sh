#!/bin/sh
# ## Overview
# Recipe for X.Org Server: Traditional X Window System display server.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.xorg-server"
...
if ! command -v Xorg >/dev/null 2>&1 && ! command -v X >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache xorg-server || true
    ...
    priv apt-get install --no-install-recommends -y xserver-xorg-core || true
    ...
    priv dnf install -y xorg-x11-server-Xorg || true
  fi
fi

if [ -f "$STAMP_FILE" ]; then
  printf '[SKIP]  %s already installed (%s)
' "xorg-server" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "X.Org Server" "$TARGET_SYSROOT" "$ACTION"
