...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.xorg-server"
if exist "%STAMP_FILE%" (
    echo [SKIP]  xorg-server already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging X.Org Server into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    xorg-server staged successfully: %STAMP_FILE%
