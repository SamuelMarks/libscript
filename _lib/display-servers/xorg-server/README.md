...
Traditional X Window System display server

## Usage

```sh
./libscript.sh install xorg-server
```
...
| `XORG_SERVER_VERSION`              | Specific version of xorg-server to install.                                                                  | `latest`                   |                  |
| `XORG_SERVER_INSTALL_METHOD`       | How to install XORG_SERVER. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager. | `system`                   |                  |
