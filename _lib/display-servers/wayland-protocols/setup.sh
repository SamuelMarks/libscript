...
STAMP_FILE="${STAMPS_DIR}/.stamp.wayland-protocols"
...
if [ ! -d /usr/share/wayland-protocols ]; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache wayland-protocols || true
    ...
    priv apt-get install -y wayland-protocols || true
...
' "wayland-protocols" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "Wayland Protocols" "$TARGET_SYSROOT" "$ACTION"
