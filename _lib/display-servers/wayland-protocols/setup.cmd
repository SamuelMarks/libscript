...
::
:: ## Overview
:: Recipe for Wayland Protocols: Official protocol extensions for Wayland.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Wayland Protocols.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.wayland-protocols"
if exist "%STAMP_FILE%" (
    echo [SKIP]  wayland-protocols already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Wayland Protocols into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    wayland-protocols staged successfully: %STAMP_FILE%
