...
Official protocol extensions for Wayland

## Usage

```sh
./libscript.sh install wayland-protocols
```
...
| `WAYLAND_PROTOCOLS_VERSION`        | Specific version of wayland-protocols to install.                                                                  | `latest`                   |                  |
| `WAYLAND_PROTOCOLS_INSTALL_METHOD` | How to install WAYLAND_PROTOCOLS. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager. | `system`                   |                  |
