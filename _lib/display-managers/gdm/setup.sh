...
STAMP_FILE="${STAMPS_DIR}/.stamp.gdm"
...
if ! command -v gdm >/dev/null 2>&1 && ! command -v gdm3 >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache gdm || true
    ...
    priv apt-get install --no-install-recommends -y gdm3 || true
