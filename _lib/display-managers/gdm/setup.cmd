...
::
:: ## Overview
:: Recipe for GDM: GNOME Display Manager daemon.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.gdm"
if exist "%STAMP_FILE%" (
    echo [SKIP]  gdm already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging GDM into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    gdm staged successfully: %STAMP_FILE%
