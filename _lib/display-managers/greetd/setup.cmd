...
::
:: ## Overview
:: Recipe for greetd: Minimal, generic and agnostic login daemon.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.greetd"
if exist "%STAMP_FILE%" (
    echo [SKIP]  greetd already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging greetd into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    greetd staged successfully: %STAMP_FILE%
