...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.lightdm"
if exist "%STAMP_FILE%" (
    echo [SKIP]  lightdm already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging LightDM into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    lightdm staged successfully: %STAMP_FILE%
