...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.sddm"
if exist "%STAMP_FILE%" (
    echo [SKIP]  sddm already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging SDDM into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    sddm staged successfully: %STAMP_FILE%
