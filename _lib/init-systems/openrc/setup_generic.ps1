...
<#
.SYNOPSIS
Windows PowerShell setup stub for openrc
...
$InstallMethod = $env:OPENRC_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls openrc; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list openrc; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls openrc; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "openrc"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote openrc; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all openrc; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all openrc; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:OPENRC_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:OPENRC_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "openrc@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global openrc "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "openrc@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "openrc") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "openrc") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading openrc $CompVersion to $DownloadDir\openrc..."
        $CompDownloadDir = Join-Path $DownloadDir "openrc"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:OPENRC_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:OPENRC_DOWNLOAD_URL -OutFile "$CompDownloadDir\openrc-$CompVersion.zip"
        } else {
            Write-Output "OPENRC_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "openrc@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install openrc "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "openrc@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add openrc; vfox install "openrc@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "openrc") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing openrc $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\openrc\openrc-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\openrc\openrc-$CompVersion.tar.gz"
        ...
        } elseif ($env:OPENRC_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "openrc.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_openrc" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling openrc $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "openrc") $CompVersion
