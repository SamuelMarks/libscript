<#
.SYNOPSIS
Windows PowerShell setup stub for etcd
...
$InstallMethod = $env:ETCD_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls etcd; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list etcd; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls etcd; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "etcd"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote etcd; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all etcd; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all etcd; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:ETCD_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:ETCD_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "etcd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global etcd "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "etcd@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "etcd") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "etcd") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading etcd $CompVersion to $DownloadDir\etcd..."
        $CompDownloadDir = Join-Path $DownloadDir "etcd"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:ETCD_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:ETCD_DOWNLOAD_URL -OutFile "$CompDownloadDir\etcd-$CompVersion.zip"
        } else {
            Write-Output "ETCD_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "etcd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install etcd "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "etcd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add etcd; vfox install "etcd@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "etcd") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing etcd $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\etcd\etcd-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\etcd\etcd-$CompVersion.tar.gz"
        ...
        } elseif ($env:ETCD_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "etcd.zip"
            Invoke-WebRequest -Uri $env:ETCD_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for etcd."
        }
    } else {
        Write-Output "etcd $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_etcd" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling etcd $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "etcd") $CompVersion
