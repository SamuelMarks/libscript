<#
.SYNOPSIS
Windows PowerShell setup stub for mariadb
...
$CompVersion = $env:MARIADB_VERSION
...
$InstallMethod = $env:MARIADB_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls mariadb; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list mariadb; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls mariadb; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "mariadb"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote mariadb; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all mariadb; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all mariadb; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:MARIADB_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:MARIADB_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "mariadb@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global mariadb "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "mariadb@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "mariadb") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "mariadb") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading mariadb $CompVersion to $DownloadDir\mariadb..."
        $CompDownloadDir = Join-Path $DownloadDir "mariadb"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:MARIADB_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:MARIADB_DOWNLOAD_URL -OutFile "$CompDownloadDir\mariadb-$CompVersion.zip"
        } else {
            Write-Output "MARIADB_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "mariadb@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install mariadb "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "mariadb@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add mariadb; vfox install "mariadb@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "mariadb") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing mariadb $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\mariadb\mariadb-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\mariadb\mariadb-$CompVersion.tar.gz"
        ...
        } elseif ($env:MARIADB_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "mariadb.zip"
            Invoke-WebRequest -Uri $env:MARIADB_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for mariadb."
        }
    } else {
        Write-Output "mariadb $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_mariadb" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling mariadb $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "mariadb") $CompVersion
