...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$MARIADB_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${MARIADB_RELEASES_URL:-}" ]; then
        curl -sSL "${MARIADB_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/mariadb" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$MARIADB_INSTALL_METHOD" = "mise" ]; then
      ...
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env mariadb \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
