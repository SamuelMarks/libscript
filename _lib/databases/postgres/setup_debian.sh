#!/bin/sh
# ## Overview
# Debian-specific setup script for PostgreSQL.
#
# ## Usage
# Installs PostgreSQL from the official `apt.postgresql.org` repository and manages sysvinit.
...
if [ -n "${PID}" ]; then
  priv kill "${PID}"
  priv rm -f -- '/var/run/postgresql/'"${POSTGRES_VERSION}"'-main.pid'
