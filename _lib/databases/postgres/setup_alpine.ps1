# ## Overview
# PowerShell script for setup_alpine.ps1.
#
# ## Usage
...
switch ($Action) {
    ...
    "start" {
        ...
        $ServiceName = if ($env:LIBSCRIPT_SERVICE_NAME) { $env:LIBSCRIPT_SERVICE_NAME } else { "libscript_postgres" }
    ...
    "uninstall" {
        if ($InstallMethod -eq "libscript_native") {
            ...
            Write-Host "Uninstalling postgres `$Exact..."
            ...
            $TargetDir = Join-Path $LibscriptHome "postgres\`$Exact"
