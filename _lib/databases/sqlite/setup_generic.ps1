<#
.SYNOPSIS
Windows PowerShell setup stub for sqlite
...
$InstallMethod = $env:SQLITE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls sqlite; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list sqlite; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls sqlite; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "sqlite"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote sqlite; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all sqlite; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all sqlite; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SQLITE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SQLITE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "sqlite@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global sqlite "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "sqlite@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "sqlite") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "sqlite") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading sqlite $CompVersion to $DownloadDir\sqlite..."
        $CompDownloadDir = Join-Path $DownloadDir "sqlite"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SQLITE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SQLITE_DOWNLOAD_URL -OutFile "$CompDownloadDir\sqlite-$CompVersion.zip"
        } else {
            Write-Output "SQLITE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "sqlite@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install sqlite "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "sqlite@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add sqlite; vfox install "sqlite@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "sqlite") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing sqlite $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\sqlite\sqlite-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\sqlite\sqlite-$CompVersion.tar.gz"
        ...
        } elseif ($env:SQLITE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "sqlite.zip"
            Invoke-WebRequest -Uri $env:SQLITE_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for sqlite."
        }
    } else {
        Write-Output "sqlite $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_sqlite" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling sqlite $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "sqlite") $CompVersion
