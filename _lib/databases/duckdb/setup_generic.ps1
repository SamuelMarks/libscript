<#
.SYNOPSIS
Windows PowerShell setup stub for duckdb
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list duckdb; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all duckdb; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:DUCKDB_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:DUCKDB_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global duckdb "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "duckdb") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "duckdb") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading duckdb $CompVersion to $DownloadDir\duckdb..."
        $CompDownloadDir = Join-Path $DownloadDir "duckdb"
        ...
        if ($env:DUCKDB_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:DUCKDB_DOWNLOAD_URL -OutFile "$CompDownloadDir\duckdb-$CompVersion.zip"
        } else {
            Write-Output "DUCKDB_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "duckdb@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install duckdb "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "duckdb@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add duckdb; vfox install "duckdb@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing duckdb $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\duckdb\duckdb-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\duckdb\duckdb-$CompVersion.tar.gz"
        ...
        } elseif ($env:DUCKDB_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "duckdb.zip"
            Invoke-WebRequest -Uri $env:DUCKDB_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for duckdb."
        }
    } else {
        Write-Output "duckdb $CompVersion is already installed."
    }
