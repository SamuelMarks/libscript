<#
.SYNOPSIS
Windows PowerShell setup stub for kubernetes-k0s
...
$InstallMethod = $env:KUBERNETES_K0S_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls kubernetes-k0s; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list kubernetes-k0s; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls kubernetes-k0s; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "kubernetes-k0s"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote kubernetes-k0s; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all kubernetes-k0s; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all kubernetes-k0s; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:KUBERNETES_K0S_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:KUBERNETES_K0S_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "kubernetes-k0s@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global kubernetes-k0s "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "kubernetes-k0s@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "kubernetes-k0s") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "kubernetes-k0s") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading kubernetes-k0s $CompVersion to $DownloadDir\kubernetes-k0s..."
        $CompDownloadDir = Join-Path $DownloadDir "kubernetes-k0s"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:KUBERNETES_K0S_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:KUBERNETES_K0S_DOWNLOAD_URL -OutFile "$CompDownloadDir\kubernetes-k0s-$CompVersion.zip"
        } else {
            Write-Output "KUBERNETES_K0S_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "kubernetes-k0s@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install kubernetes-k0s "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "kubernetes-k0s@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add kubernetes-k0s; vfox install "kubernetes-k0s@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "kubernetes-k0s") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing kubernetes-k0s $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\kubernetes-k0s\kubernetes-k0s-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\kubernetes-k0s\kubernetes-k0s-$CompVersion.tar.gz"
        ...
        } elseif ($env:KUBERNETES_K0S_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "kubernetes-k0s.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_kubernetes-k0s" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling kubernetes-k0s $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "kubernetes-k0s") $CompVersion
