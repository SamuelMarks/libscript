<#
.SYNOPSIS
Windows PowerShell setup stub for docker
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls docker; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list docker; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls docker; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "docker"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote docker; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all docker; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all docker; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:DOCKER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:DOCKER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "docker@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global docker "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "docker@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "docker") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "docker") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading docker $CompVersion to $DownloadDir\docker..."
        $CompDownloadDir = Join-Path $DownloadDir "docker"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:DOCKER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:DOCKER_DOWNLOAD_URL -OutFile "$CompDownloadDir\docker-$CompVersion.zip"
        } else {
            Write-Output "DOCKER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "docker@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install docker "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "docker@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add docker; vfox install "docker@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "docker") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing docker $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\docker\docker-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\docker\docker-$CompVersion.tar.gz"
        ...
        } elseif ($env:DOCKER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "docker.zip"
            Invoke-WebRequest -Uri $env:DOCKER_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Error "No download URL or cache available for docker."
            exit 1
        }
    } else {
        Write-Output "docker $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_docker" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling docker $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "docker") $CompVersion
