#!/bin/sh
# ## Overview
# Generic setup and installation script for the Kubernetes kubectl CLI.
#
# ## Usage
# Execute this script to download and install kubectl on the host system.
...
case "$ACTION" in
  ls)
    ...
    ;;
  ls-remote)
    git ls-remote --tags "https://github.com/kubernetes/kubectl" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    exit 0
    ;;
  use)
    if [ "${KUBECTL_INSTALL_METHOD:-}" = "system" ]; then
      ...
      libscript_symlink_alias "kubectl" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/kubectl/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "kubectl ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "kubectl" "default" "${EXACT_VERSION}"
      log_info "Set default kubectl version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env kubectl \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
