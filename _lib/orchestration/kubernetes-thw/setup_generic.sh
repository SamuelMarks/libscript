...
case "$ACTION" in
  ...
  ls-remote)
    if [ "$KUBERNETES_THW_INSTALL_METHOD" = "mise" ]; then
      ...
      if [ -n "${KUBERNETES_THW_RELEASES_URL:-}" ]; then
        curl -sSL "${KUBERNETES_THW_RELEASES_URL}" | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
      else
      git ls-remote --tags "https://github.com/libscript/kubernetes-thw" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | sort -V | uniq || printf '%s\n' "No versions found"
    fi
    fi
    exit 0
    ;;
  use)
    if [ "$KUBERNETES_THW_INSTALL_METHOD" = "mise" ]; then
      ...
      libscript_symlink_alias "kubernetes-thw" "default" "${EXACT_VERSION}"
      
      TARGET_DIR="${LIBSCRIPT_HOME:-$HOME/.libscript}/kubernetes-thw/${EXACT_VERSION}"
      if [ ! -d "$TARGET_DIR" ]; then
        log_info "kubernetes-thw ${EXACT_VERSION} is not installed. Installing it now..."
        unset SCRIPT_NAME || true
        ACTION="install" sh "$DIR/setup.sh" install "$PACKAGE_NAME" "" || exit 1
      fi

      libscript_symlink_alias "kubernetes-thw" "default" "${EXACT_VERSION}"
      log_info "Set default kubernetes-thw version to ${EXACT_VERSION}."
      log_info "To apply to the current shell, run:"
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env kubernetes-thw \"$VERSION\")"
    fi
    exit 0
    ;;
  download)
