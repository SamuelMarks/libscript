<#
.SYNOPSIS
Windows PowerShell setup stub for kubernetes-thw
...
$InstallMethod = $env:KUBERNETES_THW_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls kubernetes-thw; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list kubernetes-thw; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls kubernetes-thw; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "kubernetes-thw"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote kubernetes-thw; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all kubernetes-thw; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all kubernetes-thw; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:KUBERNETES_THW_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:KUBERNETES_THW_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "kubernetes-thw@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global kubernetes-thw "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "kubernetes-thw@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "kubernetes-thw") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "kubernetes-thw") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading kubernetes-thw $CompVersion to $DownloadDir\kubernetes-thw..."
        $CompDownloadDir = Join-Path $DownloadDir "kubernetes-thw"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:KUBERNETES_THW_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:KUBERNETES_THW_DOWNLOAD_URL -OutFile "$CompDownloadDir\kubernetes-thw-$CompVersion.zip"
        } else {
            Write-Output "KUBERNETES_THW_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "kubernetes-thw@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install kubernetes-thw "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "kubernetes-thw@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add kubernetes-thw; vfox install "kubernetes-thw@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "kubernetes-thw") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing kubernetes-thw $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\kubernetes-thw\kubernetes-thw-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\kubernetes-thw\kubernetes-thw-$CompVersion.tar.gz"
        ...
        } elseif ($env:KUBERNETES_THW_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "kubernetes-thw.zip"
            Invoke-WebRequest -Uri $env:KUBERNETES_THW_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for kubernetes-thw."
        }
    } else {
        Write-Output "kubernetes-thw $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_kubernetes-thw" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling kubernetes-thw $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "kubernetes-thw") $CompVersion
