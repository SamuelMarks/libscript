...
# If multi-node Vagrant testing is requested via VAGRANT_MULTI_NODE=1, run full multi-node test
if [ "${VAGRANT_MULTI_NODE:-0}" = "1" ]; then
...
EOF'" < /dev/null
      ...
      if [ "$VAGRANT_IMAGE_DIR" = "k8s-thw-alpine" ]; then
        vagrant ssh "${VAGRANT_IMAGE_DIR}${MACH_IDX}" -c "sudo sh -c 'cat > /usr/local/bin/systemctl && chmod +x /usr/local/bin/systemctl'" < "${SCRIPT_DIR}/alpine-systemctl.sh"
      fi
    )
  done < "${MACHINES_TXT}"
...
# Verify core Kubernetes binaries and etcd controller state
if command -v kubectl >/dev/null 2>&1; then
  kubectl version --client=true
fi

if [ -x "${SCRIPT_DIR}/kubernetes-the-hard-way/downloads/client/kubectl" ]; then
  "${SCRIPT_DIR}/kubernetes-the-hard-way/downloads/client/kubectl" version --client=true
fi

if [ -x "${SCRIPT_DIR}/kubernetes-the-hard-way/downloads/client/etcdctl" ]; then
  "${SCRIPT_DIR}/kubernetes-the-hard-way/downloads/client/etcdctl" version
fi
