# ## Overview
# PowerShell script for setup_debian.ps1.
#
# ## Usage
...
switch ($Action) {
    ...
    "start" {
        ...
        $ServiceName = if ($env:LIBSCRIPT_SERVICE_NAME) { $env:LIBSCRIPT_SERVICE_NAME } else { "libscript_kubernetes-thw" }
    ...
    "uninstall" {
        if ($InstallMethod -eq "libscript_native") {
            ...
            Write-Host "Uninstalling kubernetes-thw `$Exact..."
            ...
            $TargetDir = Join-Path $LibscriptHome "kubernetes-thw\`$Exact"
    ...
    "download" {
        if ($InstallMethod -eq "libscript_native") {
            Write-Host "Downloading kubernetes-thw..."
        }
        break
    }
    default {
