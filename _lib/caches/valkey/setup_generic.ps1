...
$InstallMethod = $env:VALKEY_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "vfox") { vfox ls valkey; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "valkey"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote valkey; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all valkey; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all valkey; exit 0 }
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "valkey@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global valkey "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "valkey@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "valkey") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "valkey") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading valkey $CompVersion to $DownloadDir\valkey..."
        $CompDownloadDir = Join-Path $DownloadDir "valkey"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:VALKEY_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:VALKEY_DOWNLOAD_URL -OutFile "$CompDownloadDir\valkey-$CompVersion.zip"
        } else {
            Write-Output "VALKEY_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "valkey@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install valkey "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "valkey@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add valkey; vfox install "valkey@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "valkey") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing valkey $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\valkey\valkey-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\valkey\valkey-$CompVersion.tar.gz"
        ...
        } elseif ($env:VALKEY_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "valkey.zip"
            Invoke-WebRequest -Uri $env:VALKEY_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for valkey."
        }
    } else {
        Write-Output "valkey $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_valkey" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling valkey $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "valkey") $CompVersion
