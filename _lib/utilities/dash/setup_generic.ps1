<#
.SYNOPSIS
Windows PowerShell setup stub for dash
...
$InstallMethod = $env:DASH_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls dash; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list dash; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls dash; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "dash"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote dash; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all dash; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all dash; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:DASH_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:DASH_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "dash@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global dash "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "dash@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "dash") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "dash") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading dash $CompVersion to $DownloadDir\dash..."
        $CompDownloadDir = Join-Path $DownloadDir "dash"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:DASH_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:DASH_DOWNLOAD_URL -OutFile "$CompDownloadDir\dash-$CompVersion.zip"
        } else {
            Write-Output "DASH_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "dash@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install dash "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "dash@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add dash; vfox install "dash@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "dash") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing dash $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\dash\dash-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\dash\dash-$CompVersion.tar.gz"
        ...
        } elseif ($env:DASH_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "dash.zip"
            Invoke-WebRequest -Uri $env:DASH_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for dash."
        }
    } else {
        Write-Output "dash $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_dash" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling dash $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "dash") $CompVersion
