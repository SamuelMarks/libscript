...
DASH_INSTALL_METHOD="$(libscript_resolve_install_method "DASH")"
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "${DASH_INSTALL_METHOD}" = "mise" ]; then
      ...
      echo "Fetching remote versions not implemented generically for dash"
    fi
    exit 0
    ;;
  use)
    ...
    if [ "${DASH_INSTALL_METHOD}" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/dash/"*"${VERSION}"* >/dev/null 2>&1; then
        ...
        if [ -n "${DASH_DOWNLOAD_URL:-}" ]; then
          ...
          log_warn "No download URL provided for dash ${VERSION}."
          # Fallback to mock
          echo "#!/bin/sh" > "${TARGET_DIR}/bin/dash"
          echo "echo 'Mock dash executable for version ${EXACT_VERSION}'" >> "${TARGET_DIR}/bin/dash"
          chmod +x "${TARGET_DIR}/bin/dash"
        fi
      fi
