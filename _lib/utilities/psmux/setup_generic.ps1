<#
.SYNOPSIS
Windows PowerShell setup stub for psmux
...
$InstallMethod = $env:PSMUX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list psmux; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all psmux; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:PSMUX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PSMUX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global psmux "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "psmux") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "psmux") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading psmux $CompVersion to $DownloadDir\psmux..."
        $CompDownloadDir = Join-Path $DownloadDir "psmux"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PSMUX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PSMUX_DOWNLOAD_URL -OutFile "$CompDownloadDir\psmux-$CompVersion.zip"
        } else {
            Write-Output "PSMUX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "psmux@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install psmux "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "psmux@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add psmux; vfox install "psmux@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "psmux") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing psmux $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\psmux\psmux-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\psmux\psmux-$CompVersion.tar.gz"
        ...
        } elseif ($env:PSMUX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "psmux.zip"
            Invoke-WebRequest -Uri $env:PSMUX_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for psmux."
        }
    } else {
        Write-Output "psmux $CompVersion is already installed."
    }
