...
case "$ACTION" in
  ...
  ls-remote)
    if [ "${PSMUX_INSTALL_METHOD}" = "mise" ]; then
      ...
      echo "Fetching remote versions not implemented generically for psmux"
    fi
    exit 0
    ;;
  use)
    ...
    if [ "${PSMUX_INSTALL_METHOD}" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/psmux/"*"${VERSION}"* >/dev/null 2>&1; then
        ...
        if [ -n "${PSMUX_DOWNLOAD_URL:-}" ]; then
          ...
          # Fallback to mock
          echo "#!/bin/sh" > "${TARGET_DIR}/bin/psmux"
          echo "echo 'Mock psmux executable for version ${EXACT_VERSION}'" >> "${TARGET_DIR}/bin/psmux"
