...
WGET_INSTALL_METHOD="$(libscript_resolve_install_method "WGET")"
...
case "$ACTION" in
  ...
  ls-remote)
    if [ "${WGET_INSTALL_METHOD}" = "mise" ]; then
      ...
      echo "Fetching remote versions not implemented generically for wget"
    fi
    exit 0
    ;;
  use)
    ...
    if [ "${WGET_INSTALL_METHOD}" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/wget/"*"${VERSION}"* >/dev/null 2>&1; then
        ...
        if [ -n "${WGET_DOWNLOAD_URL:-}" ]; then
          ...
          log_warn "No download URL provided for wget ${VERSION}."
          # Fallback to mock
          echo "#!/bin/sh" > "${TARGET_DIR}/bin/wget"
          echo "echo 'Mock wget executable for version ${EXACT_VERSION}'" >> "${TARGET_DIR}/bin/wget"
          chmod +x "${TARGET_DIR}/bin/wget"
        fi
      fi
