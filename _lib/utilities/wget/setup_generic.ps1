...
<#
.SYNOPSIS
Windows PowerShell setup stub for wget
...
$InstallMethod = $env:WGET_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls wget; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list wget; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls wget; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "wget"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote wget; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all wget; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all wget; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:WGET_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:WGET_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "wget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global wget "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "wget@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "wget") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "wget") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading wget $CompVersion to $DownloadDir\wget..."
        $CompDownloadDir = Join-Path $DownloadDir "wget"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:WGET_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:WGET_DOWNLOAD_URL -OutFile "$CompDownloadDir\wget-$CompVersion.zip"
        } else {
            Write-Output "WGET_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "wget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install wget "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "wget@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add wget; vfox install "wget@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "wget") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing wget $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\wget\wget-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\wget\wget-$CompVersion.tar.gz"
        ...
        } elseif ($env:WGET_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "wget.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_wget" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling wget $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "wget") $CompVersion
