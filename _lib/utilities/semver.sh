#!/bin/sh
# ## Overview
# Lifecycle script for semver.sh.
#
# ## Usage
# Refer to the internal functions of semver.sh for implementation details.


set -feu
# shellcheck disable=SC2296,SC3028,SC3040,SC3054
if [ "${SCRIPT_NAME-}" ]; then
  THIS_FILE="${SCRIPT_NAME}"
elif [ "${BASH_SOURCE-}" ]; then
  eval 'THIS_FILE="${BASH_SOURCE[0]}"'
  eval 'set -o pipefail'
elif [ "${ZSH_VERSION-}" ]; then
  eval 'THIS_FILE="${(%):-%x}"'
  eval 'set -o pipefail'
else
  THIS_FILE="${0}"
fi

case "${STACK+x}" in
  *':'"${THIS_FILE}"':'*)
    printf '[STOP]     processing "%s"\n' "${THIS_FILE}" >&2
    if (return 0 2>/dev/null); then return; else exit 0; fi ;;
  *) printf '[CONTINUE] processing "%s"\n' "${THIS_FILE}" >&2 ;;
esac
export STACK="${STACK:-}${THIS_FILE}"':'
SCRIPT_DIR=$(cd -- "$(dirname -- "${THIS_FILE}")" && pwd)
: "${LIBSCRIPT_ROOT_DIR:=$(d="$SCRIPT_DIR"; while [ ! -f "$d/libscript.sh" ]; do n="${d%/*}"; [ -z "$n" ] && n="/"; [ "$d" = "$n" ] && break; d="$n"; done; printf '%s\n' "$d")}"
# @description Automatically handles semver for the semver.sh (utilities) component.
# @file semver.sh


if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  log_info "Usage: ${THIS_FILE} [OPTIONS]"
  log_info "See script source or documentation for more details."
  exit 0
fi

if [ -z "$1" ] || [ -z "$2" ]; then
    log_info "Usage: ${THIS_FILE} <version> <constraint>"
    exit 2
fi

SCRIPT_DIR=$(dirname -- "${THIS_FILE}")

jq -e -L "$SCRIPT_DIR" -n --arg v "$1" --arg c "$2" "include \"semver\"; semver_satisfies(\$v; \$c)" >/dev/null 2>&1
exit $?
