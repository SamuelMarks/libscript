<#
.SYNOPSIS
Windows PowerShell setup stub for 7zip
...
$InstallMethod = $env:SEVENZIP_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls 7zip; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list 7zip; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls 7zip; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "7zip"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote 7zip; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all 7zip; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all 7zip; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:SEVENZIP_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:SEVENZIP_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "7zip@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global 7zip "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "7zip@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "7zip") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "7zip") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading 7zip $CompVersion to $DownloadDir\7zip..."
        $CompDownloadDir = Join-Path $DownloadDir "7zip"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:SEVENZIP_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:SEVENZIP_DOWNLOAD_URL -OutFile "$CompDownloadDir\7zip-$CompVersion.zip"
        } else {
            Write-Output "SEVENZIP_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "7zip@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install 7zip "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "7zip@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add 7zip; vfox install "7zip@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "7zip") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing 7zip $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\7zip\7zip-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\7zip\7zip-$CompVersion.tar.gz"
        ...
        } elseif ($env:SEVENZIP_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "7zip.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_7zip" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling 7zip $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "7zip") $CompVersion
