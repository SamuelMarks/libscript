<#
.SYNOPSIS
Windows PowerShell setup stub for aria2
...
$InstallMethod = $env:ARIA2_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls aria2; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list aria2; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls aria2; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "aria2"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote aria2; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all aria2; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all aria2; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:ARIA2_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:ARIA2_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "aria2@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global aria2 "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "aria2@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/aria2/aria2/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "aria2") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "aria2") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading aria2 $CompVersion to $DownloadDir\aria2..."
        $CompDownloadDir = Join-Path $DownloadDir "aria2"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:ARIA2_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:ARIA2_DOWNLOAD_URL -OutFile "$CompDownloadDir\aria2-$CompVersion.zip"
        } else {
            Write-Output "ARIA2_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "aria2@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install aria2 "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "aria2@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add aria2; vfox install "aria2@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "aria2") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing aria2 $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\aria2\aria2-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\aria2\aria2-$CompVersion.tar.gz"
        ...
        } elseif ($env:ARIA2_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "aria2.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_aria2" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling aria2 $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "aria2") $CompVersion
