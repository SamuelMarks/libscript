...
TMUX_INSTALL_METHOD="${TMUX_INSTALL_METHOD:-system}"
TMUX_INSTALL_METHOD="$(LIBSCRIPT_DEFAULT_INSTALL_METHOD="$TMUX_INSTALL_METHOD" libscript_resolve_install_method "TMUX")"
...
case "$ACTION" in
  ...
  use)
    if [ "${TMUX_INSTALL_METHOD}" = "mise" ]; then
      ...
      log_info "  . \"${DIR}/env.sh\" # or: . \$(\"${LIBSCRIPT_ROOT_DIR}/libscript.sh\" env tmux \"${TMUX_VERSION}\")"
    fi
    exit 0
    ;;
  download)
    ...
    if [ "${TMUX_INSTALL_METHOD}" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/tmux/"*"${VERSION:-}"* >/dev/null 2>&1; then
        ...
        if [ -n "${TMUX_DOWNLOAD_URL:-}" ]; then
          ...
          log_error "No download URL provided for tmux ${VERSION:-}."
          exit 1
        fi
      fi
