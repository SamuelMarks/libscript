<#
.SYNOPSIS
Windows PowerShell setup stub for tmux
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls tmux; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list tmux; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls tmux; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "tmux"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote tmux; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all tmux; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all tmux; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:TMUX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:TMUX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "tmux@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global tmux "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "tmux@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/tmux/tmux/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "tmux") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "tmux") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading tmux $CompVersion to $DownloadDir\tmux..."
        $CompDownloadDir = Join-Path $DownloadDir "tmux"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:TMUX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:TMUX_DOWNLOAD_URL -OutFile "$CompDownloadDir\tmux-$CompVersion.zip"
        } else {
            Write-Output "TMUX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "tmux@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install tmux "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "tmux@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add tmux; vfox install "tmux@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "tmux") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing tmux $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\tmux\tmux-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\tmux\tmux-$CompVersion.tar.gz"
        ...
        } elseif ($env:TMUX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "tmux.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_tmux" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling tmux $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "tmux") $CompVersion
