...
if %ERRORLEVEL% EQU 0 (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Invoke-WebRequest -Uri '%PWSH_URL%' -OutFile '%PWSH_OUT%'"
) else (
    certutil -urlcache -split -f "%PWSH_URL%" "%PWSH_OUT%" >nul
)
