<#
.SYNOPSIS
Windows PowerShell setup stub for curl
...
$InstallMethod = $env:CURL_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls curl; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list curl; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls curl; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "curl"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote curl; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all curl; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all curl; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CURL_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CURL_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "curl@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global curl "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "curl@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "curl") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "curl") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading curl $CompVersion to $DownloadDir\curl..."
        $CompDownloadDir = Join-Path $DownloadDir "curl"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CURL_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CURL_DOWNLOAD_URL -OutFile "$CompDownloadDir\curl-$CompVersion.zip"
        } else {
            Write-Output "CURL_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "curl@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install curl "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "curl@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add curl; vfox install "curl@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "curl") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing curl $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\curl\curl-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\curl\curl-$CompVersion.tar.gz"
        ...
        } elseif ($env:CURL_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "curl.zip"
            Invoke-WebRequest -Uri $env:CURL_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for curl."
        }
    } else {
        Write-Output "curl $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_curl" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling curl $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "curl") $CompVersion
