<#
.SYNOPSIS
Windows PowerShell setup stub for busybox
...
$InstallMethod = $env:BUSYBOX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls busybox; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list busybox; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls busybox; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "busybox"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote busybox; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all busybox; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all busybox; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:BUSYBOX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:BUSYBOX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "busybox@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global busybox "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "busybox@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "busybox") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "busybox") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading busybox $CompVersion to $DownloadDir\busybox..."
        $CompDownloadDir = Join-Path $DownloadDir "busybox"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:BUSYBOX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:BUSYBOX_DOWNLOAD_URL -OutFile "$CompDownloadDir\busybox-$CompVersion.zip"
        } else {
            Write-Output "BUSYBOX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "busybox@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install busybox "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "busybox@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add busybox; vfox install "busybox@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "busybox") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing busybox $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\busybox\busybox-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\busybox\busybox-$CompVersion.tar.gz"
        ...
        } elseif ($env:BUSYBOX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "busybox.zip"
            Invoke-WebRequest -Uri $env:BUSYBOX_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for busybox."
        }
    } else {
        Write-Output "busybox $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_busybox" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling busybox $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "busybox") $CompVersion
