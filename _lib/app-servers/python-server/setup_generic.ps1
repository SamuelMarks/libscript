<#
.SYNOPSIS
Windows PowerShell setup stub for python-server
...
$CompVersion = $env:PYTHON_SERVER_VERSION
...
$InstallMethod = $env:PYTHON_SERVER_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list python-server; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all python-server; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:PYTHON_SERVER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:PYTHON_SERVER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global python-server "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "python-server") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "python-server") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading python-server $CompVersion to $DownloadDir\python-server..."
        $CompDownloadDir = Join-Path $DownloadDir "python-server"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:PYTHON_SERVER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:PYTHON_SERVER_DOWNLOAD_URL -OutFile "$CompDownloadDir\python-server-$CompVersion.zip"
        } else {
            Write-Output "PYTHON_SERVER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "python-server@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install python-server "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "python-server@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add python-server; vfox install "python-server@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing python-server $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\python-server\python-server-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\python-server\python-server-$CompVersion.tar.gz"
        ...
        } elseif ($env:PYTHON_SERVER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "python-server.zip"
            Invoke-WebRequest -Uri $env:PYTHON_SERVER_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for python-server."
        }
    } else {
        Write-Output "python-server $CompVersion is already installed."
    }
