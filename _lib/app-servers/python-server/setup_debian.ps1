...
switch ($Action) {
    ...
    "start" {
        ...
        $ServiceName = if ($env:LIBSCRIPT_SERVICE_NAME) { $env:LIBSCRIPT_SERVICE_NAME } else { "libscript_python-server" }
    ...
    "uninstall" {
        if ($InstallMethod -eq "libscript_native") {
            ...
            Write-Host "Uninstalling python-server `$Exact..."
            ...
            $TargetDir = Join-Path $LibscriptHome "python-server\`$Exact"
    ...
    "download" {
        if ($InstallMethod -eq "libscript_native") {
            Write-Host "Downloading python-server..."
        }
        break
    }
    default {
