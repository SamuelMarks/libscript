<#
.SYNOPSIS
Windows PowerShell setup stub for vllm
...
$InstallMethod = $env:VLLM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list vllm; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all vllm; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:VLLM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:VLLM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global vllm "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "vllm") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "vllm") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading vllm $CompVersion to $DownloadDir\vllm..."
        $CompDownloadDir = Join-Path $DownloadDir "vllm"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:VLLM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:VLLM_DOWNLOAD_URL -OutFile "$CompDownloadDir\vllm-$CompVersion.zip"
        } else {
            Write-Output "VLLM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "vllm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install vllm "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "vllm@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add vllm; vfox install "vllm@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "vllm") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing vllm $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\vllm\vllm-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\vllm\vllm-$CompVersion.tar.gz"
        ...
        } elseif ($env:VLLM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "vllm.zip"
            Invoke-WebRequest -Uri $env:VLLM_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for vllm."
        }
    } else {
        Write-Output "vllm $CompVersion is already installed."
    }
