#!/bin/sh
# ## Overview
# Provides specialized installation and configuration logic for a Node.js Server on Debian/Ubuntu systems.
# It ensures Node.js dependencies (via `npm`, `yarn`, or `pnpm`) are resolved, discovers the main
# application script (`main.js`, `app.js`, etc.), dynamically builds a Systemd service file
# representing the server, installs it, and registers listening ports via `netctl`.
# 
# ## Usage
...
for LIB in "_lib/_common/environ.sh" "_lib/_common/pkg_mgr.sh" "_lib/git-servers/utils/git.sh" "_lib/languages/nodejs/setup.sh"; do
...
SERVICE_NAME=''
if [ -z "${DEST+x}" ]; then
  if [ -f "${LIBSCRIPT_DATA_DIR}/.nodejs_server_dest" ]; then
    DEST="$(cat "${LIBSCRIPT_DATA_DIR}/.nodejs_server_dest")"
    RAND="$(basename -- "${DEST}")"
  else
    RAND="$(env LC_CTYPE='C' tr -cd '[:lower:]' < '/dev/urandom' | head -c 8)"
    DEST="${LIBSCRIPT_DATA_DIR}"'/'"${RAND}"
    mkdir -p -- "${LIBSCRIPT_DATA_DIR}"
    printf '%s\n' "${DEST}" > "${LIBSCRIPT_DATA_DIR}/.nodejs_server_dest"
  fi
  export DEST
  NODEJS_SERVER_DEST="${DEST}"
  mkdir -p -- "${NODEJS_SERVER_DEST}"
  SERVICE_NAME='nodejs-'"${RAND}"
  touch "${NODEJS_SERVER_DEST}/main.js"
else
  SERVICE_NAME="$(basename -- "${NODEJS_SERVER_DEST}")"
fi
NAME=' '"${SERVICE_NAME}"
cd -- "${NODEJS_SERVER_DEST}"
...
if [ "${NODEJS_SERVER_VARS-}" ]; then
  if [ -f "${LIBSCRIPT_DATA_DIR}/dyn_env.sh" ]; then
    tmp_env=$(mktemp)
    libscript_object2key_val "${NODEJS_SERVER_VARS}" 'export ' "'" | awk -F= '{print $1}' | while read -r key_prefix; do
  ...
  if [ -f "${LIBSCRIPT_DATA_DIR}/dyn_env.csh" ]; then
    tmp_env=$(mktemp)
    libscript_object2key_val "${NODEJS_SERVER_VARS}" 'setenv ' "'" | awk '{print $2}' | while read -r key; do
      grep -v "^setenv ${key} " "${LIBSCRIPT_DATA_DIR}/dyn_env.csh" > "${tmp_env}" || true
      cat "${tmp_env}" > "${LIBSCRIPT_DATA_DIR}/dyn_env.csh"
    done
    rm -f "${tmp_env}"
  fi
  libscript_object2key_val "${NODEJS_SERVER_VARS}" 'export ' "'" >> "${LIBSCRIPT_DATA_DIR}"'/dyn_env.sh'
  libscript_object2key_val "${NODEJS_SERVER_VARS}" 'setenv ' "'" >> "${LIBSCRIPT_DATA_DIR}"'/dyn_env.csh'
fi
ENV=''
if [ -f "${LIBSCRIPT_DATA_DIR}"'/dyn_env.sh' ]; then
