...
switch ($Action) {
    ...
    "start" {
        ...
        $ServiceName = if ($env:LIBSCRIPT_SERVICE_NAME) { $env:LIBSCRIPT_SERVICE_NAME } else { "libscript_nodejs-server" }
    ...
    "uninstall" {
        if ($InstallMethod -eq "libscript_native") {
            ...
            Write-Host "Uninstalling nodejs-server `$Exact..."
            ...
            $TargetDir = Join-Path $LibscriptHome "nodejs-server\`$Exact"
    ...
    "download" {
        if ($InstallMethod -eq "libscript_native") {
            Write-Host "Downloading nodejs-server..."
        }
        break
    }
    default {
