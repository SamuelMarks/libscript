<#
.SYNOPSIS
Windows PowerShell setup stub for nodejs-server
...
$InstallMethod = $env:NODEJS_SERVER_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list nodejs-server; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all nodejs-server; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:NODEJS_SERVER_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NODEJS_SERVER_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global nodejs-server "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "nodejs-server") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "nodejs-server") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading nodejs-server $CompVersion to $DownloadDir\nodejs-server..."
        $CompDownloadDir = Join-Path $DownloadDir "nodejs-server"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NODEJS_SERVER_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NODEJS_SERVER_DOWNLOAD_URL -OutFile "$CompDownloadDir\nodejs-server-$CompVersion.zip"
        } else {
            Write-Output "NODEJS_SERVER_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "nodejs-server@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install nodejs-server "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "nodejs-server@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add nodejs-server; vfox install "nodejs-server@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        ...
        } elseif ($env:NODEJS_SERVER_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "nodejs-server.zip"
