...
switch ($Action) {
    ...
    "start" {
        ...
        $ServiceName = if ($env:LIBSCRIPT_SERVICE_NAME) { $env:LIBSCRIPT_SERVICE_NAME } else { "libscript_rust-server" }
    ...
    "uninstall" {
        if ($InstallMethod -eq "libscript_native") {
            ...
            Write-Host "Uninstalling rust-server `$Exact..."
            ...
            $TargetDir = Join-Path $LibscriptHome "rust-server\`$Exact"
    ...
    "download" {
        if ($InstallMethod -eq "libscript_native") {
            Write-Host "Downloading rust-server..."
        }
        break
    }
    default {
