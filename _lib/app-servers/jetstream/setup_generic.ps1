<#
.SYNOPSIS
Windows PowerShell setup stub for jetstream
...
$CompVersion = $env:JETSTREAM_VERSION
...
$InstallMethod = $env:JETSTREAM_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list jetstream; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all jetstream; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:JETSTREAM_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:JETSTREAM_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global jetstream "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "jetstream") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "jetstream") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading jetstream $CompVersion to $DownloadDir\jetstream..."
        $CompDownloadDir = Join-Path $DownloadDir "jetstream"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:JETSTREAM_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:JETSTREAM_DOWNLOAD_URL -OutFile "$CompDownloadDir\jetstream-$CompVersion.zip"
        } else {
            Write-Output "JETSTREAM_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "jetstream@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install jetstream "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "jetstream@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add jetstream; vfox install "jetstream@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing jetstream $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\jetstream\jetstream-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\jetstream\jetstream-$CompVersion.tar.gz"
        ...
        } elseif ($env:JETSTREAM_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "jetstream.zip"
