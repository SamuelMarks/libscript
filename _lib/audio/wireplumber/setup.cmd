...
::
:: ## Overview
:: Recipe for WirePlumber: Modular session and policy manager for PipeWire.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.wireplumber"
if exist "%STAMP_FILE%" (
    echo [SKIP]  wireplumber already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging WirePlumber into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    wireplumber staged successfully: %STAMP_FILE%
