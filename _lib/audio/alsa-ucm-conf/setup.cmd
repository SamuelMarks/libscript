...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.alsa-ucm-conf"
if exist "%STAMP_FILE%" (
    echo [SKIP]  alsa-ucm-conf already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging ALSA UCM Conf into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    alsa-ucm-conf staged successfully: %STAMP_FILE%
