#!/bin/sh
# ## Overview
# Recipe for ALSA UCM Conf: ALSA Use Case Manager configuration files.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.alsa-ucm-conf"

if [ ! -d "$STAMPS_DIR" ]; then
  mkdir -p "$STAMPS_DIR"
fi

if [ ! -d /usr/share/alsa/ucm2 ] && [ ! -d /usr/share/alsa/ucm ]; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache alsa-ucm-conf || true
    ...
    priv apt-get install -y alsa-ucm-conf || true
    ...
    priv dnf install -y alsa-ucm || priv dnf install -y alsa-ucm-conf || true
  fi
fi

if [ -f "$STAMP_FILE" ]; then
  printf '[SKIP]  %s already installed (%s)
' "alsa-ucm-conf" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "ALSA UCM Conf" "$TARGET_SYSROOT" "$ACTION"
