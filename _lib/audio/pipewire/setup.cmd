...
::
:: ## Overview
:: Recipe for PipeWire Audio: Modern multimedia graph processing server.
...
if "%~1"=="--help" (
    ...
    echo Recipe for PipeWire Audio.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.pipewire"
if exist "%STAMP_FILE%" (
    echo [SKIP]  pipewire already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging PipeWire Audio into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    pipewire staged successfully: %STAMP_FILE%
