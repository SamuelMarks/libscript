...
::
:: ## Overview
:: Recipe for ALSA Library: Advanced Linux Sound Architecture userland library.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.alsa-lib"
if exist "%STAMP_FILE%" (
    echo [SKIP]  alsa-lib already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging ALSA Library into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    alsa-lib staged successfully: %STAMP_FILE%
