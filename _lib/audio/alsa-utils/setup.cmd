...
::
:: ## Overview
:: Recipe for ALSA Utilities: Command-line tools for ALSA sound subsystem.
...
if "%~1"=="--help" (
    ...
    echo Recipe for ALSA Utilities.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.alsa-utils"
if exist "%STAMP_FILE%" (
    echo [SKIP]  alsa-utils already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging ALSA Utilities into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    alsa-utils staged successfully: %STAMP_FILE%
