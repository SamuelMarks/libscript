...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.pulseaudio"
if exist "%STAMP_FILE%" (
    echo [SKIP]  pulseaudio already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging PulseAudio into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    pulseaudio staged successfully: %STAMP_FILE%
