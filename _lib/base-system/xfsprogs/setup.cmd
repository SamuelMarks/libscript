...
::
:: ## Overview
:: Recipe for XFS Progs: XFS filesystem administration utilities.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.xfsprogs"
if exist "%STAMP_FILE%" (
    echo [SKIP]  xfsprogs already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging XFS Progs into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    xfsprogs staged successfully: %STAMP_FILE%
