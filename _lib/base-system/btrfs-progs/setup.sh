#!/bin/sh
# ## Overview
# Recipe for Btrfs Progs: Btrfs filesystem creation and management tools.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.btrfs-progs"
...
if ! command -v btrfs >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache btrfs-progs || true
    ...
    priv apt-get install -y btrfs-progs || true
    ...
    priv dnf install -y btrfs-progs || (priv dnf install -y epel-release && priv dnf install -y btrfs-progs) || true
  fi
fi

if [ -f "$STAMP_FILE" ]; then
  printf '[SKIP]  %s already installed (%s)
' "btrfs-progs" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "Btrfs Progs" "$TARGET_SYSROOT" "$ACTION"
