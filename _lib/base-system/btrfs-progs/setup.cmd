...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.btrfs-progs"
if exist "%STAMP_FILE%" (
    echo [SKIP]  btrfs-progs already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Btrfs Progs into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    btrfs-progs staged successfully: %STAMP_FILE%
