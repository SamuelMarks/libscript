...
::
:: ## Overview
:: Recipe for GNU C Library: Core C library and dynamic linker.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.glibc"
if exist "%STAMP_FILE%" (
    echo [SKIP]  glibc already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging GNU C Library into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    glibc staged successfully: %STAMP_FILE%
