...
::
:: ## Overview
:: Recipe for Argp Standalone: Hierarchical argument parsing library for musl.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Argp Standalone.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.argp-standalone"
if exist "%STAMP_FILE%" (
    echo [SKIP]  argp-standalone already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Argp Standalone into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    argp-standalone staged successfully: %STAMP_FILE%
