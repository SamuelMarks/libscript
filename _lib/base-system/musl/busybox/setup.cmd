...
::
:: ## Overview
:: Recipe for BusyBox: Swiss Army Knife of embedded Linux utilities.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.busybox"
if exist "%STAMP_FILE%" (
    echo [SKIP]  busybox already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging BusyBox into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    busybox staged successfully: %STAMP_FILE%
