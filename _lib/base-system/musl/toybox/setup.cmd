...
::
:: ## Overview
:: Recipe for Toybox: BSD-licensed POSIX all-in-one command suite.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.toybox"
if exist "%STAMP_FILE%" (
    echo [SKIP]  toybox already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Toybox into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    toybox staged successfully: %STAMP_FILE%
