...
::
:: ## Overview
:: Recipe for Musl FTS: FTS implementation shim for musl libc.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.musl-fts"
if exist "%STAMP_FILE%" (
    echo [SKIP]  musl-fts already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Musl FTS into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    musl-fts staged successfully: %STAMP_FILE%
