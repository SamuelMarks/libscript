...
::
:: ## Overview
:: Recipe for Eudev (Musl): Eudev compiled against musl libc.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.eudev-musl"
if exist "%STAMP_FILE%" (
    echo [SKIP]  eudev-musl already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Eudev (Musl) into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    eudev-musl staged successfully: %STAMP_FILE%
