...
::
:: ## Overview
:: Recipe for Mdev: BusyBox dynamic device node manager.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.mdev"
if exist "%STAMP_FILE%" (
    echo [SKIP]  mdev already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Mdev into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    mdev staged successfully: %STAMP_FILE%
