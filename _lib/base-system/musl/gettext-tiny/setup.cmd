...
::
:: ## Overview
:: Recipe for gettext-tiny: Lightweight internationalization shim.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.gettext-tiny"
if exist "%STAMP_FILE%" (
    echo [SKIP]  gettext-tiny already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging gettext-tiny into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    gettext-tiny staged successfully: %STAMP_FILE%
