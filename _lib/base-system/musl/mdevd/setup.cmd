...
::
:: ## Overview
:: Recipe for Mdevd: Compatible daemonized mdev device manager.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.mdevd"
if exist "%STAMP_FILE%" (
    echo [SKIP]  mdevd already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Mdevd into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    mdevd staged successfully: %STAMP_FILE%
