...
::
:: ## Overview
:: Recipe for Musl Obstack: GNU obstack library standalone port for musl.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.musl-obstack"
if exist "%STAMP_FILE%" (
    echo [SKIP]  musl-obstack already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Musl Obstack into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    musl-obstack staged successfully: %STAMP_FILE%
