...
::
:: ## Overview
:: Recipe for Linux PAM: Pluggable Authentication Modules library.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.pam"
if exist "%STAMP_FILE%" (
    echo [SKIP]  pam already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Linux PAM into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    pam staged successfully: %STAMP_FILE%
