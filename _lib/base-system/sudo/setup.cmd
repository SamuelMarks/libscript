...
::
:: ## Overview
:: Recipe for Sudo: Privilege escalation tool for system administration.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.sudo"
if exist "%STAMP_FILE%" (
    echo [SKIP]  sudo already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Sudo into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    sudo staged successfully: %STAMP_FILE%
