...
::
:: ## Overview
:: Recipe for Shadow: User account and password management suite.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.shadow"
if exist "%STAMP_FILE%" (
    echo [SKIP]  shadow already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Shadow into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    shadow staged successfully: %STAMP_FILE%
