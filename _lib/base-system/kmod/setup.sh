#!/bin/sh
# ## Overview
# Recipe for Kmod: Linux kernel module loader and management library.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.kmod"

if [ ! -d "$STAMPS_DIR" ]; then
  mkdir -p "$STAMPS_DIR"
fi

if ! command -v modprobe >/dev/null 2>&1 && ! command -v lsmod >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache kmod || true
    ...
    priv apt-get install -y kmod || true
