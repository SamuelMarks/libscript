...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.kmod"
if exist "%STAMP_FILE%" (
    echo [SKIP]  kmod already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Kmod into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    kmod staged successfully: %STAMP_FILE%
