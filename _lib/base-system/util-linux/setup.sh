...
STAMP_FILE="${STAMPS_DIR}/.stamp.util-linux"
...
if ! command -v fdisk >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache util-linux || true
    ...
    priv apt-get install -y util-linux fdisk || true
...
' "util-linux" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "Util-Linux" "$TARGET_SYSROOT" "$ACTION"
