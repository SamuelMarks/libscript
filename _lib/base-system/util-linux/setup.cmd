...
::
:: ## Overview
:: Recipe for Util-Linux: Essential system administration and disk utilities.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.util-linux"
if exist "%STAMP_FILE%" (
    echo [SKIP]  util-linux already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Util-Linux into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    util-linux staged successfully: %STAMP_FILE%
