...
::
:: ## Overview
:: Recipe for GNU Coreutils: Basic file, shell and text manipulation utilities.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.coreutils"
if exist "%STAMP_FILE%" (
    echo [SKIP]  coreutils already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging GNU Coreutils into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    coreutils staged successfully: %STAMP_FILE%
