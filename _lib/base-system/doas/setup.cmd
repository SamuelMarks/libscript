...
::
:: ## Overview
:: Recipe for Doas: Lightweight OpenBSD privilege escalation utility.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.doas"
if exist "%STAMP_FILE%" (
    echo [SKIP]  doas already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Doas into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    doas staged successfully: %STAMP_FILE%
