...
STAMP_FILE="${STAMPS_DIR}/.stamp.e2fsprogs"

if [ ! -d "$STAMPS_DIR" ]; then
  mkdir -p "$STAMPS_DIR"
fi

if ! command -v mkfs.ext4 >/dev/null 2>&1 && ! command -v tune2fs >/dev/null 2>&1; then
...
' "e2fsprogs" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "e2fsprogs" "$TARGET_SYSROOT" "$ACTION"
