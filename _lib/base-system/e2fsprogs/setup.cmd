...
::
:: ## Overview
:: Recipe for e2fsprogs: Ext2/Ext3/Ext4 filesystem management utilities.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.e2fsprogs"
if exist "%STAMP_FILE%" (
    echo [SKIP]  e2fsprogs already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging e2fsprogs into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    e2fsprogs staged successfully: %STAMP_FILE%
