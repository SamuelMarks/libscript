...
::
:: ## Overview
:: Recipe for Eudev: Device node manager daemon and library.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.eudev"
if exist "%STAMP_FILE%" (
    echo [SKIP]  eudev already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Eudev into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    eudev staged successfully: %STAMP_FILE%
