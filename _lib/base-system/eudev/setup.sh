...
STAMP_FILE="${STAMPS_DIR}/.stamp.eudev"
...
if ! command -v udevadm >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache eudev || true
    ...
    priv apt-get install -y udev || true
    ...
    priv dnf install -y systemd-udev eudev || true
  fi
fi
