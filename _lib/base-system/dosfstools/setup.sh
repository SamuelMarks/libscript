...
STAMP_FILE="${STAMPS_DIR}/.stamp.dosfstools"
...
if ! command -v mkfs.fat >/dev/null 2>&1 && ! command -v mkfs.vfat >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache dosfstools || true
    ...
    priv apt-get install -y dosfstools || true
...
' "dosfstools" "$STAMP_FILE"
  exit 0
fi

printf '[RECIPE] Staging %s into %s (action: %s)
' "Dosfstools" "$TARGET_SYSROOT" "$ACTION"
