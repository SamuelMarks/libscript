...
::
:: ## Overview
:: Recipe for Dosfstools: FAT and VFAT filesystem utilities.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.dosfstools"
if exist "%STAMP_FILE%" (
    echo [SKIP]  dosfstools already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Dosfstools into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    dosfstools staged successfully: %STAMP_FILE%
