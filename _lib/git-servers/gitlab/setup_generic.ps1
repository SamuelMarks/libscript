<#
.SYNOPSIS
Windows PowerShell setup stub for gitlab
...
$InstallMethod = $env:GITLAB_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls gitlab; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list gitlab; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls gitlab; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "gitlab"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote gitlab; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all gitlab; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all gitlab; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:GITLAB_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GITLAB_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "gitlab@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global gitlab "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "gitlab@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "gitlab") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "gitlab") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading gitlab $CompVersion to $DownloadDir\gitlab..."
        $CompDownloadDir = Join-Path $DownloadDir "gitlab"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GITLAB_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GITLAB_DOWNLOAD_URL -OutFile "$CompDownloadDir\gitlab-$CompVersion.zip"
        } else {
            Write-Output "GITLAB_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "gitlab@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install gitlab "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "gitlab@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add gitlab; vfox install "gitlab@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "gitlab") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing gitlab $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\gitlab\gitlab-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\gitlab\gitlab-$CompVersion.tar.gz"
        ...
        } elseif ($env:GITLAB_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "gitlab.zip"
            Invoke-WebRequest -Uri $env:GITLAB_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for gitlab."
        }
    } else {
        Write-Output "gitlab $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_gitlab" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling gitlab $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "gitlab") $CompVersion
