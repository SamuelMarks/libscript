...
| `GITEA_VERSION` | Specific version of gitea to install. Can be a numeric version or an alias. | `latest` | latest, stable |
| `GITEA_INSTALL_METHOD` | How to install GITEA. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager, 'mise', 'asdf', 'pkgx', or 'vfox' defers to third-party tools. | `libscript_native` |  |
...
| `GITEA_LISTEN_PORT` | Port for GITEA to listen on | `none` |  |
| `GITEA_LISTEN_ADDRESS` | Address for GITEA to listen on | `none` |  |
| `GITEA_LISTEN_SOCKET` | Unix socket for GITEA to listen on | `none` |  |
<!-- END_VARS -->
...
Libscript manages gitea versions natively by installing them into isolated directories under
`LIBSCRIPT_HOME/gitea/<version>`.
