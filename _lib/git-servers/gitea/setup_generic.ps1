<#
.SYNOPSIS
Windows PowerShell setup stub for gitea
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list gitea; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all gitea; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:GITEA_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:GITEA_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global gitea "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "gitea") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "gitea") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading gitea $CompVersion to $DownloadDir\gitea..."
        $CompDownloadDir = Join-Path $DownloadDir "gitea"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:GITEA_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:GITEA_DOWNLOAD_URL -OutFile "$CompDownloadDir\gitea-$CompVersion.zip"
        } else {
            Write-Output "GITEA_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "gitea@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install gitea "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "gitea@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add gitea; vfox install "gitea@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "gitea") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing gitea $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\gitea\gitea-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\gitea\gitea-$CompVersion.tar.gz"
        ...
        } elseif ($env:GITEA_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "gitea.zip"
            Invoke-WebRequest -Uri $env:GITEA_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for gitea."
        }
    } else {
        Write-Output "gitea $CompVersion is already installed."
    }
