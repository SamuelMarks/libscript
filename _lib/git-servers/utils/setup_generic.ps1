...
$InstallMethod = $env:UTILS_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls utils; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list utils; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls utils; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "utils"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote utils; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all utils; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all utils; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:UTILS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:UTILS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "utils@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global utils "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "utils@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "utils") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "utils") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading utils $CompVersion to $DownloadDir\utils..."
        $CompDownloadDir = Join-Path $DownloadDir "utils"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:UTILS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:UTILS_DOWNLOAD_URL -OutFile "$CompDownloadDir\utils-$CompVersion.zip"
        } else {
            Write-Output "UTILS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "utils@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install utils "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "utils@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add utils; vfox install "utils@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "utils") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing utils $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\utils\utils-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\utils\utils-$CompVersion.tar.gz"
        ...
        } elseif ($env:UTILS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "utils.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_utils" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling utils $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "utils") $CompVersion
