...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.weston"
if exist "%STAMP_FILE%" (
    echo [SKIP]  weston already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Weston into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    weston staged successfully: %STAMP_FILE%
