...
STAMP_FILE="${STAMPS_DIR}/.stamp.labwc"
...
if ! command -v labwc >/dev/null 2>&1; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache labwc || true
    ...
    priv apt-get install --no-install-recommends -y labwc || true
