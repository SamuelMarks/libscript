...
::
:: ## Overview
:: Recipe for labwc: Openbox-like stacking Wayland compositor.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.labwc"
if exist "%STAMP_FILE%" (
    echo [SKIP]  labwc already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging labwc into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    labwc staged successfully: %STAMP_FILE%
