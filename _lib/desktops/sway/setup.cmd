...
::
:: ## Overview
:: Recipe for Sway Compositor: i3-compatible tiling Wayland compositor.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Sway Compositor.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.sway"
if exist "%STAMP_FILE%" (
    echo [SKIP]  sway already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Sway Compositor into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    sway staged successfully: %STAMP_FILE%
