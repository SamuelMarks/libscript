...
::
:: ## Overview
:: Recipe for LXQt Desktop: Lightweight Qt-based desktop environment.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.lxqt"
if exist "%STAMP_FILE%" (
    echo [SKIP]  lxqt already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging LXQt Desktop into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    lxqt staged successfully: %STAMP_FILE%
