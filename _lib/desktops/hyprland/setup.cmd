...
::
:: ## Overview
:: Recipe for Hyprland: Dynamic tiling Wayland compositor with animations.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.hyprland"
if exist "%STAMP_FILE%" (
    echo [SKIP]  hyprland already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Hyprland into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    hyprland staged successfully: %STAMP_FILE%
