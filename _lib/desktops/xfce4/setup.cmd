...
::
:: ## Overview
:: Recipe for XFCE Desktop: Fast and lightweight desktop environment.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.xfce4"
if exist "%STAMP_FILE%" (
    echo [SKIP]  xfce4 already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging XFCE Desktop into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    xfce4 staged successfully: %STAMP_FILE%
