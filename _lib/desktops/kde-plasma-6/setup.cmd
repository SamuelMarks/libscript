...
::
:: ## Overview
:: Recipe for KDE Plasma 6: Feature-rich desktop workspace by KDE.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.kde-plasma-6"
if exist "%STAMP_FILE%" (
    echo [SKIP]  kde-plasma-6 already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging KDE Plasma 6 into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    kde-plasma-6 staged successfully: %STAMP_FILE%
