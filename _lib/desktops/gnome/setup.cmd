...
::
:: ## Overview
:: Recipe for GNOME Desktop: Modern desktop environment for Linux.
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.gnome"
if exist "%STAMP_FILE%" (
    echo [SKIP]  gnome already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging GNOME Desktop into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    gnome staged successfully: %STAMP_FILE%
