...
**Purpose**: This document describes the `_server` folder, which houses various server components
...
This directory serves as the root for various server components (like Nginx, Caddy, HTTPD, Python,
etc.). Every component within this folder works both as a local version manager (similar to rvm,
nvm, pyenv, uv) for precise software version control, and can be invoked seamlessly from the global
version manager `libscript`.

Furthermore, the server components housed here can be used by libscript to build bigger stacks (like
WordPress, Open edX, nextcloud, etc.), allowing for complex, multi-tier architectures.

You can manage the lifecycle of any server component located in this directory using the global
`libscript` command or the local CLI. For example, replacing `web-servers` with a component like
`nginx` or `caddy`:
...
`WEB_SERVERS_INSTALL_METHOD` variable. The local override takes highest precedence.
