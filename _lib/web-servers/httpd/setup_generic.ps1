<#
.SYNOPSIS
Windows PowerShell setup stub for httpd
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list httpd; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all httpd; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:HTTPD_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:HTTPD_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global httpd "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "httpd") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "httpd") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading httpd $CompVersion to $DownloadDir\httpd..."
        $CompDownloadDir = Join-Path $DownloadDir "httpd"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:HTTPD_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:HTTPD_DOWNLOAD_URL -OutFile "$CompDownloadDir\httpd-$CompVersion.zip"
        } else {
            Write-Output "HTTPD_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "httpd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install httpd "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "httpd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add httpd; vfox install "httpd@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "httpd") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing httpd $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\httpd\httpd-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\httpd\httpd-$CompVersion.tar.gz"
        ...
        } elseif ($env:HTTPD_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "httpd.zip"
