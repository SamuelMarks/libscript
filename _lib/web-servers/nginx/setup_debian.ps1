...
switch ($Action) {
    ...
    "start" {
        ...
        $ServiceName = if ($env:LIBSCRIPT_SERVICE_NAME) { $env:LIBSCRIPT_SERVICE_NAME } else { "libscript_nginx" }
    ...
    "uninstall" {
        if ($InstallMethod -eq "libscript_native") {
            ...
            Write-Host "Uninstalling nginx `$Exact..."
            ...
            $TargetDir = Join-Path $LibscriptHome "nginx\`$Exact"
