<#
.SYNOPSIS
Windows PowerShell setup stub for nginx
...
$InstallMethod = $env:NGINX_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls nginx; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list nginx; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls nginx; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "nginx"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote nginx; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all nginx; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all nginx; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:NGINX_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:NGINX_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "nginx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global nginx "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "nginx@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "nginx") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "nginx") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading nginx $CompVersion to $DownloadDir\nginx..."
        $CompDownloadDir = Join-Path $DownloadDir "nginx"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:NGINX_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:NGINX_DOWNLOAD_URL -OutFile "$CompDownloadDir\nginx-$CompVersion.zip"
        } else {
            Write-Output "NGINX_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "nginx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install nginx "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "nginx@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add nginx; vfox install "nginx@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "nginx") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing nginx $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\nginx\nginx-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\nginx\nginx-$CompVersion.tar.gz"
        ...
        } elseif ($env:NGINX_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "nginx.zip"
            Invoke-WebRequest -Uri $env:NGINX_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for nginx."
        }
    } else {
        Write-Output "nginx $CompVersion is already installed."
    }
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_nginx" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling nginx $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "nginx") $CompVersion
