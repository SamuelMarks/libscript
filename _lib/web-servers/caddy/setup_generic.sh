...
case "$ACTION" in
  ...
  ls-remote)
    if [ "${CADDY_INSTALL_METHOD}" = "mise" ]; then
      ...
      echo "Fetching remote versions not implemented generically for caddy"
    fi
    exit 0
    ;;
  use)
    ...
    if [ "${CADDY_INSTALL_METHOD}" = "system" ]; then
      ...
      if ls "${DOWNLOAD_DIR:-/tmp/libscript_downloads}/caddy/"*"${VERSION}"* >/dev/null 2>&1; then
        ...
        if [ -n "${CADDY_DOWNLOAD_URL:-}" ]; then
          ...
          # Fallback to mock
          echo "#!/bin/sh" > "${TARGET_DIR}/bin/caddy"
          echo "echo 'Mock caddy executable for version ${EXACT_VERSION}'" >> "${TARGET_DIR}/bin/caddy"
