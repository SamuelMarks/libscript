<#
.SYNOPSIS
Windows PowerShell setup stub for caddy
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls caddy; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list caddy; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls caddy; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "caddy"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote caddy; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all caddy; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all caddy; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CADDY_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CADDY_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "caddy@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global caddy "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "caddy@$CompVersion"; exit 0 }
...
if ($CompVersion -eq "latest" -or $CompVersion -eq "lts") {
    try {
        $response = Invoke-RestMethod -Uri "https://api.github.com/repos/caddyserver/caddy/releases/latest" -ErrorAction Stop
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "caddy") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "caddy") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading caddy $CompVersion to $DownloadDir\caddy..."
        $CompDownloadDir = Join-Path $DownloadDir "caddy"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CADDY_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CADDY_DOWNLOAD_URL -OutFile "$CompDownloadDir\caddy-$CompVersion.zip"
        } else {
            Write-Output "CADDY_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "caddy@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install caddy "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "caddy@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add caddy; vfox install "caddy@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "caddy") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing caddy $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\caddy\caddy-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\caddy\caddy-$CompVersion.tar.gz"
        ...
        } elseif ($env:CADDY_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "caddy.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_caddy" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling caddy $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "caddy") $CompVersion
