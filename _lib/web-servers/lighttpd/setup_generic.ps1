<#
.SYNOPSIS
Windows PowerShell setup stub for lighttpd
...
$InstallMethod = $env:LIGHTTPD_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list lighttpd; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all lighttpd; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:LIGHTTPD_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:LIGHTTPD_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global lighttpd "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "lighttpd") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "lighttpd") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading lighttpd $CompVersion to $DownloadDir\lighttpd..."
        $CompDownloadDir = Join-Path $DownloadDir "lighttpd"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:LIGHTTPD_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:LIGHTTPD_DOWNLOAD_URL -OutFile "$CompDownloadDir\lighttpd-$CompVersion.zip"
        } else {
            Write-Output "LIGHTTPD_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "lighttpd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install lighttpd "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "lighttpd@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add lighttpd; vfox install "lighttpd@$CompVersion"; exit 0 }
    ...
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing lighttpd $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\lighttpd\lighttpd-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\lighttpd\lighttpd-$CompVersion.tar.gz"
        ...
        } elseif ($env:LIGHTTPD_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "lighttpd.zip"
            Invoke-WebRequest -Uri $env:LIGHTTPD_DOWNLOAD_URL -OutFile $TempFile
            Expand-Archive -Path $TempFile -DestinationPath $TargetDir -Force
        } else {
            Write-Output "No download URL or cache available for lighttpd."
        }
    } else {
        Write-Output "lighttpd $CompVersion is already installed."
    }
