...
| `LIGHTTPD_INSTALL_METHOD` | How to install LIGHTTPD. 'libscript_native' uses isolated version dirs, 'system' uses OS package manager, 'mise', 'asdf', 'pkgx', or 'vfox' defers to third-party tools. | `libscript_native` |  |
| `LIGHTTPD_VERSION` | Specific version of lighttpd to install. Can be a numeric version or an alias. | `latest` | latest, stable |
...
| `LIGHTTPD_LISTEN_PORT` | Port for LIGHTTPD to listen on | `none` |  |
| `LIGHTTPD_LISTEN_ADDRESS` | Address for LIGHTTPD to listen on | `none` |  |
| `LIGHTTPD_LISTEN_SOCKET` | Unix socket for LIGHTTPD to listen on | `none` |  |
<!-- END_VARS -->
...
Libscript manages lighttpd versions natively by installing them into isolated directories under
`LIBSCRIPT_HOME/lighttpd/<version>`.
