<#
.SYNOPSIS
Windows PowerShell setup stub for iis
...
$InstallMethod = $env:IIS_INSTALL_METHOD
...
if ($Action -eq "ls") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list iis; exit 0 }
...
if ($Action -eq "ls-remote") {
    ...
    if ($InstallMethod -eq "asdf") { asdf list all iis; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    ...
    if ($env:IIS_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:IIS_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    ...
    if ($InstallMethod -eq "asdf") { asdf global iis "$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "iis") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "iis") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading iis $CompVersion to $DownloadDir\iis..."
        $CompDownloadDir = Join-Path $DownloadDir "iis"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:IIS_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:IIS_DOWNLOAD_URL -OutFile "$CompDownloadDir\iis-$CompVersion.zip"
        } else {
            Write-Output "IIS_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "iis@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install iis "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "iis@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add iis; vfox install "iis@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "iis") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing iis $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\iis\iis-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\iis\iis-$CompVersion.tar.gz"
        ...
        } elseif ($env:IIS_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "iis.zip"
