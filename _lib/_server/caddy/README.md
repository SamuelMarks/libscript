...
- **Install:** `libscript install caddy`
- **Start:** `libscript start caddy`
- **Stop:** `libscript stop caddy`
- **Uninstall:** `libscript uninstall caddy`
- **Package:** `libscript package_as docker caddy` (or other formats like `docker_compose`, `msi`, etc.)

Alternatively, you can interact with it locally:

**Unix (Linux/macOS):**
```sh
./cli.sh <COMMAND> caddy [VERSION] [OPTIONS]
```

**Windows:**
```cmd
cli.cmd <COMMAND> caddy [VERSION] [OPTIONS]
```
