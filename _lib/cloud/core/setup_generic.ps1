<#
.SYNOPSIS
Windows PowerShell setup stub for core
...
$InstallMethod = $env:CORE_INSTALL_METHOD
...
if ($Action -eq "ls") {
    if ($InstallMethod -eq "mise") { mise ls core; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list core; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls core; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls directly here."; exit 0 }
    $CompDir = Join-Path $LibscriptHome "core"
    if (Test-Path $CompDir) { Get-ChildItem -Path $CompDir -Name }
    exit 0
}

if ($Action -eq "ls-remote") {
    if ($InstallMethod -eq "mise") { mise ls-remote core; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf list all core; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not have a local list command"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox ls all core; exit 0 }
    if ($InstallMethod -eq "system") { Write-Output "System package manager does not support ls-remote directly here."; exit 0 }
    if ($env:CORE_RELEASES_URL) {
        Invoke-WebRequest -Uri $env:CORE_RELEASES_URL | Select-Object -ExpandProperty Content
    } else {
...
if ($Action -eq "use") {
    if ($InstallMethod -eq "mise") { mise use "core@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf global core "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { Write-Output "pkgx does not use explicit versions this way"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox use "core@$CompVersion"; exit 0 }
...
$TargetDir = Join-Path (Join-Path $LibscriptHome "core") $ExactVersion
$AliasDir = Join-Path (Join-Path $LibscriptHome "core") $CompVersion
...
if ($Action -eq "download") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Downloading core $CompVersion to $DownloadDir\core..."
        $CompDownloadDir = Join-Path $DownloadDir "core"
        if (-not (Test-Path $CompDownloadDir)) {
            New-Item -ItemType Directory -Force -Path $CompDownloadDir | Out-Null
        }
        if ($env:CORE_DOWNLOAD_URL) {
            Invoke-WebRequest -Uri $env:CORE_DOWNLOAD_URL -OutFile "$CompDownloadDir\core-$CompVersion.zip"
        } else {
            Write-Output "CORE_DOWNLOAD_URL is not defined. Skipping."
        }
    }
    exit 0
}
...
if ($Action -eq "install") {
    ...
    if ($InstallMethod -eq "mise") { mise install "core@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "asdf") { asdf install core "$CompVersion"; exit 0 }
    if ($InstallMethod -eq "pkgx") { pkgx install "core@$CompVersion"; exit 0 }
    if ($InstallMethod -eq "vfox") { vfox add core; vfox install "core@$CompVersion"; exit 0 }

    $TargetDir = Join-Path (Join-Path $LibscriptHome "core") $CompVersion
    $TargetBin = Join-Path $TargetDir "bin"
    if (-not (Test-Path $TargetBin)) {
        Write-Output "Installing core $CompVersion natively to $TargetDir..."
        New-Item -ItemType Directory -Force -Path $TargetBin | Out-Null
        $CacheFileZip = "$DownloadDir\core\core-$CompVersion.zip"
        $CacheFileTar = "$DownloadDir\core\core-$CompVersion.tar.gz"
        ...
        } elseif ($env:CORE_DOWNLOAD_URL) {
            Write-Output "Downloading and extracting..."
            $TempFile = Join-Path $env:TEMP "core.zip"
...
if ($Action -match "^(start|stop|restart|status|health|logs|up|down)$") {
    if ($InstallMethod -eq "libscript_native" -or $InstallMethod -eq "system") {
        ...
        if ($env:LIBSCRIPT_SERVICE_NAME) { $ServiceName = $env:LIBSCRIPT_SERVICE_NAME } elseif ($env:PACKAGE_NAME) { $ServiceName = "libscript_$($env:PACKAGE_NAME)" } else { $ServiceName = "libscript_core" }
...
if ($Action -eq "uninstall") {
    if ($InstallMethod -eq "libscript_native") {
        Write-Output "Uninstalling core $CompVersion..."
        $TargetDir = Join-Path (Join-Path $LibscriptHome "core") $CompVersion
