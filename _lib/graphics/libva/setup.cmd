...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.libva"
if exist "%STAMP_FILE%" (
    echo [SKIP]  libva already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging LibVA into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    libva staged successfully: %STAMP_FILE%
