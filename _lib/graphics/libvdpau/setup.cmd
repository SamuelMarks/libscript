...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.libvdpau"
if exist "%STAMP_FILE%" (
    echo [SKIP]  libvdpau already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging LibVDPAU into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    libvdpau staged successfully: %STAMP_FILE%
