#!/bin/sh
# ## Overview
# Recipe for LibVDPAU: Video Decode and Presentation API for Unix.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.libvdpau"

if [ ! -d "$STAMPS_DIR" ]; then
  mkdir -p "$STAMPS_DIR"
fi

if [ ! -f /usr/lib/libvdpau.so.1 ] && [ ! -f /usr/lib64/libvdpau.so.1 ] && [ ! -f /lib/libvdpau.so.1 ]; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache libvdpau || true
    ...
    priv apt-get install -y libvdpau1 || priv apt-get install -y libvdpau || true
    ...
    priv dnf install -y libvdpau || (priv dnf install -y epel-release && priv dnf install -y libvdpau) || true
  fi
fi
...
' "LibVDPAU" "$TARGET_SYSROOT" "$ACTION"
