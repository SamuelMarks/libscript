...
::
:: ## Overview
:: Recipe for Mesa 3D Graphics: Open-source OpenGL and Vulkan driver collection.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Mesa 3D Graphics.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.mesa"
if exist "%STAMP_FILE%" (
    echo [SKIP]  mesa already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Mesa 3D Graphics into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    mesa staged successfully: %STAMP_FILE%
