...
::
:: ## Overview
:: Recipe for Intel Media Driver: VAAPI hardware accelerated video driver for Intel.
...
if "%~1"=="--help" (
    ...
    echo Recipe for Intel Media Driver.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.intel-media-driver"
if exist "%STAMP_FILE%" (
    echo [SKIP]  intel-media-driver already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Intel Media Driver into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    intel-media-driver staged successfully: %STAMP_FILE%
