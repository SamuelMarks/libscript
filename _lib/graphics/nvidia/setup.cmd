...
::
:: ## Overview
:: Recipe for NVIDIA Graphics Driver: Proprietary NVIDIA display driver and libraries.
...
if "%~1"=="--help" (
    ...
    echo Recipe for NVIDIA Graphics Driver.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.nvidia"
if exist "%STAMP_FILE%" (
    echo [SKIP]  nvidia already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging NVIDIA Graphics Driver into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    nvidia staged successfully: %STAMP_FILE%
