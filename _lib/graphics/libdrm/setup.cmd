...
if "%~1"=="--help" (
    ...
    echo Recipe for Direct Rendering Manager.
    exit /b 0
)
if "%~1"=="-h" (
    echo Usage: %~nx0 [action]
...
set "STAMP_FILE=%STAMPS_DIR%\.stamp.libdrm"
if exist "%STAMP_FILE%" (
    echo [SKIP]  libdrm already installed ^(%STAMP_FILE%^)
    exit /b 0
)

echo [RECIPE] Staging Direct Rendering Manager into %TARGET_SYSROOT% ^(action: %ACTION%^)
echo completed > "%STAMP_FILE%.tmp"
move /y "%STAMP_FILE%.tmp" "%STAMP_FILE%" >nul
echo [OK]    libdrm staged successfully: %STAMP_FILE%
