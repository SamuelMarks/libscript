#!/bin/sh
# ## Overview
# Recipe for Direct Rendering Manager: Low-level userspace interface to kernel DRM.
...
STAMP_FILE="${STAMPS_DIR}/.stamp.libdrm"

if [ ! -d "$STAMPS_DIR" ]; then
  mkdir -p "$STAMPS_DIR"
fi

if [ ! -f /usr/lib/libdrm.so.2 ] && [ ! -f /usr/lib64/libdrm.so.2 ] && [ ! -f /lib/libdrm.so.2 ]; then
  if command -v apk >/dev/null 2>&1; then
    ...
    priv apk add --no-cache libdrm || true
    ...
    priv apt-get install -y libdrm2 || true
    ...
    priv dnf install -y libdrm || true
  fi
fi
...
' "Direct Rendering Manager" "$TARGET_SYSROOT" "$ACTION"
