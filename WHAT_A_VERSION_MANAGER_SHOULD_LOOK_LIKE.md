...
1. **Default Method**: The fallback/default installation method must be `libscript-native` (meaning
...
1. Update the default value for `<COMPONENT>_INSTALL_METHOD` from `system` to `libscript-native`.
...
variable.** If the method is `libscript-native` (the libscript default), it executes the native
logic defined below. If the method is `system`, `mise`, or `asdf`, the scripts must dynamically
branch to delegate the lifecycle command to that specific package/version manager.
...
- **`ls`**: Lists all locally installed versions. For `libscript-native`, it inspects the
  `${LIBSCRIPT_HOME}/<component>/` directory, indicating aliases and the active version. For
  `mise`/`asdf`, it delegates to `mise ls <comp>` / `asdf list <comp>`.
- **`ls-remote`**: Queries the official upstream API or release index to list available versions
  that can be downloaded and installed. For delegates, calls `mise ls-remote` / `asdf list all`.
...
- **`install-daemon`**: Registers the component as a persistent background service using the
  OS-native init system (e.g., `systemd`, `launchd`, or Windows Services).
- **`uninstall-daemon`**: Stops the daemon if running and safely unregisters/removes it from the
  host OS.
- **`uninstall`**: Completely removes the installed version. For `libscript-native`, it removes the
...
defaults to `libscript-native`.

Components dynamically evaluate their method fallback sequence as follows:
`<COMPONENT>_INSTALL_METHOD` -> `LIBSCRIPT_DEFAULT_INSTALL_METHOD` -> `libscript-native`

Supported methods:

- **`libscript-native`**: (Default) The libscript-native isolated version manager logic detailed in
  this document.
...
While `libscript-native` is the strict standard, exceptions are permitted for tools with profound OS
