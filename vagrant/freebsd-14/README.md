...
## Usage
**Purpose**: This document describes the `freebsd-14` Vagrant folder and environment within the LibScript ecosystem. LibScript is a modular, zero-dependency shell-script framework designed for cross-platform software provisioning.
...
For example—assuming you've exported `vagrant ssh-config` and named it `freebsd-14`—run:
...
You can override the global setting for specific dependencies by setting their respective `FREEBSD_14_INSTALL_METHOD` variable. The local override takes highest precedence. 
