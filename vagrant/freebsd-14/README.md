...
**Purpose**: This document describes the `freebsd-14` Vagrant folder and environment within the
...
- **Install**: `libscript install vagrant/freebsd-14`
- **Start**: `libscript start vagrant/freebsd-14`
- **Stop**: `libscript stop vagrant/freebsd-14`
- **Uninstall**: `libscript uninstall vagrant/freebsd-14`
- **Package**: `libscript package vagrant/freebsd-14`
...
For some reason the -c syntax doesn't work so you have to use `<<<` which limits what host shell you
are using.

Also, you need to manually copy files over as the vagrant/hypervisor folder sync feature doesn't
work. For example—assuming you've exported `vagrant ssh-config` and named it `freebsd-14`—run:

rsync -avz --exclude='\*\*/.vagrant' /path/to/libscript freebsd-14:/opt/repos/
...
`FREEBSD_14_INSTALL_METHOD` variable. The local override takes highest precedence.
