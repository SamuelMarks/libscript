@echo off
set "THIS_FILE=%~f0"
:: # run_alpine_tests.cmd
::
:: ## Overview
:: Native batch script to execute component tests sequentially on Alpine Linux Vagrant VM.
::
:: ## Usage
:: run_alpine_tests.cmd [TARGETS...|all]
...
set "VAGRANT_DIR=%REPO_ROOT%\vagrant\alpine-3.24"
...
:show_help
echo Usage: run_alpine_tests.cmd [TARGETS...^|all]
echo.
echo Runs local tests sequentially on Alpine Linux Vagrant VM.
...
:run_tests
echo === Ensuring Alpine Linux Vagrant VM is running ===
pushd "%VAGRANT_DIR%"
vagrant status | findstr /i "running" >nul 2>&1
if errorlevel 1 (
    echo Starting Alpine Linux Vagrant VM...
...
set "stdout_file=%TESTS_TMP_DIR%\!target!.linux.alpine.stdout"
set "stderr_file=%TESTS_TMP_DIR%\!target!.linux.alpine.stderr"
set "success_file=%TESTS_TMP_DIR%\!target!.linux.alpine.success"
set "failure_file=%TESTS_TMP_DIR%\!target!.linux.alpine.failure"
