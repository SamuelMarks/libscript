...
set "THIS_FILE=%~f0"
:: # run_debian_tests.cmd
::
:: ## Overview
:: Runs component installation and test verification sequentially for libscript
:: components on a local Debian Linux Vagrant VM (`bento/debian-13`).
::
:: ## Usage
:: tests\run_debian_tests.cmd [TARGETS...|all]

set "THIS_DIR=%~dp0"
if "%THIS_DIR:~-1%"=="" set "THIS_DIR=%THIS_DIR:~0,-1%"

set "REPO_ROOT=%THIS_DIR%\.."
set "VAGRANT_DIR=%REPO_ROOT%\vagrant\debian-13"
...
:run_tests
echo === Ensuring Debian Linux Vagrant VM is running ===
pushd "%VAGRANT_DIR%"
vagrant status | findstr /i "running" >nul 2>&1
if errorlevel 1 (
    echo Starting Debian Linux Vagrant VM...
...
set "stdout_file=%TESTS_TMP_DIR%\!target!.linux.debian.stdout"
set "stderr_file=%TESTS_TMP_DIR%\!target!.linux.debian.stderr"
set "success_file=%TESTS_TMP_DIR%\!target!.linux.debian.success"
set "failure_file=%TESTS_TMP_DIR%\!target!.linux.debian.failure"
