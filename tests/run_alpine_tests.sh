#!/bin/sh
# ## Overview
# Runs tests sequentially for libscript components on a local Alpine Linux Vagrant VM
# (`bento/alpine-3.24`). Restores the vanilla snapshot before each test, verifies
...
#
# ## Usage
# ./tests/run_alpine_tests.sh [TARGETS...|all]
# Example: ./tests/run_alpine_tests.sh curl sqlite jq
# Example: ./tests/run_alpine_tests.sh all
# Example: ./tests/run_alpine_tests.sh all --loop --stop-after 10
...
VAGRANT_DIR="$REPO_ROOT/vagrant/alpine-3.24"
...
# ## find_disk_image
# Finds the active qcow2 disk image for the Alpine Vagrant machine.
find_disk_image() {
...
restore_vanilla_snapshot() {
    ...
    if [ -n "$_img" ] && command -v qemu-img >/dev/null 2>&1; then
        if qemu-img snapshot -U -l "$_img" 2>/dev/null | grep -q "vanilla"; then
            echo "Halting Alpine VM to restore vanilla snapshot..."
            (cd "$VAGRANT_DIR" && vagrant halt) || true
            echo "Restoring vanilla snapshot..."
            qemu-img snapshot -a vanilla "$_img"
            echo "Starting Alpine Vagrant VM from vanilla snapshot..."
...
# ## sync_repo_to_guest
# Synchronizes the libscript codebase to the guest Alpine VM using rsync over SSH.
sync_repo_to_guest() {
    echo "=== Syncing LibScript repository to Alpine Linux ==="
...
# Updates the status of a single component in the Linux (apk) column of README.md.
update_component_in_readme() {
    _comp="$1"
    _status="$2"
...
while [ $# -gt 0 ]; do
    case "$1" in
        --help|-h|/?)
            ...
            echo ""
            echo "Runs local tests sequentially on Alpine Linux (bento/alpine-3.24) Vagrant VM."
            echo ""
            echo "Arguments:"
            ...
            echo ""
            echo "Results are written to tests_tmp/ (*.linux.alpine.stdout, *.linux.alpine.stderr, *.linux.alpine.success/failure)."
            exit 0
            ;;
        --loop|-l)
            LOOP_MODE="1"
...
if [ ! -d "$VAGRANT_DIR" ]; then
    echo "Error: Alpine Vagrant environment not found at $VAGRANT_DIR"
    exit 1
fi
...
while true; do
    if [ "$LOOP_MODE" = "1" ]; then
        echo "============================================================"
        echo "Starting Alpine Test Loop (Iteration $iteration)"
        echo "============================================================"
    fi
    ...
    echo "=== Ensuring Alpine Vagrant VM is running ==="
    cd "$VAGRANT_DIR"
    ...
    if ! echo "$vm_status" | grep -q "running"; then
        if [ -n "$DISK_IMG" ] && [ "$USE_SNAPSHOT" = "1" ]; then
            ...
            echo "Starting Alpine Vagrant VM..."
    ...
    for target in $UNIQUE_TARGETS; do
        ...
        if [ -n "$MANIFEST_PATH" ]; then
            ...
            /"os_blacklist"\s*:/ {
                in_bl=1; in_wl=0
                if ($0 ~ /"alpine"/ || $0 ~ /"linux"/) { result="no"; exit }
                if ($0 ~ /\]/) { in_bl=0 }
                next
            }
            /"os_whitelist"\s*:/ {
                in_wl=1; in_bl=0; has_wl=1
                if ($0 ~ /"alpine"/ || $0 ~ /"linux"/ || $0 ~ /"all"/) { wl_match=1 }
                if ($0 ~ /\]/) { in_wl=0 }
                next
            }
            /"arch_whitelist"\s*:/ {
            ...
            if [ "$SUPPORTED" = "no" ]; then
                echo "Skipping $target (not supported on alpine / aarch64)"
        ...
        echo "============================================================"
        echo "Running test for $target on Alpine Linux..."
        echo "============================================================"
        ...
        test_cmd="export LIBSCRIPT_ROOT_DIR=/opt/repos/libscript; timeout 600 /opt/repos/libscript/libscript.sh install $target && timeout 120 /opt/repos/libscript/libscript.sh test $target && timeout 600 /opt/repos/libscript/libscript.sh install $target && timeout 120 /opt/repos/libscript/libscript.sh test $target"
        ...
        if [ "$USE_SNAPSHOT" != "1" ] && [ -z "$SKIP_UNINSTALL" ]; then
            cleanup_cmd="export LIBSCRIPT_ROOT_DIR=/opt/repos/libscript; /opt/repos/libscript/libscript.sh uninstall $target >/dev/null 2>&1 || true; rm -rf /home/vagrant/.libscript/$target 2>/dev/null || true"
    ...
    echo ""
    echo "============================================================"
    echo "Alpine Linux Tests Complete (Iteration $iteration)"
