...
This document describes the `dockerfiles-ssh` directory within the LibScript ecosystem. This folder
contains Dockerfiles and related configuration for setting up SSH servers via Docker.

It works both as a local version manager (similar to rvm, nvm, pyenv, uv) and can be invoked from
the global version manager `libscript`. Additionally, it can be used by libscript to build bigger
stacks (like WordPress, Open edX, nextcloud, etc.) by providing secure SSH access to the underlying
infrastructure.

You can manage this component using the global `libscript` CLI or the local `cli.sh`/`cli.cmd`.

- **Install:** `libscript install dockerfiles-ssh`
...
`DOCKERFILES_SSH_INSTALL_METHOD` variable. The local override takes highest precedence.
